...
`hsb-npc-training`의 H100 4장과 `wo-pvc-2`의 H100 2장을 묶어 `semi_control_stable_w_val` 최신 prefix-valid default no-slip control-space pretrain을 돌릴 때는 아래 launcher를 씁니다. 이 launcher는 기존 running pod 안의 tmux session과 학습 프로세스만 만들거나 교체하며, pod를 새로 만들거나 재시작하지 않습니다.
...
| metadata | `${REMOTE_LOG_DIR}/dataset_metadata/womd_training_validation_memory_balance_h100x6_hsb_wo_pvc2.pt` preflight 생성/검증 |

이 브랜치는 최종 test 제출용 full-data pretrain을 위해 train dataloader만 `training + validation`으로 확장합니다. validation pkl은 학습용으로 한 번, 기존 validation metric용으로 한 번 별도 경로에서 읽습니다. 따라서 학습 중 Fast-RMM/validation 계산은 기존처럼 validation split만 대상으로 수행되지만, 그 validation split은 더 이상 held-out 일반화 지표가 아니라 학습에 포함된 split에 대한 sanity/checkpoint 지표입니다. validation pkl을 training 폴더로 복사하거나 symlink하지 않으며, memory-balanced metadata도 training-only cache와 섞이지 않도록 `womd_training_validation_*` 파일명을 씁니다.
...
`hsb-npc-training-2`의 H100 4장과 `wo-pvc-1`의 H100 2장을 묶어 같은 `semi_control_stable_w_val` control-space pretrain을 돌릴 때는 아래 전용 wrapper를 씁니다. 이 wrapper도 기존 running pod 안의 tmux session과 학습 프로세스만 만들거나 교체하며, pod를 새로 만들거나 재시작하지 않습니다.
...
| cache root | 두 pod 모두 `/workspace/womd_v1_3/SMART_cache` |
| train split | `${CACHE_ROOT}/training` + `${CACHE_ROOT}/validation` |
| validation split | 기존과 동일하게 `${CACHE_ROOT}/validation` |
...
| metadata | `${REMOTE_LOG_DIR}/dataset_metadata/womd_training_validation_memory_balance_h100x6_hsb2_wo1.pt` preflight 생성/검증 |
...
- `data.train_memory_balance_metadata_cache=${paths.log_dir}/dataset_metadata/womd_training_validation_memory_balance_v1.pt`
...
##### V100 47GPU latest semi_control_stable_w_val tail-prefix run

기존 W&B run인 `flow_control_space_pretrain_v100x47_prefix_default_noslip_tailprefix_roundtrip05_lr6e-4_bs4`와 설정은 맞추되, `semi_control_stable_w_val` 최신 코드로 새 실험을 시작하려면 아래 wrapper를 씁니다. 이 wrapper는 같은 preset과 같은 V100 47GPU fleet를 쓰지만, 기존 run과 log/checkpoint/W&B task name이 섞이지 않도록 기본 task/session 이름만 분리합니다.
...
| branch | `semi_control_stable_w_val` 최신 head. `--git-ref`를 넘기지 않으면 각 pod에서 launch 시점의 branch head를 checkout |
...
| 기존 W&B run과의 차이 | 동일 계열 설정이지만 최신 `semi_control_stable_w_val` 코드 기준. 기존 task name을 재사용하지 않아 과거 run resume/log 혼선을 피함 |
...
- 기본 실행 브랜치: `semi_control_stable_w_val`
- V100x4 pod는 4 rank, V100x3 pod는 3 rank를 띄워 총 47 rank를 사용합니다.
- `trainer.precision=16-mixed`
- `model.model_config.decoder.flow_window_steps=20`
- `model.model_config.token_processor.use_kinematic_control_flow=true`
...
Self-forced fine-tuning은 학습 중 `self_forced.sampling.sample_steps` 값을 줄이지 않고도 평균 sampler 호출 수를 줄일 수 있습니다. 현재 repository 기본값은 `sample_steps=16`이며, 학습 rollout마다 DDP 전체 rank가 terminal denoising step `s` 하나를 공유합니다. rank0에서 뽑은 `s`를 모든 rank로 broadcast하므로, 모든 rank 안의 scenario/agent와 0.5초 commit block들은 같은 `s`를 사용합니다.
