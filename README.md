...
`hsb-npc-training`의 H100 4장과 `wo-pvc-1`의 H100 2장을 묶어 총 6 rank로 같은
...
`wo-pvc-1`에는 rank `4~5`를 직접 배정하고,
...
| 항목 | 값 |
|---|---|
| pod / GPU | `hsb-npc-training` 4GPU + `wo-pvc-1` 2GPU |
| total DDP ranks | 6 |
...
kubectl exec -it -n p-pnc wo-pvc-1 -c main -- tmux attach -t catk-smart-ntp-h100x4-h100x2
```
