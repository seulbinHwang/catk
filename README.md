...
-> 2초 GT pose/heading/velocity를 0.1초 단위 2D control로 변환
...
| train anchor 수 | 16 |
| control dim | `[acceleration, yaw_rate]` |
| MDG state dim | `[x, y, cos(yaw), sin(yaw), speed]` |
...
이 브랜치에서는 **사람, 자전거, 자동차를 모두 같은 MDG-style control-state dynamics로 처리**합니다. 보행자, 자전거, 자동차별 no-slip point나 보행자 전용 heading 변환식은 쓰지 않습니다.
...
GT pose/heading/velocity에서 0.1초 step별 acceleration과 yaw rate를 만들고, chunk 평균은 적용하지 않습니다. MDG state는 `[local_x, local_y, cos(local_yaw), sin(local_yaw), speed]`이며, 이전 3D control 방식의 `x/20`, `speed/10` state scaling을 쓰지 않습니다.
...
```text
${CACHE_ROOT}/semi_mdg_sidecar_v2_accel_yawrate/training/*.pkl
```
...
CACHE_ROOT=/workspace/womd_v1_3/SMART_cache \
SIDECAR_ROOT=/workspace/womd_v1_3/SMART_cache/semi_mdg_sidecar_v2_accel_yawrate \
...
```bash
TRAIN_SIDECAR_DIR=/workspace/womd_v1_3/SMART_cache/semi_mdg_sidecar_v2_accel_yawrate/training \
...
| check | result |
| --- | --- |
| sidecar format | `semi_mdg_token_flow_sidecar_v2_accel_yawrate` |
...
2026-06-03 KST의 아래 기록은 이전 3D no-slip control 구현 검증 기록입니다. 현재 구현은 위의 2D acceleration/yaw-rate MDG-style dynamics로 대체되었습니다.
