...
- 의미상 기존 학습 모델과 맞추기 위해 시간 차 scalar는 신호 상태 종류와 무관하게 주변 map-agent relation에 유지합니다.
...
### 7.5 V100x47 control-space checkpoint validation 자동 제출

`flow_control_space_pretrain_v100x47_prefix_roundtrip2_bs8`의 W&B checkpoint를
최신 `semi_control` 코드로 validation set 제출까지 한 번에 처리하려면 아래 래퍼를 사용합니다.

```bash
ssh user@10.60.188.78
tmux attach -t hsb-rl-train
```

tmux 안에서 새 window를 만든 뒤:
...
- `origin/semi_control`을 fast-forward로 동기화합니다.
- W&B artifact `jksg01019-naver-labs/SMART-FLOW/epoch-last-48v4vo86:v61`에서
  `epoch_last.ckpt`를 내려받습니다.
- checkpoint를
  `checkpoints/wandb/flow_control_space_pretrain_v100x47_prefix_roundtrip2_bs8/epoch-last-48v4vo86_v61/epoch_last.ckpt`
  에 캐시합니다.
- `sim_agents_sub_flow + action=validate`로 validation split 전체 제출 파일을 생성합니다.
- `waymo_submission.enabled=true`로 Waymo validation set에 자동 업로드합니다.

서버에 `secrets/waymo/waymo_storage_state.json`이 없으면 실행 초기에 rank 0이
storage state JSON 붙여넣기를 요청합니다. GUI 환경에서 먼저 아래 파일을 만든 뒤 내용을
그대로 붙여넣으면 됩니다.
...
실행이 끝나면 결과는
`logs/flow_control_space_pretrain_v100x47_prefix_roundtrip2_bs8_wosac_val_submit/runs/<timestamp>/sim_agents_2025_submission.tar.gz`
에 남습니다.

### 7.6 A100x4x2 control-space checkpoint Fast WOSAC 1680-scene validation on RTX 5090

`testa + testaa`에서 실행 중인
`flow_control_space_pretrain_a100x4x2_prefix_roundtrip2_lr6e-4_bs26`의 최신
`epoch_last.ckpt`를 `testa`에서 가져와, `user@10.60.188.78`의 RTX 5090 한 장으로
Fast WOSAC closed-loop validation을 1680개 scenario 규모로 돌리려면 아래 launcher를 사용합니다.

```bash
python scripts/launch_fast_wosac_val_a100x4x2_epoch_last_ssh_5090.py --replace
```

기본 동작:

- source checkpoint:
  `testa:/mnt/nuplan/projects/catk/logs/flow_control_space_pretrain_a100x4x2_prefix_roundtrip2_lr6e-4_bs26/runs/<latest>/checkpoints/epoch_last.ckpt`
...
```bash
ssh -t user@10.60.188.78 'tmux attach -t hsb-rl-train'
```

이미 원격 checkpoint가 최신이고 복사를 건너뛰고 싶으면:

```bash
python scripts/launch_fast_wosac_val_a100x4x2_epoch_last_ssh_5090.py --replace --skip-ckpt-copy
```

### 7.7 H100x4x2 full-valid epoch-31 checkpoint Fast WOSAC 1680-scene validation on RTX 5090

`hsb-npc-training + hsb-npc-training2`에서 실행 중인
`flow_control_space_pretrain_h100x4x2_fullvalid_roundtrip2_lr6e-4_bs26`의
0-index `epoch_031.ckpt`를 `hsb-npc-training`에서 가져와,
`user@10.60.188.78`의 RTX 5090 한 장으로 Fast WOSAC closed-loop validation을
1680개 scenario 규모로 돌리려면 아래 launcher를 사용합니다.

```bash
python scripts/launch_fast_wosac_val_h100x4x2_epoch031_ssh_5090.py --replace
```

기본 동작:

- source checkpoint:
  `hsb-npc-training:/mnt/nuplan/projects/catk/logs/flow_control_space_pretrain_h100x4x2_fullvalid_roundtrip2_lr6e-4_bs26/runs/2026-05-11_14-43-27/checkpoints/epoch_031.ckpt`
- remote repo: `/media/user/E/projects/catk`
- remote cache: `/media/user/E/dataset/womd_v1_3/SMART_cache`
- remote checkpoint cache:
  `/media/user/E/projects/catk/checkpoints/from_pods/flow_control_space_pretrain_h100x4x2_fullvalid_roundtrip2_lr6e-4_bs26/epoch_031.ckpt`
- tmux target: `hsb-rl-train:fast-wosac-h100x4x2`
- `scorer_scene_num=1680`, `n_rollout_closed_val=32`, `data.val_batch_size=4`
- control-space validation override:
