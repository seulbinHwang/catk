...
`testas` A100x7 pod에서 실행할 때는 아래 wrapper를 쓴다. 이 preset은 GPU당
`train_batch_size=8`을 직접 사용하고 RLFTSim 내부 gradient accumulation은 쓰지 않는다.
따라서 전체 effective scenario batch는 `7 GPUs * 8 = 56`이다. Fit-time validation은
closed-loop만 수행하고, validation rollout sampling은 top-k 32를 사용한다.
