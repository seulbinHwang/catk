...
### A100x4x2 멀티 노드 SMART NTP pretrain

`testa`와 `testaa`처럼 A100 4장이 붙은 pod 두 개에서 SMART NTP
...
pod 목록은 `testa testaa`, 원격 저장소 위치는
...
`semi_control_stable`의 A100x4x2 control-space pretrain recipe와 학습 실행 조건을 맞추기
...
A100x4x2 SMART NTP launcher는 기본으로 `CATK_ATTENTION_GRAPH_FP32=1`을 설정한다.
...
따라서 현재 기본값은 `CATK_ATTENTION_GRAPH_FP32=1`과 `data.train_batch_size=13`이다.
batch 14는 빠르지만 peak memory가 A100 80GB 한계에 너무 가까워 정식 pretrain 기본값으로
쓰지 않는다. `--accumulate-grad-batches 2`처럼 gradient accumulation을 켜는 override는
학습을 시작하기 전에 실패한다.

이 batch 13 전략은 `data.train_use_eval_agent_selection=false`인 학습 전용 target
...
2026-05-20 기준으로 `testa + testaa` A100 8장에서 아래 검증을 수행했다.
...
| 운영값 근접 DDP pretrain 검증 | `train_batch_size=13`, `hybrid`, `CATK_ATTENTION_GRAPH_FP32=1`, 50 train batch | 통과 |

50 train batch 검증은 `trainer/global_step=49`까지 정상 증가했고, `train/loss=6.38681`로
finite 값을 기록했다. CUDA OOM, Triton/CUDA extension build error, DDP hang,
`AttentionLayer` dtype mismatch는 발생하지 않았다. 다만 이 값은 validation을 끈 짧은
runtime integration 검증 결과이며, 최종 수렴 성능이나 전체 epoch 속도 benchmark로 해석하지
않는다.
...
batch 16은 epoch 0의 42% 이상까지 정상 진행했고, 관측 peak GPU memory는 약 74.2 GiB 수준이었다.
그래서 현재 A100x4x2 preset은 기본 batch를 16으로 운용한다.
...
`trainer.accumulate_grad_batches=1`을 유지한다. `data.train_batch_size`는 기본 16이고,
