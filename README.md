...
batch 24와 22는 train forward에서 CUDA OOM이 났고, batch 20은 짧은 train/validation
smoke는 통과했지만 긴 train smoke에서 CUDA OOM이 났다. batch 18도 epoch 후반의
map-agent attention에서 CUDA OOM이 났다. 현재 기본값은 같은 조건에서 1시간 이상 정상 진행하며
명확한 메모리 여유를 보인 `16`이다. `--accumulate-grad-batches 2`처럼 gradient accumulation을 켜는 override는
학습을 시작하기 전에 실패한다.

이 batch 16 전략은 `data.train_use_eval_agent_selection=false`인 학습 전용 target
