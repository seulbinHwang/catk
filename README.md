...
- 학습 중에는 non-finite parameter, `total_loss`, gradient를 fail-fast로 감지해
  ...
  zero-gradient로 건너뛰는 `model.model_config.nonfinite_fm_loss_policy=skip`을 사용합니다.
...
MLX wrapper는 기본적으로 `model.model_config.nonfinite_fm_loss_policy=skip`을 함께 넘깁니다. 특정 rank의 특정 batch에서 FP16 FM loss가 non-finite가 되면 그 rank의 해당 batch만 zero-gradient로 지나가고 `train/nonfinite_fm_loss_skipped`에 기록합니다. 예전처럼 즉시 중단시키고 싶으면 생성기에서 `--nonfinite-fm-loss-policy raise`를 쓰거나 wrapper 실행 전 `NONFINITE_FM_LOSS_POLICY=raise`를 지정하세요.
