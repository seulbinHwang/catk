...
- Closed-loop self-forcing curriculum. `closed_loop_sf_global_max_step=A` 가 1 이상이면 t=0 self-forced stage를 끝낸 뒤 추가 closed-loop stage를 A번 더 수행합니다. `trainer.max_epochs`는 t=0 stage의 총 epoch 수이며 `self_forced.start_epoch`, 초기 generated-estimator warmup, t=0 generator epoch을 모두 포함합니다. `trainer.max_closed_loop_epochs`는 추가 closed-loop stage 1개당 총 epoch 수이며, 생략하면 `trainer.max_epochs`와 같은 값으로 동작합니다. 각 추가 stage는 요청된 `estimator_warmup_epochs` 만큼의 generated-estimator warmup을 먼저 반복한 뒤, `trainer.max_closed_loop_epochs - estimator_warmup_epochs` epoch만큼 online Generator를 학습합니다. `self_forced.start_epoch`는 t=0 stage에만 적용되고 closed-loop stage 반복 길이에는 섞이지 않습니다. W&B bank hit로 초기 t=0 warmup이 skip되어도 추가 stage warmup은 요청값을 유지합니다. 각 추가 stage 시작 시 validation/test에 쓰는 EMA Generator weight를 online Generator로 복사하고, stale AdamW momentum이 남지 않도록 Generator optimizer state를 reset합니다. `closed_loop_sf_local_max_step=N` 은 0.5초 commit block 기준의 stage 폭입니다. 기본값 `closed_loop_see_all=false` 에서는 기존 동작을 유지합니다. 즉 mini-batch마다 DDP rank 0이 local pre-roll 길이 `M∈[1,N]` 을 하나 샘플해 모든 rank가 공유하고, stage s의 실제 EMA pre-roll 길이는 `(s - 1) * N + M` 입니다. N=4이면 stage 1은 0.5~2.0초, stage 2는 2.5~4.0초, stage 3은 4.5~6.0초 상태에서 self-forcing을 시작합니다. `closed_loop_see_all=true` 로 두면 stage s마다 누적 구간 전체에서 `K∈[0, s*N]` 을 샘플합니다. N=4이면 stage 1은 0~2.0초, stage 2는 0~4.0초, stage 3은 0~6.0초 상태를 모두 볼 수 있습니다. 두 모드 모두 EMA pre-roll은 `torch.no_grad()` 로 한 번의 decoder rollout 호출만 사용하고, 그 이후 generated-estimator warmup 또는 self-forcing loss에 쓰는 rollout은 online Generator로 다시 인코딩해 수행합니다. Warmup epoch에서는 online rollout도 `torch.no_grad()`로 만들어 `F_psi`만 적응시키고, generator 학습 epoch에서는 실제 self-forcing loss gradient가 online Generator로 흐릅니다.
- `model.model_config.self_forced.gradually_see=false` 가 기본값이며, false이면 위 closed-loop prefix 샘플 범위를 처음부터 전부 사용합니다. true이면 stage 안에서 epoch이 진행될수록 허용되는 local prefix upper bound를 1 block씩 점진적으로 엽니다. stage 총 epoch 수를 `E_s`, local 폭을 `N`이라 할 때 `K=max(1, round(E_s/N))`, `m(e)=min(N, 1+floor(e/K))` 입니다. `closed_loop_see_all=false` 에서는 stage `s`의 샘플 범위가 `[N(s-1)+1, N(s-1)+m(e)]` 이고, `closed_loop_see_all=true` 에서는 `[0, N(s-1)+m(e)]` 입니다. 예를 들어 `E_s=7`, `N=4`이면 local upper bound는 `1, 1, 2, 2, 3, 3, 4` 순서로 열립니다.
...
- Self-forced cosine LR decay. `model.model_config.self_forced.lr_cosine_final_ratio=1.0` 이 기본값입니다. Generator와 generated estimator optimizer는 매 epoch 시작 시 현재 curriculum epoch 위치에 맞는 LR로 갱신됩니다. 기본값에서는 epoch 0부터 마지막 실질 self-forced epoch까지 시작 LR을 유지합니다. decay를 쓰려면 예를 들어 `0.01` 로 override하면 됩니다. `estimator_warmup_epochs=0`, `max_epochs=5`, `max_closed_loop_epochs=5`, `closed_loop_sf_global_max_step=4`, `lr_cosine_final_ratio=0.01`이면 실질 self-forced 길이는 `25` epoch이고, epoch 0은 시작 LR, epoch 24는 시작 LR의 1%가 됩니다. `max_closed_loop_epochs`를 `3`으로 줄이면 같은 설정의 총 epoch은 `5 + 4 * 3 = 17` 입니다. `action=fit ckpt_path=...` 로 중간 재개해도 checkpoint의 epoch 번호 기준으로 같은 cosine 위치의 LR을 다시 계산합니다.
...
이때 cosine LR은 20 epoch 전체를 하나의 curriculum으로 보고 epoch 19에서 시작 LR의 1%가 됩니다.
`trainer.max_closed_loop_epochs`를 별도로 override하지 않으면 위처럼 `max_epochs`와 같은 5로 취급합니다.
예를 들어 `trainer.max_closed_loop_epochs=3` 을 추가하면 전체 길이는 `5 + 3 * 3 = 14` epoch이고,
각 closed-loop stage는 warmup 포함 3 epoch만 실행합니다.
...
같은 fm-sf-5 H100 8GPU recipe에서 cosine LR decay의 final ratio만 `1.0`으로 둔 비교 실험은
아래 launcher로 실행합니다. 이 설정은 기존 `lr_cosine_final_ratio=0.01` 실험과 동일한
epoch-5 resume checkpoint, closed-loop curriculum, batch, validation 범위를 쓰며,
학습 내내 generator와 generated estimator LR이 시작값 `7e-5`로 유지되는지 확인하는 목적입니다.

```bash
python scripts/launch_closed_loop_self_forced_h100x8_fmsf5_gradually_see_maxcl10_lr1_static_pod.py --replace
```
...
| train batch | per-rank `72`, effective global batch `576` |
| OOM fallback | `72 -> 70 -> 68 -> ... -> 2`, latest self-forced checkpoint resume |
| lr | Generator `7e-5`, generated estimator `7e-5` |
| lr decay | `model.model_config.self_forced.lr_cosine_final_ratio=1.0` |
...
| gradually-see | `gradually_see=true`, `closed_loop_see_all=true` |
...
| val/test batch | per-rank `8` |
| tmux session | `catk-clsf-h100x8-fmsf5-graduallysee-maxcl10-lrdecay1` |

testa/testaa A100x4x2에서는 같은 fm-sf-5 gradually-see maxcl10 resume recipe를 유지하되,
cosine LR decay의 final ratio만 `0.1`로 바꾼 비교 실험을 아래 launcher로 실행합니다. 이 launcher는
pod를 만들거나 재시작하지 않고, 기존 `testa`, `testaa` pod 안에 같은 tmux session과 2-node
`torchrun`을 시작합니다.

```bash
python scripts/launch_closed_loop_self_forced_a100x4x2_testa_testaa_gradually_see_lr01_static_pods.py --replace
```
...
| validation | `val_closed_loop=true`, `val_open_loop=false`, `limit_val_batches=0.1`, `scorer_scene_num=1680` |
| val/test batch | per-rank `8` |
| tmux session | `catk-clsf-a100x4x2-testa-testaa-graduallysee-maxcl10-lrdecay01` |
...
fm-sf-3 H100 8GPU에서는 같은 see-all maxcl10 LR 0.1 recipe에서
`gradually_see=true`만 비교하려면 아래 launcher를 사용합니다.

```bash
python scripts/launch_closed_loop_self_forced_h100x8_fmsf3_gradually_see_lr01_maxcl10_static_pod.py --replace
```
...
| validation | `val_closed_loop=true`, `val_open_loop=false`, `limit_val_batches=0.1`, `scorer_scene_num=1680` |
| val/test batch | per-rank `8` |
| tmux session | `catk-clsf-h100x8-fmsf3-graduallysee-maxcl10-lrdecay01` |
