...
- per-GPU train batch: `4`
- global train batch: `4 x 16 = 64`
- validation batch: `4`, 그래서 한 validation batch의 scene 수가 `4 x 16 = 64`로 H100x4의 `16 x 4 = 64`와 같습니다.
- OOM이 나면 launcher가 `4 -> 2` 순서로 per-GPU batch를 낮추고, 최신 `epoch_last.ckpt`로 `action=fit` 재개합니다.
...
python scripts/launch_self_forced_v100x4x4_static_pods.py \
  ...
  --limit-val-batches 0 \
  --max-epochs 1 \
  --task-name flow_self_forced_v100x4x4_unfrozen_except_map_encoder_estimator_warmup_1_bs4_smoke
```
...
#### 2-node x 7 V100 static pod SiD-lite 실행

이미 떠 있는 `testv`, `testvv` pod를 그대로 사용해서 V100 7장 x 2노드 SiD-lite self-forced fine-tuning을 돌릴 때는 아래 preset과 launcher를 사용합니다.

```text
configs/experiment/self_forced_npfm_sid_v100x7x2.yaml
scripts/launch_self_forced_sid_v100x7x2_static_pods.py
```

이 preset은 `self_forced_npfm_sid_h100_4.yaml`의 실험 조건을 유지하되 V100 32GB x 14 ranks에 맞게 하드웨어 관련 값만 바꿉니다.

- 핵심 실험 조건: `distribution_matching_objective=sid`, `unfrozen_range=except_map_encoder`, `estimator_warmup_epochs=1`
- precision: V100은 bf16을 지원하지 않으므로 `trainer.precision=16-mixed`
- per-GPU train batch: `4`
- global train batch: `4 x 14 = 56`. `bs=8`은 V100 32GB에서 epoch 1 generated-estimator update 중 OOM이 확인되어 기본값에서 제외했습니다.
- validation batch: `4`, 그래서 한 validation batch의 scene 수가 `4 x 14 = 56`으로 H100x4의 `16 x 4 = 64`와 가깝습니다.
- scorer scene 수: `280`, world size 14와 val batch 4에서 official scorer 5 batch가 됩니다.
- OOM이 나면 launcher가 모든 pod의 attempt status를 rank-0 retry sync server로 모은 뒤, 어느 노드에서든 OOM이 확인되면 전체 노드가 함께 `4 -> 2` 순서로 per-GPU batch를 낮추고 최신 `epoch_last.ckpt`로 `action=fit` 재개합니다.
...
```bash
python scripts/launch_self_forced_sid_v100x7x2_static_pods.py --replace
```

smoke test:

```bash
python scripts/launch_self_forced_sid_v100x7x2_static_pods.py \
  --replace \
  --limit-train-batches 20 \
  --limit-val-batches 0 \
  --max-epochs 1 \
  --task-name flow_self_forced_sid_v100x7x2_unfrozen_except_map_encoder_estimator_warmup_1_bs4_smoke
```

attach:

```bash
kubectl exec -it -n p-pnc testv -c main -- tmux attach -t catk-sf-sid-v100x7x2-exceptmap-warmup1
```

중지:

```bash
python scripts/launch_self_forced_sid_v100x7x2_static_pods.py --stop
```

#### 5-node x 3 V100 static pod 실행

이미 떠 있는 `fv`, `fvv`, `fvvv`, `fvvvv`, `fvvvvv` pod를 그대로 사용해서 V100 3장 x 5노드 self-forced fine-tuning을 돌릴 때는 아래 preset과 launcher를 사용합니다.

```text
configs/experiment/self_forced_npfm_v100x3x5.yaml
scripts/launch_self_forced_v100x3x5_static_pods.py
```

이 preset은 `self_forced_npfm_h100_4.yaml`의 self-forcing 실험 조건을 유지하되 V100 32GB x 15 ranks에 맞게 하드웨어 관련 값만 바꿉니다.

- 핵심 실험 조건: `unfrozen_range=except_map_encoder`, `estimator_warmup_epochs=4`
- precision: V100은 bf16을 지원하지 않으므로 `trainer.precision=16-mixed`
- per-GPU train batch: `4`
- global train batch: `4 x 15 = 60`
...
```bash
python scripts/launch_self_forced_v100x3x5_static_pods.py --replace
```

smoke test:

```bash
python scripts/launch_self_forced_v100x3x5_static_pods.py \
  --replace \
  --limit-train-batches 20 \
  --limit-val-batches 0 \
  --max-epochs 1 \
  --task-name flow_self_forced_v100x3x5_unfrozen_except_map_encoder_estimator_warmup_4_bs4_smoke
```

attach:

```bash
kubectl exec -it -n p-pnc fv -c main -- tmux attach -t catk-sf-v100x3x5-exceptmap-warmup4
```

중지:

```bash
python scripts/launch_self_forced_v100x3x5_static_pods.py --stop
```
...
V100 7장 x 2노드 static pod 실험은
`experiment=self_forced_npfm_sid_v100x7x2` 와
`scripts/launch_self_forced_sid_v100x7x2_static_pods.py` 를 사용합니다.
