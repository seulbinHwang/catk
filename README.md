...
`AttentionLayer`는 `CATK_ATTENTION_GRAPH_FP32=1`을 수동으로 설정하면 PyG graph attention
aggregation만 fp32로 계산할 수 있다. 다만 A100x4x2 SMART NTP launcher는 이 값을 기본으로
켜지 않는다. 기본값은 `CATK_ATTENTION_GRAPH_FP32=0`이며, 전역 `bf16-mixed` 설정에 맡긴다.
fp32 graph attention은 모델 구조, 파라미터 수, edge set, radius, loss target을 바꾸지는
않지만 A100 80GB에서 peak memory를 늘릴 수 있으므로, 정식 pretrain 기본 경로에서는 opt-in
실험 옵션으로만 둔다.
...
`testa/testaa`, A100 4장 x 2 pod, validation off, 기본 graph attention dtype
(`CATK_ATTENTION_GRAPH_FP32=0`), `data.train_use_eval_agent_selection=true` 조건에서
...
|---:|---|---:|---:|---:|---:|
| 13 | 통과 | 300 train step | `1.17~1.18 it/s` | 약 `72,429 MiB` | 약 `71~72시간` |
| 14 | CUDA OOM | 약 190 step 근처 실패 | `1.05~1.08 it/s` | 약 `80,747 MiB` | 사용 안 함 |

`7dd61c4`의 Fourier embedding compile 경로는 유지하지만, A100 80GB 장기 pretrain에서는
