...
따라서 현재 H100/A100 공통 pretrain 기본값은 `CATK_ATTENTION_GRAPH_FP32=1`,
`data.train_use_eval_agent_selection=true`, `data.train_batch_size=15`이다.
`--accumulate-grad-batches 2`처럼 gradient accumulation을 켜는 override는
학습을 시작하기 전에 실패한다.

주의: 위 batch 15 측정 일부는 legacy `data.train_use_eval_agent_selection=false`
target selection 기준 기록이다. 현재 main 기본값은 `true`이므로, batch capacity와
OOM 위험은 같은 agent selection 조건에서 다시 확인해야 한다. 다른
`train_use_eval_agent_selection` 값을 사용한 결과를 같은 학습 대상 agent set으로
해석하면 안 된다.
