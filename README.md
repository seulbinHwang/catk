...
이 config는 `semi_control_stable`의 WOSAC 제출 운영값을 main SMART NTP 기준으로
맞춘다. 제출 메타데이터는 기본으로 `authors: [Seulbin Hwang, Kiyoung Om]`,
`affiliation: NaverLabs`, `method_link: "not available yet"`,
`account_name: "h.sb@naverlabs.com"`을 사용한다. 모델과 방법론은 main의 SMART NTP이므로
`method_name`과 `description`은 SMART NTP 기준으로 둔다. 32-rollout 제출 export가
학습용 validation보다 더 큰 메모리를 쓰는 점을 고려해 `val_batch_size: 4`,
`test_batch_size: 8`, `precision: bf16-mixed`, `gradient_clip_val: 5.0`,
`prefetch_factor: 1`도 명시한다.
