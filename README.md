...
- Bounded Clean-DMD guidance 기본값 `clean_dmd_normalizer_eps=0.05`, `dmd_stable_scale_scope=agent`, `clean_dmd_tau_low=0.02`, `clean_dmd_tau_high=0.98` 을 함께 둡니다. `clean_dmd_normalizer_eps` 는 stable scale의 최소값입니다.
...
- self-forcing DMD/SID 본체의 초기 stage는 scene 안의 16개 rollout anchor를 모두 사용합니다. 이후 closed-loop 추가 stage에서만 `model.model_config.self_forced.rollout_anchor_stride` 간격을 적용합니다. 기본값 `2` 는 이후 stage에서 사람 기준 `1, 3, 5, 7, 9, 11, 13, 15`번째 anchor, 코드 기준 offset `0, 2, 4, 6, 8, 10, 12, 14`를 쓰며, `1` 로 두면 이후 stage도 16개 anchor를 모두 사용합니다.
- `fm-sf-quarter-1~4` H100x2x4 launcher는 기본적으로 초기 16-anchor self-forcing stage를 다시 돌리지 않습니다. 대신 W&B checkpoint `jksg01019-naver-labs/SMART-FLOW/epoch-last-81s5s6k3:latest`의 `epoch_last.ckpt`를 받아 그 다음 closed-loop stage부터 이어서 학습합니다. 이 checkpoint는 lr `5e-5`, effective train batch size `5 x 8 = 40`으로 학습된 epoch 5 / global step 106542 self-forcing checkpoint입니다.
- self-forced fine-tuning에서는 Generator optimizer와 generated estimator optimizer 모두 self-forced curriculum epoch 위치에 맞춘 cosine LR ratio를 적용합니다. 시작 LR은 각각 `model.model_config.lr`, `model.model_config.self_forced.generated_estimator_lr` 이고, 마지막 실질 self-forced epoch LR은 `model.model_config.self_forced.lr_cosine_final_ratio` 배가 됩니다. `closed_loop_sf_anchor` 기본값은 `1.0` 이라 override하지 않으면 기존처럼 constant LR로 동작합니다. 감쇠가 필요하면 launcher에서 `--lr-cosine-final-ratio 0.01` 처럼 명시합니다.
...
| `dmd_stable_scale_scope` | `agent` | `agent`는 agent별, `type`은 같은 scene 안 같은 agent type별, `scene`은 같은 scene 전체 agent별로 stable scale을 공유합니다. |
...
| `dmd_use_injection_ramp` | `false` | `true`이면 DMD target 주입량을 각 generator-DMD stage 안에서 \(\lambda_j=0.25+0.75\min(1,\frac{\max(0,j)}{2})\) 로 2 epoch 동안 키웁니다. `closed_loop_sf_global_max_step>0` 에서는 초기 anchor stage와 각 closed-loop stage마다 \(j=0\)부터 ramp를 다시 시작합니다. `false`이면 항상 \(\lambda=1\) 입니다. |
...
`(estimator_warmup_epochs, generated_estimator_lr)` 조합을 먼저 찾고, 정확히 일치하면 warmup을
...
| entry | generated estimator lr | source checkpoint | 포함 내용 |
|---:|---:|---|---|
...
bank key에 쓰는 lr은 Generator lr이 아니라 실제 실험에 적용되는
`model.model_config.self_forced.generated_estimator_lr` 입니다. launcher에서
`--generated-estimator-learning-rate` 또는 `CATK_GENERATED_ESTIMATOR_LR`를 지정하면
그 값이 generated estimator optimizer와 bank download/upload key에 함께 적용됩니다.
지정하지 않으면 기존 기본값처럼 `${model.model_config.lr}` 를 따라갑니다.
...
```text
1. W&B artifact에서 요청 warmup 이하 중 가장 큰 generated_estimator_lr=1e-6 entry 다운로드
...
#### fm-sf-1 H100x8 single-pod stride-1 DMD self-forcing fine-tuning

`p-sp-labs-reai-training` namespace의 `fm-sf-1` 단일 H100 8GPU pod에서 위
pvc H100x3x2 stride-1 설정과 같은 DMD-style self-forced fine-tuning을 돌릴 때는
아래 launcher를 씁니다. 이 launcher는 pod를 만들거나 지우거나 재시작하지 않고,
`fm-sf-1` 안의 tmux session과
`torchrun --nnodes=1 --nproc_per_node=8`만 시작합니다.

```bash
python scripts/launch_self_forced_dmd_h100x8_fmsf1_sf_anchor_static_pod.py --replace
```
...
python scripts/launch_self_forced_dmd_h100x8_fmsf1_sf_anchor_static_pod.py \
  --replace \
  --task-name flow_self_forced_dmd_h100x8_fmsf1_smoke_bs8 \
...
| precision | `bf16-mixed` |
| lr | Generator `5.0e-5`, generated estimator `5.0e-5` |
| estimator warmup | 요청값 `2` epoch, W&B generated-estimator warmup bank `generated-estimator-warmup-bank-pretrain-x5f9g0ce-v57-lr5e-5:latest` 사용. exact hit이면 warmup은 다운로드로 대체되고 실제 generator+DMD 학습 예산은 6 epoch로 조정됩니다. |
| DMD objective | `model.model_config.self_forced.distribution_matching_objective=dmd` |
...
#### fm-sf-4 H100x8 single-pod multi-anchor closed-loop curriculum

`closed_loop_sf_anchor` 브랜치에서 16개 rollout anchor 전체를 쓰는 closed-loop
self-forcing curriculum을 `p-sp-labs-reai-training/fm-sf-4` 단일 H100 8GPU pod에서
검증/학습하려면 아래 wrapper를 씁니다. pod를 만들거나 삭제하지 않고 tmux session만
시작합니다.

```bash
python scripts/launch_closed_loop_self_forced_h100x8_fmsf4_sf_anchor_static_pod.py --replace
```

빠른 smoke/probe:

```bash
python scripts/launch_closed_loop_self_forced_h100x8_fmsf4_sf_anchor_static_pod.py \
  --replace \
...
| 항목 | 값 |
|---|---|
| pod / namespace | `fm-sf-4` / `p-sp-labs-reai-training` |
...
#### wo-pvc-3-1 / wo-pvc-3-2 H100x3 self-forced launchers

`wo-pvc-3-1`, `wo-pvc-3-2` pod에서도 같은 H100x3 self-forced DMD recipe를
돌릴 수 있습니다. 기존 `hsb3n1/hsb3n2` launcher와 동작은 같고, pod / clean
checkout / task name / tmux session / checkpoint cache path만 별도로 분리됩니다.

```bash
python scripts/launch_self_forced_dmd_h100x3_wopvc31_static_pod.py --replace
python scripts/launch_self_forced_dmd_h100x3_wopvc32_static_pod.py --replace
```

기본 차이:

| 항목 | `wo-pvc-3-1` | `wo-pvc-3-2` |
|---|---|---|
...
```bash
kubectl exec -it -n p-pnc wo-pvc-3-1 -c main -- \
  tmux attach -t catk-self-forced-dmd-h100x3-wopvc31

kubectl exec -it -n p-pnc wo-pvc-3-2 -c main -- \
  tmux attach -t catk-self-forced-dmd-h100x3-wopvc32
```

학습 프로세스만 멈추고 pod는 그대로 두려면:

```bash
python scripts/launch_self_forced_dmd_h100x3_wopvc31_static_pod.py --stop
python scripts/launch_self_forced_dmd_h100x3_wopvc32_static_pod.py --stop
```
...
- `model.model_config.self_forced.use_distribution_matching_loss=false` 이면 self-rollout / generated estimator 업데이트 / DMD·SiD loss / estimator warmup을 모두 생략하므로, `estimator_warmup_epochs` 값이 있어도 warmup으로 들어가지 않습니다.
...
- Multi-anchor closed-loop self-forcing curriculum. `closed_loop_sf_global_max_step=A` 가 1 이상이면 초기 multi-anchor self-forcing stage 뒤에 closed-loop stage를 A개 추가합니다. 각 추가 stage는 요청된 `estimator_warmup_epochs` 만큼 generated-estimator warmup을 반복한 뒤, 초기 stage와 같은 수의 generator epoch을 학습합니다. `closed_loop_sf_local_max_step=N` 은 0.5초 block 기준 stage 폭입니다. 기본 `closed_loop_see_all=false` 에서는 각 valid rollout anchor lane이 자기 anchor 시점을 기준으로 stage `s`에서 `(s - 1) * N + M` 개 block을 EMA Generator로 한 번에 no-grad pre-roll합니다. 여기서 `M`은 rank 0이 `[1, N]`에서 샘플해 모든 DDP rank가 공유합니다. `closed_loop_see_all=true` 에서는 stage `s`마다 각 anchor lane의 자기 anchor 시점 기준 `K∈[0, s*N]` 를 샘플하므로, stage 2는 0초부터 `2N*0.5초`까지 누적 구간 전체에서 시작 상태를 볼 수 있습니다. `gradually_see=false` 가 기본값이며, false이면 위 prefix 샘플 범위를 처음부터 전부 사용합니다. true이면 stage 안에서 epoch이 진행될수록 local prefix upper bound를 1 block씩 점진적으로 엽니다. stage 총 epoch 수를 `E_s`, local 폭을 `N`이라 할 때 `K=max(1, round(E_s/N))`, `m(e)=min(N, 1+floor(e/K))` 입니다. `closed_loop_see_all=false` 에서는 stage `s`의 각 anchor lane 샘플 범위가 `[N(s-1)+1, N(s-1)+m(e)]` 이고, `closed_loop_see_all=true` 에서는 `[0, N(s-1)+m(e)]` 입니다. 이 curriculum은 한 scene 안에서 병렬로 쓰는 anchor lane 수나 `rollout_anchor_stride`는 바꾸지 않고, 각 selected anchor lane의 pre-roll 시간 상한만 조절합니다. 두 모드 모두 16개 anchor lane은 하나의 anchor-expanded batch로 병렬 처리되고, prefix rollout도 불필요한 Python loop 없이 한 번의 EMA decoder rollout으로 수행됩니다. horizon이 부족해 prefix 끝 state가 invalid가 된 anchor lane은 self-forced loss에서 제외됩니다. stage 시작 시 EMA Generator weight를 online Generator로 복사하고 Generator optimizer state를 reset합니다. `update_open_loop_teacher_when_roll=false` 기본값에서는 `F_rho` teacher를 pretrained 기준점으로 고정합니다.
- Closed-loop stage에서 EMA pre-roll 뒤 `F_rho` / `F_psi` 가 noisy path의 clean estimate를 계산할 때도 마지막 두 generated state만 따로 축약한 context를 쓰지 않습니다. pre-roll이 끝난 시점의 validation-like rollout state를 조건으로 사용하므로, multi-anchor online Generator, generated estimator, teacher가 같은 generated history window를 공유합니다. 이 규칙은 `closed_loop_see_all=false` 와 `closed_loop_see_all=true` 모두에 적용됩니다.
- Self-forced cosine LR ratio는 단순 `trainer.max_epochs` 입력값이 아니라 위 closed-loop stage 확장 이후의 실제 curriculum 전체 epoch 수를 기준으로 계산합니다. `model.model_config.self_forced.lr_cosine_final_ratio=1.0` 이 기본값이라 기존 실험과 같은 constant LR이며, `0 < ratio <= 1` 범위의 값으로 override하면 Generator와 generated estimator optimizer가 같은 cosine ratio를 공유합니다. `action=fit ckpt_path=...` 로 중간 재개해도 checkpoint epoch 기준의 ratio를 다시 계산해 LR jump를 줄입니다.
...
python -m src.run experiment=self_forced_npfm action=finetune \
  ...
  data.train_epoch_sample_fraction_shuffle_flag=true
```
