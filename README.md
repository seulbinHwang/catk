...
- closed-loop inference에서 `model.model_config.decoder.use_stop_motion=false` 를 기본으로 사용하며,
  stop token 과 일치하는 agent도 다음 0.5초 chunk를 현재 상태로 강제 고정하지 않습니다.
- `model.model_config.decoder.use_lqr=true` 를 켜면 vehicle / bicycle에
...
### Map-Agent Edge Batch 정렬 보정

`SMARTAgentEncoder.build_map2agent_edge`의 `torch_cluster.radius` 호출은 batch index가 단조 비감소 순서로 들어온다는 silent 가정에 영향을 받습니다.
`flow_agent_decoder`는 여러 coarse step의 agent를 펼칠 때 `batch_s = tokenized_agent["batch"].repeat(n_step)` 형태로 같은 scene 번호 묶음을 step마다 반복합니다.
이 배열은 step 사이마다 큰 scene 번호에서 작은 값으로 떨어져 정렬 조건을 만족하지 못합니다.

정렬하지 않은 채 `radius`를 호출하면 다른 scene의 지도가 섞이기보다는, 같은 scene 안에서 만들어져야 할 map-agent edge가 조용히 일부 누락됩니다.
실제 재현 입력에서는 기대 edge 288개 중 126개만 생성되어, 162개가 빠졌습니다.
이 경우 일부 agent/time-step은 자기 scene의 차선, 도로 경계, 신호등 정보를 충분히 받지 못한 채 학습·추론됩니다.
WOSAC RMM의 map-based 항목(`offroad_indication_likelihood`, `distance_to_road_edge_likelihood`, `traffic_light_violation_likelihood`)에 직접 영향을 줄 수 있습니다.

현재 코드는 `radius` 호출 직전에 agent 쪽 batch와 map 쪽 batch를 각각 `argsort`로 정렬하고,
반환된 edge index를 원래 index 체계로 되돌립니다.
...
`train_use_eval_agent_selection=true`에서는 학습에서도 validation/추론과 같은 agent 선택 기준을 쓰므로 dense scene이 한 rank batch에 몰리면 CUDA peak가 튈 수 있습니다. 그래서 기본 pretrain도 memory-balanced batch sampler를 켭니다. sampler는 pkl별 agent 수 / current valid agent 수 / map point 수 같은 metadata로 batch 순서만 균형 있게 바꾸며, 학습 objective나 per-GPU batch size는 바꾸지 않습니다.
...
python tools/analyze_transition_alignment_error.py \
  --split training \
  --num-workers 48 \
  --files-per-task 512 \
  --progress-interval 10000 \
  --output-json outputs/transition_alignment_training.json
```
...
#### testsv/svv V100x4x2 execution-context balanced pretrain

`semi_control_rolling_gan` 브랜치에서 위 H100x6 4+2 pod 실험을 `testsv`, `svv` 두 V100x4 pod로 옮겨 돌릴 때는 아래 V100 전용 preset과 launcher를 씁니다. pod는 새로 만들거나 재시작하지 않고, 기존 running pod 안에 tmux session만 만듭니다.

```text
configs/experiment/pre_bc_flow_control_v100x4x2_execctx_balanced.yaml
scripts/launch_pre_bc_flow_control_v100x4x2_testsv_svv_execctx_balanced_oom_retry_static_pods.py
```
...
- 시작 per-GPU train batch: `2`, OOM fallback `2 -> 1`
- gradient accumulation: `7`
- optimizer-step effective global train batch: `2 x 8 x 7 = 112`
- learning rate: `2e-4`
...
```bash
python scripts/launch_pre_bc_flow_control_v100x4x2_testsv_svv_execctx_balanced_oom_retry_static_pods.py \
  --prebuild-metadata \
  --replace
```
...
python scripts/launch_pre_bc_flow_control_v100x4x2_testsv_svv_execctx_balanced_oom_retry_static_pods.py \
  --prebuild-metadata \
  --replace \
  --task-name flow_control_space_pretrain_v100x4x2_testsv_svv_smoke \
  --session catk-control-pretrain-v100x4x2-testsv-svv-smoke \
...
```bash
kubectl exec -it -n p-pnc testsv -c main -- tmux attach -t catk-control-pretrain-v100x4x2-testsv-svv-execctx-balanced
kubectl exec -it -n p-pnc svv -c main -- tmux attach -t catk-control-pretrain-v100x4x2-testsv-svv-execctx-balanced
```

실험 코드만 멈추고 pod는 그대로 두려면:

```bash
python scripts/launch_pre_bc_flow_control_v100x4x2_testsv_svv_execctx_balanced_oom_retry_static_pods.py --stop
```
...
이 preset은 `train_use_eval_agent_selection=true`로 학습에서도 validation/추론과 같은 agent 선택 기준을 쓰기 때문에, dense scene 몇 개가 한 rank의 local batch에 같이 들어가면 CUDA peak가 크게 튈 수 있습니다. 그래서 pkl별 `agent_count`, current-step valid agent 수, map polyline 수만 한 번 스캔해 metadata cache로 저장하고, 이후 epoch마다 무거운 scene이 local batch 안에 몰리지 않도록 balanced batch sampler가 index 순서만 재배치합니다. 모델, loss, per-rank batch size, effective global batch `188`은 바뀌지 않고, 학습 objective도 그대로입니다.
...
- `model.model_config.decoder.use_stop_motion=false`를 validation / test / submission inference의 기본값으로 고정합니다. Self-forced 학습 rollout의 stop-motion 여부도 `model.model_config.self_forced.use_stop_motion=false`를 기본으로 둡니다.
...
- stop-motion gate는 기본 비활성화되어 stop token 과 일치하는 agent 도 다음 0.5초 5점을 별도로 현재 상태에 고정하지 않습니다.
...
- `trainer.limit_val_batches`는 validation에 실제로 사용할 batch 양입니다. `0.1`이면 전체 validation batch의 10%, `1.0`이면 전체, 정수 `20`이면 앞 20 batch만 평가합니다. `scorer_scene_num`이 켜져 있고 이 값이 Fast WOSAC scorer에 필요한 batch 수보다 작다고 판단되면, 실행 중 scorer batch 수까지 자동으로 늘립니다.
...
# stop-motion gate 비활성 유지
... model.model_config.decoder.use_stop_motion=false

# self-forced 학습 rollout에서도 stop-motion gate 비활성 유지
... model.model_config.self_forced.use_stop_motion=false

# vehicle / bicycle curvature-LQR commit bridge만 적용하고 stop-motion은 비활성 유지
... model.model_config.decoder.use_stop_motion=false \
...
- monitored checkpoint 저장 시도는 validation이 도는 시점에 함께 일어납니다. 현재 `pre_bc_flow`는 `check_val_every_n_epoch=32` 이라 기본적으로 32 epoch마다 평가됩니다.
...
- 현재 `pre_bc_flow` 기본값은 validation이 `32` epoch마다 돌아 monitored checkpoint는 그 시점에만 갱신되지만, `epoch_last.ckpt` 는 매 epoch train loop가 끝나는 즉시 먼저 갱신됩니다.
...
학습 범위를 "validation/추론과 완전히 같은 기준"으로 넓히는 것이 아니라 기존 train 규칙 안에서만 학습 대상 수를 늘리는 실험은 이제 기본 preset에서 분리된 ablation으로 취급합니다. 그런 ablation에서는 150m 입력 제한과 ego 기준 100m 학습 대상 제한이 그대로 남으므로, 현재 브랜치의 기본 pretrain/fine-tuning 결과와 직접 비교할 때 agent 선택 기준 차이를 별도로 기록해야 합니다.
...
- `policy=paper_uniform` 은 실제 실행 denoising step `K` 를 `[min_executed_steps, sample_steps]` 범위에서 균등 샘플링합니다. 기본 `min_executed_steps=16` 이므로 `sample_steps=16` 에서는 `K=16` 만 사용합니다.
...
- `local_val_flow` 기본값은 `model.model_config.scorer_scene_num=1680`, `data.val_batch_size=4`, `trainer.limit_val_batches=420` 입니다. single GPU 기준으로 Fast WOSAC RMM을 약 `1680` scene에서 계산합니다.
- `scorer_scene_num`이 양의 정수이면 `model.model_config.n_batch_sim_agents_metric`는 자동 계산값으로 덮입니다. 따라서 “1600 scene으로 보고 싶다”면 `n_batch_sim_agents_metric`가 아니라 `model.model_config.scorer_scene_num=1600`을 지정합니다.
- 빠른 smoke check만 하고 싶으면 `model.model_config.scorer_scene_num=240 trainer.limit_val_batches=60`처럼 scorer scene 수와 validation batch 수를 함께 줄입니다.
...
- 현재 브랜치에서는 self-forced 학습 rollout과 validation / test / submission inference 모두 stop-motion gate를 쓰지 않습니다. 관련 설정은 `model.model_config.self_forced.use_stop_motion=false`, `model.model_config.decoder.use_stop_motion=false` 기준으로 통일합니다.
...
Self-forced fine-tuning은 학습 중 `self_forced.sampling.sample_steps` 값을 줄이지 않고도 평균 sampler 호출 수를 줄일 수 있습니다. `sample_steps=16`는 전체 denoising grid로 유지하고, 학습 rollout마다 DDP 전체 rank가 terminal denoising step `s` 하나를 공유합니다. rank0에서 뽑은 `s`를 모든 rank로 broadcast하므로, 모든 rank 안의 scenario/agent와 0.5초 commit block들은 같은 `s`를 사용합니다.
...
현재 브랜치의 표준 실험은 두 경로 모두 `use_stop_motion=false`로 통일합니다. 런처와 모델 내부 경로도 true 설정을 허용하지 않도록 맞춰 둡니다.
...
5. 선택된 future를 기존 WOMD `.pkl` schema와 같은 RoaD `.pkl` cache로 저장합니다. Traffic-light는 원본 현재 관측값을 static map feature로만 유지하며, cache 생성 중 미래 traffic-light 상태나 stale time scalar를 만들지 않습니다.
