...
- self-forced preset은 각 epoch마다 train dataset의 10%만 새로 랜덤 샘플링해 학습합니다. 비율은 `data.train_epoch_sample_fraction` 으로 바꾸며, `1.0` 으로 두면 전체 train dataset을 사용합니다.
...
| branch | `semi_control_stable` |
| experiment | `self_forced_npfm_h100_6` |
| task | `flow_self_forced_dmd_h100x6_hsb1_epoch061_x5f9g0ce_activecontrol_sample16_backprop8_lr1e-6_bs18_frac010_ep16_oomretry` |
...
| default task | `flow_self_forced_dmd_a100x4x2_testa_epoch061_x5f9g0ce_activecontrol_sample16_backprop8_lr1e-6_bs20_frac010_ep16_middle_oomretry` |
| pretrained checkpoint artifact | `jksg01019-naver-labs/SMART-FLOW/epoch-last-x5f9g0ce:v57` |
...
시간 감각은 다음처럼 보면 됩니다. 같은 `train_epoch_sample_fraction=0.1`에서
`bs160`은 epoch당 약 `39` train step이고, `bs20`은 global batch가 8배 작아져
epoch당 약 `312` train step이 됩니다. 이전 `bs18`, `train_epoch_sample_fraction=0.25`
측정에서 train-only는 epoch당 약 `142~143분`이었으므로, 같은 step 속도 가정에서는
`bs20`, fraction `0.1`의 train-only는 epoch당 약 `52분`, 16 epoch 약 `14시간`
수준입니다. 반면 `bs160`, fraction `0.1`은 실측 기준 train-only epoch당 약 `13분`,
...
- epoch별 train subset sampling. self-forced preset은 `data.train_epoch_sample_fraction=0.1` 를 기본으로 두어 매 epoch 전체 train dataset의 10%만 새로 랜덤 샘플링해 학습합니다. DDP에서는 모든 rank가 같은 전역 subset을 공유한 뒤 rank별로 나눠 받습니다. `1.0` 으로 override하면 기존처럼 전체 train dataset을 씁니다.
...
model:
  model_config:
    self_forced:
      ...
      ema_start_step: 50
data:
  train_epoch_sample_fraction: 0.1
```
