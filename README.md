...
### Map-to-Agent Radius Batch Sorting

`SMARTAgentEncoder.build_map2agent_edge`의 `torch_cluster.radius` 호출에도 같은 sorted-batch silent 가정이 적용됩니다.
caller(flow_agent_decoder)는 `batch_s = tokenized_agent["batch"].repeat(n_step)`으로 같은 scene 번호 묶음을 step마다 반복해서 넘기는데,
이 배열은 step 사이마다 큰 scene 번호에서 작은 값으로 떨어져 단조 비감소 조건을 만족하지 못합니다.
그대로 호출하면 같은 scene 안의 map-agent edge가 silent하게 절반 가까이 누락돼, 일부 agent/time-step이 자기 scene의 지도 정보를 받지 못한 채 학습됩니다.
WOSAC RMM의 map-based 항목(`offroad_indication_likelihood`, `distance_to_road_edge_likelihood`, `traffic_light_violation_likelihood`)에 직접 영향을 줄 수 있는 문제이므로,
`build_map2agent_edge`는 `radius` 호출 직전에 agent 쪽과 map 쪽 batch를 모두 `argsort`로 정렬하고, 받은 edge index를 원래 순서로 되돌려 downstream feature 계산이 그대로 작동하도록 합니다.
회귀 테스트 `test_map2agent_edge_no_silent_drop_cpu` / `_gpu`가 production 패킹·위치 분포에서 brute-force로 계산한 기대 edge 수와 실제 생성된 edge 수가 일치하는지 CPU/GPU 양쪽에서 확인합니다.
...
- 학습에서는 `control_alignment_filter.enabled=true`가 기본으로 켜져 있습니다. 이 필터는 vehicle/cyclist를 holonomic으로 바꾸지 않고 non-holonomic control target을 그대로 유지하되, raw GT와 transition-aligned trajectory의 anchor-window L2 오차가 type별 기준을 넘는 학습 anchor만 loss에서 제외합니다. 0.5초 endpoint+substep 분포(`outputs/transition_alignment_training.json`)의 vehicle `p99=0.473m`, `p99.9=1.575m`, cyclist `p99=0.751m`, `p99.9=9.999m (saturated)`을 기준으로 기본값을 vehicle `2.0m`, cyclist `2.2m`로 둡니다. vehicle `2.0m`는 정상 분포의 p99.9보다 약간 큰 값이라 대부분의 vehicle supervision을 유지하면서 수 m~수백 m급 GT jump만 제거하고, cyclist `2.2m`는 부족한 cyclist 표본을 과하게 버리지 않으면서 saturated tail을 잘라냅니다. pedestrian/holonomic target은 raw GT를 그대로 쓰므로 거리가 0이라 영향이 없습니다. 이 필터는 학습 anchor 선택에만 적용되며 validation, closed-loop rollout, fast RMM, WOSAC 제출물 생성의 평가 agent 선택은 바꾸지 않습니다. 기준을 ablation하려면 `model.model_config.token_processor.control_alignment_filter.cyclist_max_error_m=1.0`처럼 override합니다.
...
```
  ...
  주요 출력은 세 가지입니다. `step_error`는 current 이후 유효한 모든 10Hz future step의 raw-vs-aligned 위치 오차 분포이고, `agent_max_error`는 agent별 future 전체의 최대 오차이며, `anchor_window_max_error`는 실제 Flow 학습 anchor window 안에서 loss에 들어가는 step들의 최대 오차입니다. 분석 도구도 학습 코드와 같은 type-aware target을 따릅니다. vehicle/cyclist는 0.5초 endpoint + kinematic substep trajectory로 측정하고, pedestrian/holonomic target은 raw per-step trajectory로 측정합니다. 기본값으로 학습이 실제로 보는 분포를 측정하기 위해 invalid block endpoint로 인해 오염된 non-holonomic substep을 통계에서 제외합니다. rolling 자체의 raw distortion을 그대로 확인하려면 `--no-mask-by-aligned-substep-validity` 옵션을 추가하면 됩니다. 기본값은 현재 control-space preset과 맞춘 `vehicle_no_slip_point_ratio=0.2289518863`, `cyclist_no_slip_point_ratio=0.0495847873`, `anchor_valid_mode=prefix`, `commit_steps=5`입니다. `all / vehicle / pedestrian / cyclist`별 p50/p90/p99/p99.9, threshold 초과율, horizon별 mean/max를 JSON에 저장합니다. `heuristic_status=warn`은 보수적인 sanity flag일 뿐이고, 최종 판단은 type별 tail과 threshold 초과율을 보고 정합니다. 특히 vehicle/cyclist의 `anchor_window_max_error` p99나 `>1m` 비율이 크면 transition-aligned supervision이 raw 궤적을 과하게 왜곡한다는 신호이므로 no-slip ratio, holonomic ablation, 또는 이 방식 자체를 재검토해야 합니다.
...
#### testa/testaa A100x4x2 0.5s endpoint-balanced control-space pretrain

`semi_control_rolling_0.5` 브랜치의 주 실험은 아래 전용 launcher를 씁니다. 이미 `testa/testaa`에서 다른 multi-node 학습이 돌고 있다면 그 학습이 끝난 뒤에 실행하세요.

```bash
python scripts/launch_pre_bc_flow_control_a100x4x2_endpoint_balanced_static_pods.py \
  --replace
```

실제 pod에 명령을 보내기 전에 확인만 하려면:
...
이 launcher는 `configs/experiment/pre_bc_flow_control_a100x4x2_endpoint_balanced.yaml`을 사용합니다. 핵심 설정은 아래와 같습니다.
...
```bash
kubectl exec -it -n p-pnc testa -c main -- tmux attach -t catk-control-pretrain-a100x4x2-endpoint-balanced
kubectl exec -it -n p-pnc testaa -c main -- tmux attach -t catk-control-pretrain-a100x4x2-endpoint-balanced
```

실험 코드만 멈추고 pod는 그대로 두려면:

```bash
python scripts/launch_pre_bc_flow_control_a100x4x2_endpoint_balanced_static_pods.py --stop
```
