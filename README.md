...
`smart_ntp_pretrain_a100x4x2_bs15_freq102_main`을 사용한다. 다른 batch를 실험하려면
...
agent selection, `num_freq_bands: 102` 같은 모델/알고리즘 설정은 바꾸지 않는다. 대신
...
SMART 공정 비교 pretrain은 `num_freq_bands: 102`를 쓰므로, resume도 같은
...
`model.model_config.decoder.num_freq_bands: 102`도 명시한다. 최신 main 코드에서
`experiment=pre_bc`와 `num_freq_bands=102`로 SMART 모델을 실제 instantiate해 센
총 파라미터 수는 7,191,264개이며, 모두 trainable parameter이다. 직전
`num_freq_bands=66` 공정 비교 설정의 7,043,232개 대비 증가분은 148,032개이다.
...
읽어야 하므로 `model.model_config.decoder.num_freq_bands: 102`를 명시한다. 이 값을
