...
또한 A100x4x2 launcher는 `--ckpt-path`가 명시된 validate/test 실행에서도 master pod의
checkpoint를 다른 pod의 같은 경로로 복사하고 파일 크기를 확인한 뒤 tmux를 시작한다.
따라서 rank 0에서만 생성된 checkpoint로도 멀티 노드 검증을 바로 수행할 수 있다.
...
기존 task name 아래 checkpoint가 있어도 첫 attempt를 반드시 새 학습으로 시작하려면
`FRESH_START=1`을 지정한다. 이 경우 첫 attempt만 checkpoint 탐색을 건너뛰며, 이후 같은
실행 안에서 OOM이 발생하면 새로 생성된 checkpoint를 사용해 retry/resume한다.

현재 `origin/main` 최신 코드로 `testa + testaa`에서 resume 없이 처음부터 새 SMART NTP
pretrain을 시작하려면 아래 fresh wrapper를 사용한다.
...
`GIT_REF`를 고정하지 않으므로 pod 안의 clean checkout은 실행 시점의 `origin/main`을
따른다. `FRESH_START=1`이 기본이라 첫 attempt에서는 같은 task name 아래의 기존
checkpoint를 무시하고 새 학습을 시작한다. task name에는 기본적으로 실행 시각
`RUN_STAMP`가 붙으므로 과거 run의 checkpoint가 섞이지 않는다. 단, 새 학습이 진행되다가
OOM이 나면 그 새 run에서 저장된 `epoch_last.ckpt`를 찾아 batch size를 1 낮춰 resume한다.
...
submission과 같은 후보 폭을 쓰도록 `validation_rollout_sampling.num_k: 32`를 명시한다. 그 외에는
...
closed-loop validation의 rollout 후보 폭은 `validation_rollout_sampling.num_k=32`이다.
...
사용한다. a2a radius graph는 예전 SMART 기준과 같이 전체 agent/time-step 후보에서 먼저
만든 뒤, 유효하지 않은 agent state가 포함된 edge를 제거한다. 따라서 invalid agent를 후보
탐색 전에 미리 제외하지 않고, agent node에도 별도 motion-missingness feature를 추가하지
않는다.
...
- `road.road_data_use_ratio`가 1.0보다 작으면, 매 epoch 원본 training scenario 중 해당 비율만 무작위로 골라 RoaD cache를 생성한다.
- `road_topk_dist`처럼 deterministic한 RoaD 후보 선택에서는 scenario마다 rollout 1개만 만든다.
- Stochastic RoaD 후보 선택을 쓰는 경우에만 `road.num_rollouts_per_scenario`를 2 이상으로 올려 scenario마다 여러 독립 rollout을 만들 수 있다.
- Training dataset은 생성된 rollout 파일을 모두 학습 sample로 사용하므로, 기본 `road_topk_dist` 설정의 effective length는 이번 epoch에 선택된 scenario 수와 같다.
...
| 학습률 | `5e-5` |
| label smoothing | `0.0` |
| trainable 범위 | map encoder만 frozen, 나머지 trainable |
| train target selection | `data.train_use_eval_agent_selection=true` |
| train dataloader worker | `persistent_workers=false` |
| RoaD cache root | `/workspace/road_cache` |
| training scenario 사용 비율 | `road_data_use_ratio=1.0` |
| scenario당 rollout 수 | `1` |
| candidate 정책 | 확률 상위 Top-K |
| candidate 수 | `64` |
| sampling 온도 | 사용하지 않음 |
| candidate 선택 | 정답 다음 상태에 가장 가까운 top-k token |
| GT invalid 시 선택 | 확률 1위 token |
...
`${paths.output_dir}/road_cache/epoch_XXX` 아래 cache 디렉터리를 초기화한 뒤, 모든 rank가
같은 scenario subset을 rank별 shard로 나누어 epoch-local RoaD cache를 병렬 생성한다.
그 뒤 모든 rank가 synchronize한 다음 생성된 전체 cache를 읽는다. 생성된 데이터를 debug하려는
경우에만 `road.delete_after_use=false`를 설정한다.

Cache 생성은 같은 source batch를 scenario당 rollout 수만큼 다시 읽지 않고, 한 번 로드한
batch에서 rollout replica를 연속 생성한다. 같은 batch의 map encoding도 한 번만 계산해
재사용한다. `road_topk_dist`는 같은 source batch에서 항상 같은 rollout을 만들기 때문에,
요청된 rollout 수가 2 이상이어도 실제 cache와 train dataset은 scenario당 1개 rollout만
사용한다. `testas` A100 7장 launcher는 no-grad cache 생성의 낮은 메모리 사용량을 이용해
기본 `ROAD_GENERATION_BATCH_SIZE=160`으로 cache 생성 GPU 활용도를 높이며, 메모리가 부족한
환경에서는 환경변수로 직접 낮출 수 있다.

`testas` A100 7장 단일 노드에서 `1iapr5ed` run의 epoch 21 완료 checkpoint로 바로
RoaD fine-tuning을 시작하려면 아래 launcher를 사용한다.

```bash
python scripts/launch_road_clsft_testas_a100x7.py
```
