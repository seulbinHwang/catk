...
SMART NTP의 학습 전용 target selection을 명시적으로 쓰는 경로이다. batch 24를
...
- `data.train_batch_size: 24`, 즉 8개 rank 기준 effective global batch 192
...
학습 objective, 모델 구조, per-GPU `train_batch_size=24`, 전체 effective batch 192는
...
검증할 수 있는 범위인 `24` 이하의 양의 정수만 허용한다. 기본값은 `24`이다. batch 24에서
CUDA OOM이 나면 모델 구조나 loss를 바꾸지 않고 `--train-batch-size 22`, `20`, ...처럼
2씩 낮춰 OOM이 나지 않는 가장 큰 per-GPU batch를 찾는다. `--accumulate-grad-batches 2`
처럼 gradient accumulation을 켜는 override는 학습을 시작하기 전에 실패한다.

이 batch 24 전략은 `data.train_use_eval_agent_selection=false`인 학습 전용 target
...
실행했을 때 첫 train batch의 map-agent attention에서 CUDA OOM이 발생했다. 그래서 현재
A100x4x2 preset은 기본 batch를 24로 낮추고, 필요하면 같은 launcher option으로 2씩 낮춰
성공 가능한 최대 batch를 찾는 방식으로 운용한다.
...
`trainer.accumulate_grad_batches=1`을 유지한다. `data.train_batch_size`는 기본 24이고,
