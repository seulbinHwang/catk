...
`data.train_use_eval_agent_selection: false`를 켠다.
...
- `data.train_batch_size: 26`, 즉 8개 rank 기준 effective global batch 160
...
바꾸지 않으면서 A100x4x2 batch 20 학습의 map-agent attention 메모리 사용량을 줄인다.
...
학습 objective, 모델 구조, per-GPU `train_batch_size=20`, 전체 effective batch 160은
...
`data.train_batch_size=20`, `trainer.accumulate_grad_batches=1`을 강제한다. 따라서
...
주의할 점은 `train_batch_size=26`을 A100x4x2 SMART NTP에 그대로 쓰지 않는다는 것이다.
`testa/testaa` A100 80GB에서 `pre_bc_a100x4x2`, 8 ranks, batch 26 smoke를 직접 실행하면
첫 train batch에서 CUDA OOM이 발생했다. 그래서 A100 전용 SMART NTP preset은 실제 검증된
batch 20을 기본 공정 비교 실행 단위로 둔다. KFM 쪽 batch 26과 전역 batch가 달라지는 점은
실험표에 별도로 기록해야 한다.

현재 A100x4x2 preset은 `testa/testaa`의 `/tmp/catk_smart_ntp_a100_smoke_main` 별도 clone에서
검증했다. 검증 조건은 `pre_bc_a100x4x2`, 2 nodes, 8 ranks, `data.train_batch_size=20`,
train 1 batch, validation 1 batch, `trainer.check_val_every_n_epoch=1`이다. 이 smoke에서
학습 step, epoch-end closed-loop validation, WOSAC/Sim Agents 2025 metric logging이 끝까지
실행되었고 rank 0 로그에 `run.py DONE!!!`가 남았다.
...
`data.train_batch_size=20`, `trainer.accumulate_grad_batches=1`이 아닌 값이 들어오면
실행을 시작하기 전에 실패한다. 논문용 공정 비교가 아닌 별도 ablation에서 이 값을 바꾸려면
다른 experiment preset을 따로 만들고, 어떤 값을 바꿨는지 KFM 쪽 recipe와 함께 기록해야 한다.
