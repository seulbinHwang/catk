...
`main@2429ad1`, `testa/testaa`, A100 4장 x 2 pod, validation off,
...
|---:|---|---:|---:|---:|---:|
| 12 | 1 epoch 완주 | `5073/5073`, `1:12:38` | `1.16 it/s` | `79,123 MiB` | 약 `77.5시간` |
| 13 | CUDA OOM | step `2470/4683` 부근 실패 | - | `80,231 MiB` | 사용 안 함 |

`train_batch_size=13`은 epoch 중반 이후 heavy batch에서 OOM이 확인됐으므로, 기본값은
1 epoch 완주를 확인한 `train_batch_size=12`로 둔다. 장기 학습은
`scripts/start_smart_ntp_a100x4x2_testa_pretrain_with_oom_retry.sh`로 시작하는 것을
권장한다. 이 wrapper는 `INITIAL_BS=12`에서 시작하고, 이후 OOM marker가 보이면 최신
`epoch_last.ckpt`에서 batch를 1 낮춰 resume한다.
...
`data.train_batch_size=12`이다. 위 시간은 train-only 추정이며,
