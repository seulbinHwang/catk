...
- 그 다음 `agent_encoder.flow_decoder` 전체를 unfreeze합니다.
- 즉 fine-tuning에서는 `HierarchicalFlowDecoder` 내부 블록 전체를 학습하고, map encoder, agent embedding, attention layers는 그대로 frozen 상태를 유지합니다.
