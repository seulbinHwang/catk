...
SMART NTP의 학습 전용 target selection을 명시적으로 쓰는 경로이다. batch 26을
...
학습 objective, 모델 구조, per-GPU `train_batch_size=26`, 전체 effective batch 208은
...
이 batch 26 전략은 `data.train_use_eval_agent_selection=false`인 학습 전용 target
selection을 전제로 한다. 이전에 넓은 all-agent 학습 모드
(`data.train_use_eval_agent_selection=true`)에서 batch 26을 시도하면 첫 train batch의
...
`data.train_batch_size=26`, `trainer.accumulate_grad_batches=1`이 아닌 값이 들어오면
