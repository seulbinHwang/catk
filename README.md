...
SMART NTP의 학습 전용 target selection을 명시적으로 쓰는 경로이다. batch 18을
...
- `data.train_batch_size: 18`, 즉 8개 rank 기준 effective global batch 144
...
학습 objective, 모델 구조, per-GPU `train_batch_size=18`, 전체 effective batch 144는
...
smoke는 통과했지만 긴 train smoke에서 CUDA OOM이 났다. 현재 기본값은 100 train batch smoke를
통과한 `18`이다. `--accumulate-grad-batches 2`처럼 gradient accumulation을 켜는 override는
학습을 시작하기 전에 실패한다.

이 batch 18 전략은 `data.train_use_eval_agent_selection=false`인 학습 전용 target
...
100 train batch smoke를 통과했고, 관측 peak GPU memory는 약 76.3 GiB 수준이었다. 그래서
현재 A100x4x2 preset은 기본 batch를 18로 운용한다.
...
`trainer.accumulate_grad_batches=1`을 유지한다. `data.train_batch_size`는 기본 18이고,
