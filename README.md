...
`semi_control_decoder_last`에서 global 2초 step refiner 구조를 유지한 채 seven H100x7로 sidecar prebuild, DataLoader preload, train metric off, OOM fallback을 함께 쓰려면:

```bash
python scripts/launch_pre_bc_flow_h100x7_seven_global_2s_refiner_sidecar_static_pod.py --replace
```
...
| 항목 | 값 |
|---|---|
| branch | `semi_control_decoder_last` |
| pod / GPU | `seven`, H100 x 7 |
| experiment | `pre_bc_flow_2x4_h100` |
| task name | `flow_open_loop_pretrain_global2s_refiner_sidecar_metricoff_h100x7_seven_bs18_lr6p5e-4_warm5_val8_membal` |
| per-rank train batch | `18` |
| learning rate | `6.5e-4` |
| train metric | `train_open_loop_metrics=false` |
| sidecar prebuild | `true` |
| sidecar prebuild parallelism | `torchrun --nproc_per_node=7`, rank별 shard 처리 |
...
동일 기능을 도입한 `semi_control_stable`의 `439f10a8` 기준 참조 검증 결과는 다음과 같습니다. 이 측정은 2026-06-13 fm-sf-5 H100x8, `bs=18`, 80 train batches, validation/checkpoint ranking off 조건에서 수행했습니다.
...
2026-06-13 `semi_control_stable`의 `439f10a8` 검증에서 `scripts/verify_flow_target_coverage.py`로 target coverage를 전체 스캔했습니다.
...
seven sidecar pretrain launcher는 train split 전체에 targetless sample이 없다는 위 검증을 근거로
