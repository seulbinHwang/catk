...
agent 집합도 명시해야 한다. 현재 A100x4x2 SMART NTP pretrain 전략은
`configs/experiment/pre_bc.yaml`과 `configs/experiment/pre_bc_a100x4x2.yaml`에서
`data.train_use_eval_agent_selection: false`를 사용한다.

이 설정에서는 validation과 같은 no-op agent transform을 쓰지 않고,
학습 전용 target builder를 사용한다. 따라서 학습 전에 ego 기준 150m 밖 agent의
valid mask를 자르고, role agent 또는 현재 100m 이내이면서 미래 valid가 충분한
agent를 train target으로 고른다. target 수가 많으면 scenario당 최대
`data.train_max_num`개까지 선택한 `train_mask`를 만들고, SMART NTP loss는 이
`train_mask`가 true인 agent에 대해서만 계산된다.

즉 이 경로는 "validation/evaluation agent set과 같은 넓은 학습 대상"이 아니라,
SMART NTP의 학습 전용 target selection을 명시적으로 쓰는 경로이다. batch 16을
유지하기 위해 넓은 all-agent 학습 대신 이 legacy train target selection을 쓰는
실험으로 기록한다.
...
SMART NTP pretrain을 바로 시작하려면 아래 launcher를 사용한다.
...
검증할 수 있는 범위인 `24` 이하의 양의 정수만 허용한다. `testa/testaa` 실제 smoke 기준으로
full bf16에서는 batch 24와 22가 train forward에서 CUDA OOM이 났고, batch 20은 짧은
train/validation smoke는 통과했지만 긴 train smoke에서 CUDA OOM이 났다. batch 18도
epoch 후반의 map-agent attention에서 CUDA OOM이 났다. full bf16 batch 16은 가능했지만
100-step train-only 기준 `0.37~0.38 it/s`로, 64 epoch train-only 예상 시간이 약
`7.4~7.5일`이었다.

graph attention fp32 경로는 같은 `testa/testaa` A100x4x2, validation off, 100-step
train-only 기준으로 아래처럼 측정했다.

| 설정 | per-GPU batch | 100-step 시간 | 속도 | 관측 peak memory | 64 epoch train-only 예상 |
|---|---:|---:|---:|---:|---:|
| full bf16 | 16 | `4:26` | `0.37~0.38 it/s` | 약 `74.2 GiB` | 약 `7.4~7.5일` |
| graph attention fp32 | 14 | `2:19` | `0.72 it/s` | 약 `81.1 GiB` | 약 `4.5일`, OOM 위험 큼 |
| graph attention fp32 | 13 | `1:59` | `0.84 it/s` | 약 `69.8 GiB` | 약 `4.1일` |

H100x4x2에서는 `main@d873f2be`, `hsb-npc-training/hsb-npc-training-2`,
validation off 조건으로 batch 13~17을 직접 확인했다. batch 17은 40-step 중 OOM,
batch 16은 120-step smoke는 통과했지만 300-step 확인에서 OOM이 났다. batch 15는
300-step 확인을 통과했고, 관측 peak memory는 `78,145 MiB / 81,559 MiB`,
속도는 약 `0.92 it/s`였다. training cache `486,996`개 기준으로 64 epoch train-only
예상 시간은 약 `78.4시간`, 즉 약 `3.27일`이다.

따라서 현재 H100/A100 공통 pretrain 기본값은 `CATK_ATTENTION_GRAPH_FP32=1`과
`data.train_batch_size=15`이다. `--accumulate-grad-batches 2`처럼 gradient accumulation을 켜는 override는
학습을 시작하기 전에 실패한다.

이 batch 15 전략은 `data.train_use_eval_agent_selection=false`인 학습 전용 target
selection을 전제로 한다. 이전에 넓은 all-agent 학습 모드
(`data.train_use_eval_agent_selection=true`)에서 큰 batch를 시도하면 첫 train batch의
...
2026-05-20 기준으로 `testa + testaa` A100 8장 및
`hsb-npc-training + hsb-npc-training-2` H100 8장에서 아래 검증을 수행했다.

| 검증 | 설정 | 결과 |
|---|---|---|
| CUDA extension 단위 검증 | `CUDA_VISIBLE_DEVICES=0`, forward/backward smoke | 통과 |
| 수식 통합 검증 | `AttentionLayer` PyG backend와 segmented backend 출력 비교 | 통과 |
| forced segmented DDP smoke | `train_batch_size=12`, `CATK_ATTENTION_LAYER_BACKEND=segmented`, 3 train batch | 통과 |
| 기본 hybrid DDP smoke | `train_batch_size=12`, `CATK_ATTENTION_LAYER_BACKEND=hybrid`, 1 train batch | 통과 |
| A100 운영값 근접 DDP pretrain 검증 | `train_batch_size=13`, `hybrid`, `CATK_ATTENTION_GRAPH_FP32=1`, 50 train batch | 통과 |
| H100 기본값 DDP pretrain 검증 | `train_batch_size=15`, `hybrid`, `CATK_ATTENTION_GRAPH_FP32=1`, 300 train batch | 통과 |

A100 50 train batch 검증은 `trainer/global_step=49`까지 정상 증가했고,
`train/loss=6.38681`로 finite 값을 기록했다. H100 300 train batch 검증은
`trainer/global_step=299`까지 정상 증가했고, `train/loss=3.99208`로 finite 값을
기록했다. CUDA OOM, Triton/CUDA extension build error, DDP hang,
`AttentionLayer` dtype mismatch는 발생하지 않았다. 다만 이 값은 validation을 끈 짧은
runtime integration 검증 결과이며, 최종 수렴 성능으로 해석하지 않는다.
...
실측에서도 batch 24와 22는 OOM, batch 20은 장기 train smoke 중 OOM이 났다. batch 18은
100 train batch smoke를 통과했지만 epoch 0의 약 97% 지점에서 map-agent attention OOM이 났다.
batch 16은 H100x4x2 300-step 확인에서 CUDA OOM이 났고, batch 15는 H100x4x2
300-step 확인을 통과했다. 그래서 현재 x4x2 preset은 기본 batch를 15로 운용한다.
