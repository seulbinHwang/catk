#!/usr/bin/env bash
# Run UniMM Anchor-Based-4s across an equal-size 4 node x 2 GPU V100 pod fleet.
set -Eeuo pipefail
...
main() {
  ...
  local trainer_precision="${TRAINER_PRECISION:-16-mixed}"
  ...
  local task_name="${TASK_NAME:-unimm_anchor_based_4s_v100x4x2}"
  ...
  if [[ ! -f "$anchor_file" ]]; then
    ...
    log "Build it first with scripts/build_unimm_anchors.sh or scripts/launch_unimm_v100x4x2.py --build-anchors."
    exit 2
  fi
