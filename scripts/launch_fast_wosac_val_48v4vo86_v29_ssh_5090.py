#!/usr/bin/env python3
"""Launch 48v4vo86:v29 Fast WOSAC validation on the RTX 5090 host.

The checkpoint is downloaded from the W&B artifact
``jksg01019-naver-labs/SMART-FLOW/epoch-last-48v4vo86:v29`` after verifying
...
DEFAULT_REMOTE_CKPT_PATH = (
    ...
    "flow_control_space_pretrain_v100x47_prefix_roundtrip2_bs8/"
    "epoch-last-48v4vo86_v29/epoch_last.ckpt"
)
DEFAULT_REMOTE_DOWNLOAD_DIR = (
    "/media/user/E/projects/catk/checkpoints/from_wandb/"
    "flow_control_space_pretrain_v100x47_prefix_roundtrip2_bs8/"
    "epoch-last-48v4vo86_v29/artifact"
)
DEFAULT_WANDB_ARTIFACT = "jksg01019-naver-labs/SMART-FLOW/epoch-last-48v4vo86:v29"
DEFAULT_EXPECTED_ARTIFACT_EPOCH = 33
DEFAULT_EXPECTED_GLOBAL_STEP = 82911
DEFAULT_EXPECTED_COMPLETED_EPOCHS = 32
DEFAULT_STEPS_PER_EPOCH = 2591
DEFAULT_BRANCH = "semi_control"
DEFAULT_TMUX_SESSION = "hsb-rl-train"
DEFAULT_WINDOW_NAME = "fast-wosac-48v4vo86-v29"
DEFAULT_TASK_NAME = (
    "flow_control_space_pretrain_v100x47_prefix_roundtrip2_bs8_"
    "epoch031_v29_fast_wosac_val1680"
)
...
def prepare_checkpoint_on_remote(args: argparse.Namespace) -> None:
    if args.dry_run:
        ...
        print(
            ...
            f"(global_step + 1) == {args.expected_completed_epochs} * {args.steps_per_epoch}"
        )
...
if global_step + 1 != expected_completed_epochs * steps_per_epoch:
    raise SystemExit(
        "artifact does not match expected completed epoch count: "
        f"global_step+1={global_step + 1}, "
...
echo "[fast-wosac-48v4vo86-v29] host=$(hostname) task=${{TASK_NAME}}"
echo "[fast-wosac-48v4vo86-v29] started at $(date '+%F %T')"
echo "[fast-wosac-48v4vo86-v29] repo=$(pwd)"
echo "[fast-wosac-48v4vo86-v29] commit=$(git rev-parse --short HEAD 2>/dev/null) $(git log -1 --pretty=%s 2>/dev/null)"
echo "[fast-wosac-48v4vo86-v29] python=$(command -v python 2>/dev/null)"
echo "[fast-wosac-48v4vo86-v29] cache=${{CACHE_ROOT}}"
echo "[fast-wosac-48v4vo86-v29] ckpt=${{CKPT_PATH}}"
echo "[fast-wosac-48v4vo86-v29] scorer_scene_num=${{SCORER_SCENE_NUM}} val_batch_size=${{VAL_BATCH_SIZE}} limit_val_batches=${{LIMIT_VAL_BATCHES}}"
echo "[fast-wosac-48v4vo86-v29] n_rollout_closed_val=${{N_ROLLOUT_CLOSED_VAL}} nproc=${{NPROC_PER_NODE}} precision=${{PRECISION}}"
echo

if [[ ! -f "$CKPT_PATH" ]]; then
  echo "[fast-wosac-48v4vo86-v29] checkpoint does not exist: $CKPT_PATH" >&2
  exec bash
fi
if [[ ! -d "$CACHE_ROOT" ]]; then
  echo "[fast-wosac-48v4vo86-v29] CACHE_ROOT does not exist: $CACHE_ROOT" >&2
  exec bash
fi
if ! command -v python >/dev/null 2>&1; then
  echo "[fast-wosac-48v4vo86-v29] python is not available after conda activation" >&2
  exec bash
fi
...
ARGS=(
  ...
  "logger.wandb.tags=[fast_wosac,48v4vo86,v29,epoch031,rtx5090,control_space,prefix_valid]"
)

echo "[fast-wosac-48v4vo86-v29] output_dir=$OUTPUT_DIR"
echo "[fast-wosac-48v4vo86-v29] command: python -m torch.distributed.run --standalone --nproc_per_node=$NPROC_PER_NODE ${{ARGS[*]}}"
python -m torch.distributed.run --standalone --nproc_per_node="$NPROC_PER_NODE" "${{ARGS[@]}}"
status=$?

echo
echo "[fast-wosac-48v4vo86-v29] exited with status $status at $(date '+%F %T')"
echo "[fast-wosac-48v4vo86-v29] output_dir=$OUTPUT_DIR"
echo "[fast-wosac-48v4vo86-v29] leaving shell open for inspection"
exec bash
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Verify and download W&B artifact epoch-last-48v4vo86:v29, then "
