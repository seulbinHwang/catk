#!/bin/sh
# Fine-tuning: ODE full-grad → KinematicProjection as reward → reward gradient back through ODE.
# GPU 2, 3 전용.
...
MY_EXPERIMENT="${MY_EXPERIMENT:-kinematic_reward_ft}"
MY_TASK_NAME="${MY_TASK_NAME:-${MY_EXPERIMENT}-a100-kinrewardft}"
...
FLOW_REG_LAMBDA="${FLOW_REG_LAMBDA:-0.5}"
...
PORT="$(get_free_port)"
echo "==== Start training (kinematic_reward_ft): train_batch_size=${TRAIN_B}, val_batch_size=${VAL_B} ===="
