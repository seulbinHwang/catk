#!/usr/bin/env python3
"""Launch Fast WOSAC validation for the 48v4vo86 control-space checkpoint on wo-pvc-800.
...
DEFAULT_TASK_NAME = "flow_control_space_pretrain_v100x47_prefix_roundtrip2_bs8_48v4vo86_fast_wosac_val1680"
DEFAULT_SESSION = "catk-fast-wosac-48v4vo86-wo-pvc-800"
DEFAULT_WANDB_ARTIFACT = "jksg01019-naver-labs/SMART-FLOW/epoch-last-48v4vo86:v61"
DEFAULT_CKPT_PATH = "/workspace/flow_control_space_pretrain_v100x47_prefix_roundtrip2_bs8/v61/epoch_last.ckpt"
DEFAULT_DOWNLOAD_DIR = "/workspace/flow_control_space_pretrain_v100x47_prefix_roundtrip2_bs8/v61/artifact"
...
echo "[fast-wosac-48v4vo86] pod=$(hostname) task=${{TASK_NAME}}"
echo "[fast-wosac-48v4vo86] started at $(date '+%F %T')"
echo "[fast-wosac-48v4vo86] repo=$(pwd)"
echo "[fast-wosac-48v4vo86] commit=$(git rev-parse --short HEAD 2>/dev/null) $(git log -1 --pretty=%s 2>/dev/null)"
echo "[fast-wosac-48v4vo86] cache=${{CACHE_ROOT}}"
echo "[fast-wosac-48v4vo86] ckpt=${{CKPT_PATH}}"
echo "[fast-wosac-48v4vo86] scorer_scene_num=${{SCORER_SCENE_NUM}} val_batch_size=${{VAL_BATCH_SIZE}} limit_val_batches=${{LIMIT_VAL_BATCHES}}"
echo "[fast-wosac-48v4vo86] n_rollout_closed_val=${{N_ROLLOUT_CLOSED_VAL}} nproc=${{NPROC_PER_NODE}} precision=${{PRECISION}}"
echo

prepare_checkpoint() {{
  if [[ -f "$CKPT_PATH" ]]; then
    echo "[fast-wosac-48v4vo86] using checkpoint: $CKPT_PATH"
    return 0
  fi
...
for candidate in candidates:
    if candidate.is_file():
        ...
        print(f"[fast-wosac-48v4vo86] checkpoint ready from local path: {{target}}")
        raise SystemExit(0)
raise SystemExit(1)
PY
  local status=$?
  ...
  echo "[fast-wosac-48v4vo86] local checkpoint not found; downloading W&B artifact: $WANDB_ARTIFACT"
  python - <<'PY'
import glob
import os
import shutil
...
print(f"[fast-wosac-48v4vo86] checkpoint ready from W&B artifact: {{target_ckpt}}")
PY
}}
...
status=$?
if (( status != 0 )); then
  echo "[fast-wosac-48v4vo86] checkpoint preparation failed with status $status" >&2
  echo "[fast-wosac-48v4vo86] leaving shell open for inspection"
  exec bash
fi

if [[ ! -d "$CACHE_ROOT" ]]; then
  echo "[fast-wosac-48v4vo86] CACHE_ROOT does not exist: $CACHE_ROOT" >&2
  echo "[fast-wosac-48v4vo86] leaving shell open for inspection"
  exec bash
fi
...
ARGS=(
  ...
  "logger.wandb.tags=[fast_wosac,48v4vo86,wo-pvc-800,control_space]"
)
...
echo "[fast-wosac-48v4vo86] output_dir=$OUTPUT_DIR"
echo "[fast-wosac-48v4vo86] command: python -m torch.distributed.run --standalone --nproc_per_node=$NPROC_PER_NODE ${{ARGS[*]}}"
python -m torch.distributed.run --standalone --nproc_per_node="$NPROC_PER_NODE" "${{ARGS[@]}}"
status=$?

echo
echo "[fast-wosac-48v4vo86] exited with status $status at $(date '+%F %T')"
echo "[fast-wosac-48v4vo86] output_dir=$OUTPUT_DIR"
echo "[fast-wosac-48v4vo86] leaving shell open for inspection"
exec bash
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Start Fast WOSAC validation for the 48v4vo86 control-space checkpoint "
