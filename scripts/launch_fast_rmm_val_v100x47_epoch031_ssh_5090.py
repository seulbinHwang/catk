#!/usr/bin/env python3
"""Launch a quick Fast WOSAC/RMM validation for the V100x47 epoch-31 run.

The launcher is meant to be run from this repo on a workstation. It connects to
the RTX 5090 host, copies the selected checkpoint from the running V100 pod
fleet via that host's kubectl context, and starts validation in an ``hsb-rl-train``
tmux window on the 5090 box.

The requested epoch-31 training result is stored in the pod run as
``epoch_031.ckpt``. The moving ``epoch_last.ckpt`` has already advanced in the
ongoing training run, so the default source path is pinned to the epoch-31
checkpoint while still verifying the Lightning checkpoint epoch before launch.
"""
...
DEFAULT_SOURCE_TASK = "flow_control_space_pretrain_v100x47_prefix_roundtrip2_stable_lr6e-4_bs4"
DEFAULT_SOURCE_CKPT = (
    "/mnt/nuplan/projects/catk/logs/"
    "flow_control_space_pretrain_v100x47_prefix_roundtrip2_stable_lr6e-4_bs4/"
    "runs/2026-05-13_20-34-15/checkpoints/epoch_031.ckpt"
)
...
DEFAULT_WINDOW_NAME = "fast-rmm-v100x47-e31"
DEFAULT_TASK_NAME = (
    "flow_control_space_pretrain_v100x47_prefix_roundtrip2_stable_lr6e-4_bs4_"
    "epoch031_fast_rmm_val1680"
)
...
def remote_kubectl_script(args: argparse.Namespace, pod_script: str) -> str:
    return (
        "kubectl exec "
        f"-n {shq(args.source_namespace)} "
        f"{shq(args.source_pod)} "
        f"-c {shq(args.source_container)} "
        "-- bash -lc "
        f"{shq(pod_script)}"
    )
...
def resolve_source_ckpt(args: argparse.Namespace) -> str:
    ...
    if args.dry_run:
        return (
            "/mnt/nuplan/projects/catk/logs/"
            f"{args.source_task}/runs/<latest>/checkpoints/epoch_031.ckpt"
        )
...
find /mnt/nuplan/projects/catk/logs/"$task"/runs \\
  -path '*/checkpoints/epoch_031.ckpt' -type f \\
  -printf '%T@ %p\\n' 2>/dev/null | sort -nr | head -1 | cut -d' ' -f2-
"""
    source = run_ssh(args, remote_kubectl_script(args, find_script), capture=True)
    if not source:
        raise RuntimeError(f"failed to find epoch_031.ckpt for task {args.source_task}")
    return source
...
def source_file_size(args: argparse.Namespace, source_ckpt: str) -> int:
    output = run_ssh(
        args,
        remote_kubectl_script(args, f"stat -c %s {shq(source_ckpt)}"),
        capture=True,
    )
...
def copy_checkpoint_to_remote(args: argparse.Namespace, source_ckpt: str) -> None:
    ...
    if args.dry_run:
        print(f"[launcher] would copy checkpoint from {args.source_pod}:{source_ckpt}")
...
set -Eeuo pipefail
tmp={shq(tmp_path)}
dst={shq(args.remote_ckpt_path)}
{remote_kubectl_script(args, f"cat {shq(source_ckpt)}")} > "$tmp"
mv "$tmp" "$dst"
stat -c %s "$dst"
"""
    remote_size = int(run_ssh(args, copy_script, capture=True).splitlines()[-1])
    if remote_size != source_size:
        raise RuntimeError(
            f"checkpoint copy size mismatch: source={source_size}, remote={remote_size}"
        )


def verify_remote_checkpoint_epoch(args: argparse.Namespace) -> None:
    if args.skip_epoch_check:
        return
    if args.dry_run:
        print(f"[launcher] would verify checkpoint epoch == {args.expected_epoch}")
        return

    verify_script = f"""
set -Eeuo pipefail
cd {shq(args.remote_project_root)}
if [[ -x /media/user/E/miniforge/envs/catk/bin/python ]]; then
  PYTHON=/media/user/E/miniforge/envs/catk/bin/python
else
  PYTHON=python
fi
"$PYTHON" - <<'PY'
...
path = {args.remote_ckpt_path!r}
expected = {int(args.expected_epoch)}
ckpt = torch.load(path, map_location="cpu", weights_only=False)
actual = int(ckpt.get("epoch", -1))
global_step = ckpt.get("global_step")
print(f"[launcher] checkpoint metadata: epoch={{actual}} global_step={{global_step}}")
if actual != expected:
    raise SystemExit(f"expected checkpoint epoch {{expected}}, got {{actual}}")
PY
"""
    run_ssh(args, verify_script)
...
echo "[fast-rmm-v100x47] host=$(hostname) task=${{TASK_NAME}}"
echo "[fast-rmm-v100x47] started at $(date '+%F %T')"
echo "[fast-rmm-v100x47] repo=$(pwd)"
echo "[fast-rmm-v100x47] commit=$(git rev-parse --short HEAD 2>/dev/null) $(git log -1 --pretty=%s 2>/dev/null)"
echo "[fast-rmm-v100x47] python=$(command -v python 2>/dev/null)"
echo "[fast-rmm-v100x47] cache=${{CACHE_ROOT}}"
echo "[fast-rmm-v100x47] ckpt=${{CKPT_PATH}}"
echo "[fast-rmm-v100x47] scorer_scene_num=${{SCORER_SCENE_NUM}} val_batch_size=${{VAL_BATCH_SIZE}} limit_val_batches=${{LIMIT_VAL_BATCHES}}"
echo "[fast-rmm-v100x47] n_rollout_closed_val=${{N_ROLLOUT_CLOSED_VAL}} nproc=${{NPROC_PER_NODE}} precision=${{PRECISION}}"
echo

if [[ ! -f "$CKPT_PATH" ]]; then
  echo "[fast-rmm-v100x47] checkpoint does not exist: $CKPT_PATH" >&2
  exec bash
fi
if [[ ! -d "$CACHE_ROOT" ]]; then
  echo "[fast-rmm-v100x47] CACHE_ROOT does not exist: $CACHE_ROOT" >&2
  exec bash
fi
if ! command -v python >/dev/null 2>&1; then
  echo "[fast-rmm-v100x47] python is not available after conda activation" >&2
  exec bash
fi
...
ARGS=(
  ...
  model.model_config.decoder.use_holonomic_model_only=false
  model.model_config.token_processor.use_rolling_supervision=true
  model.model_config.decoder.use_rolling_supervision=true
  model.model_config.token_processor.control_pos_scale_m=1.0
  model.model_config.token_processor.control_vehicle_no_slip_point_ratio=0.0
  model.model_config.decoder.control_vehicle_no_slip_point_ratio=0.0
  model.model_config.token_processor.control_cyclist_no_slip_point_ratio=0.0
  model.model_config.decoder.control_cyclist_no_slip_point_ratio=0.0
  ...
  "logger.wandb.tags=[fast_rmm,fast_wosac,v100x47,epoch031,rtx5090,control_space,semi_control_stable]"
)

echo "[fast-rmm-v100x47] output_dir=$OUTPUT_DIR"
echo "[fast-rmm-v100x47] command: python -m torch.distributed.run --standalone --nproc_per_node=$NPROC_PER_NODE ${{ARGS[*]}}"
python -m torch.distributed.run --standalone --nproc_per_node="$NPROC_PER_NODE" "${{ARGS[@]}}"
status=$?

echo
echo "[fast-rmm-v100x47] exited with status $status at $(date '+%F %T')"
echo "[fast-rmm-v100x47] output_dir=$OUTPUT_DIR"
echo "[fast-rmm-v100x47] leaving shell open for inspection"
exec bash
"""
...
def render_remote_start(args: argparse.Namespace) -> str:
    ...
    run_root = f"{args.remote_log_dir.rstrip('/')}/tmux_fast_rmm_val/{safe_task}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Copy the V100x47 epoch-31 checkpoint from testsv and launch quick "
            "Fast WOSAC/RMM validation on the RTX 5090 host."
        )
    )
    ...
    parser.add_argument("--expected-epoch", type=int, default=31)
    parser.add_argument("--skip-epoch-check", action="store_true")
...
def main() -> None:
    ...
    print(f"[launcher] expected epoch:    {args.expected_epoch}")
    ...
    verify_remote_checkpoint_epoch(args)
