#!/usr/bin/env python3
"""Launch SMART CAT-K/CLSFT fine-tuning on the existing testas A100x7 pod."""
...
def render_env_file(*, args: argparse.Namespace, task_name: str, run_id: str) -> str:
    lines = [
        ...
        export_line("NUM_FREQ_BANDS", args.num_freq_bands),
    ]
    optional_env = {
...
bash scripts/smart_clsft_testas_a100x7_finetune.sh
status=$?
echo
...
def render_start_command(*, args: argparse.Namespace, task_name: str, run_id: str) -> str:
    ...
    run_root = f"{args.log_dir.rstrip('/')}/tmux_smart_clsft_testas_a100x7/{safe_task}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Start SMART CAT-K/CLSFT fine-tuning in tmux on the testas A100x7 pod.",
    )
    ...
    parser.add_argument(
        "--action",
        choices=["finetune", "validate", "test"],
    ...
    parser.add_argument("--session", default=os.environ.get("SESSION", "catk-smart-clsft-testas-a100x7"))
    parser.add_argument("--master-port", default=os.environ.get("MASTER_PORT", "29573"))
    ...
    parser.add_argument("--train-batch-size", default=os.environ.get("TRAIN_BATCH_SIZE", "10"))
    parser.add_argument("--val-batch-size", default=os.environ.get("VAL_BATCH_SIZE", "10"))
    parser.add_argument("--test-batch-size", default=os.environ.get("TEST_BATCH_SIZE", "10"))
    ...
    if not args.ckpt_path and not args.ckpt_artifact:
    ...
    if not args.task_name:
        ...
        args.task_name = f"smart_clsft_testas_a100x7_{stamp}"
    if not args.run_id:
