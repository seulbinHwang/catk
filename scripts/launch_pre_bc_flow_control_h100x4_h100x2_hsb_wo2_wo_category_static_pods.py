#!/usr/bin/env python3
"""Launch the wo-category ablation pretrain on hsb-npc-training + wo-pvc-2.
...
``semi_control_stable`` and the W&B
``flow_control_space_pretrain_h100x4_h100x2_prefix_default_noslip_tailprefix_roundtrip05_lr6e-4_bs20``
run, but checks out ``semi_control_stable_wo_category`` so map surface category
ablation is the intended model/data-path difference. This ablation starts from
per-rank train batch size 18 for additional memory margin.
"""
...
DEFAULT_ARGS = [
    ...
    "/mnt/nuplan/projects/catk/logs/dataset_metadata/womd_training_memory_balance_h100x6_hsb_wo_pvc2_wo_category.pt",
]
