#!/usr/bin/env python3
"""Launch MDG A100/H100 x4x2 pretrain on existing pods.
...
DEFAULT_CACHE_ROOT_BY_POD = {
    "testa": "/workspace/womd_v1_3/MDG_cache",
    "testaa": "/workspace/womd_v1_3/MDG_cache",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Start MDG A100/H100 x4x2 pretrain in tmux on existing pods.",
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Shared Hydra run id used by all distributed ranks. Defaults to one "
            "timestamp generated by the launcher."
        ),
    )
    parser.add_argument("--session", default="catk-mdg-a100x4x2")
    ...
    if not args.task_name:
        ...
        args.task_name = f"mdg_wosac_pretrain_a100x4x2_{stamp}"
    if not args.run_id:
