#!/usr/bin/env python3
"""Launch commit-aligned H100x8 open-loop Flow pretrain on fm-sf-5.
...
DEFAULT_PROJECT_ROOT = "/mnt/nuplan/projects/catk_worktrees/semi_control_stable_2_weight"
DEFAULT_SOURCE_ROOT = "/mnt/nuplan/projects/catk"
DEFAULT_BRANCH = "semi_control_stable_2_weight"
...
DEFAULT_TASK_NAME = (
    "flow_open_loop_pretrain_commit_aligned_sidecar_metricoff_"
    "h100x8_fmsf5_bs18_lr6p5e-4_warm5_val8_membal"
)
DEFAULT_SESSION = "catk-pretrain-commit-aligned-h100x8-fmsf5"
...
def run_root(args: argparse.Namespace) -> str:
    ...
    return f"{args.log_dir.rstrip('/')}/tmux_commit_aligned_flow_h100x8_fmsf5/{safe_task}"
...
git checkout --detach refs/remotes/origin/{shq(branch)}
"""
...
RUN_ROOT="${{CATK_LOG_DIR%/}}/tmux_commit_aligned_flow_h100x8_fmsf5/${{TASK_NAME//\\//_}}"
mkdir -p "$RUN_ROOT"

echo "[commit-aligned-h100x8-fmsf5] pod=$(hostname) task=${{TASK_NAME}}"
echo "[commit-aligned-h100x8-fmsf5] started at $(date '+%F %T')"
echo "[commit-aligned-h100x8-fmsf5] git=$(git rev-parse --short HEAD 2>/dev/null || true)"
echo "[commit-aligned-h100x8-fmsf5] experiment=${{EXPERIMENT}} initial_bs=${{INITIAL_BS}} oom_step=${{OOM_STEP}} min_bs=${{MIN_BS}}"
echo "[commit-aligned-h100x8-fmsf5] nproc=${{NPROC_PER_NODE}} cache=${{CACHE_ROOT}}"
echo "[commit-aligned-h100x8-fmsf5] commit_aligned_flow_loss=enabled weights=[1.25,1.01,0.87] boundary_steps=[5,10]"
echo "[commit-aligned-h100x8-fmsf5] run_root=${{RUN_ROOT}}"
echo "[commit-aligned-h100x8-fmsf5] attach survives after exit; press Ctrl-b d to detach"
echo

if [[ "${{SIDECAR_PREBUILD}}" == "true" ]]; then
  echo "[commit-aligned-h100x8-fmsf5] prebuilding flow target sidecars dir=${{FLOW_TARGET_SIDECAR_DIR}} max_samples=${{SIDECAR_PREBUILD_MAX_SAMPLES}} nproc=${{NPROC_PER_NODE}}"
  ...
  echo "[commit-aligned-h100x8-fmsf5] sidecar prebuild exited with status $sidecar_status log=$sidecar_log"
  if (( sidecar_status != 0 )); then
    echo "[commit-aligned-h100x8-fmsf5] sidecar prebuild failed; aborting before training"
    exec bash
  fi
fi
...
PY
  ...
  echo "[commit-aligned-h100x8-fmsf5] dataloader sidecar root=${{FLOW_TARGET_SIDECAR_ROOT}}"
fi
...
terminate_task_processes() {{
  ...
  echo "[commit-aligned-h100x8-fmsf5] terminating task processes for $reason: ${{pids[*]}}"
  kill -TERM "${{pids[@]}}" 2>/dev/null || true
  sleep 15
  mapfile -t pids < <(task_process_pids || true)
  if (( ${{#pids[@]}} > 0 )); then
    echo "[commit-aligned-h100x8-fmsf5] force killing task processes for $reason: ${{pids[*]}}"
...
while (( bs >= MIN_BS )); do
  ...
  if [[ -n "$latest_ckpt" ]]; then
    echo "[commit-aligned-h100x8-fmsf5] attempt #${{attempt}} bs=${{bs}} resume=${{latest_ckpt}}"
    ckpt_override="ckpt_path=${{latest_ckpt}}"
  else
    echo "[commit-aligned-h100x8-fmsf5] attempt #${{attempt}} bs=${{bs}} fresh fit"
    ckpt_override="ckpt_path=null"
  fi
  echo "[commit-aligned-h100x8-fmsf5] attempt log: $attempt_log"
  ...
  HYDRA_OVERRIDES=(
    ...
    "model.model_config.commit_aligned_flow_loss.enabled=true"
    "model.model_config.commit_aligned_flow_loss.weights=[1.25,1.01,0.87]"
    "model.model_config.commit_aligned_flow_loss.boundary_steps=[5,10]"
  )
  ...
  printf '[commit-aligned-h100x8-fmsf5] torchrun command:'
  ...
  echo "[commit-aligned-h100x8-fmsf5] attempt #${{attempt}} exited with status $exit_code"
  ...
  if grep -Eq "$OOM_REGEX" "$attempt_log"; then
    non_oom_retry_count=0
    new_bs=$(( bs - OOM_STEP ))
    echo "[commit-aligned-h100x8-fmsf5] OOM detected at bs=${{bs}}; lowering to bs=${{new_bs}}"
    if (( new_bs < MIN_BS )); then
      echo "[commit-aligned-h100x8-fmsf5] next bs=${{new_bs}} is below MIN_BS=${{MIN_BS}}; aborting"
      final_status=1
      break
    fi
    bs="$new_bs"
  ...
  if is_retryable_non_oom_exit "$exit_code" && (( non_oom_retry_count < MAX_NON_OOM_RETRIES )); then
    ...
    echo "[commit-aligned-h100x8-fmsf5] retryable non-OOM exit=${{exit_code}}; retrying bs=${{bs}} (${{non_oom_retry_count}}/${{MAX_NON_OOM_RETRIES}})"
    continue
  fi

  echo "[commit-aligned-h100x8-fmsf5] non-OOM failure; see $attempt_log"
  final_status="$exit_code"
  break
done

echo
echo "[commit-aligned-h100x8-fmsf5] finished with status $final_status at $(date '+%F %T')"
echo "[commit-aligned-h100x8-fmsf5] leaving shell open for inspection"
exec bash
"""
...
if [ ! -e {shq(args.project_root)}/.git ] && [ -e {shq(args.source_root)}/.git ]; then
  echo "[launcher] preparing clean worktree: {args.project_root}"
  git -C {shq(args.source_root)} config --global --add safe.directory {shq(args.source_root)} || true
  git -C {shq(args.source_root)} fetch origin +{shq(args.branch)}:refs/remotes/origin/{shq(args.branch)}
  mkdir -p "$(dirname -- {shq(args.project_root)})"
  git -C {shq(args.source_root)} worktree add --detach {shq(args.project_root)} refs/remotes/origin/{shq(args.branch)}
fi
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Launch fm-sf-5 H100x8 open-loop pretrain with commit-aligned Flow Matching weights."
    )
