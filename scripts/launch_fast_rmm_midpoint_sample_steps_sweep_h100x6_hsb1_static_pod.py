#!/usr/bin/env python3
"""Launch a Fast-RMM midpoint flow sample-step sweep on hsb-npc-training-1 H100x6.

This launcher is intentionally conservative:

* it never creates, deletes, or restarts pods;
* it starts/stops only one tmux session inside the target pod;
...
DEFAULT_PODS = ("hsb-npc-training-1",)
...
DEFAULT_SESSION = "fast-rmm-midpoint-sample-steps-sweep-h100x6-hsb1"
DEFAULT_SWEEP_NAME = "fast_rmm_midpoint_sample_steps_sweep_epoch061_x5f9g0ce_h100x6_hsb1"
DEFAULT_WANDB_GROUP = "fast_rmm_midpoint_sample_steps_sweep_epoch061_x5f9g0ce_h100x6_hsb1_bs56"
DEFAULT_MASTER_PORT = "29950"
DEFAULT_INTER_RUN_SLEEP_SEC = 15
DEFAULT_VAL_BATCH_SIZE = 56
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "existing hsb-npc-training-1 H100x6 pod."
        )
    )
    ...
    if len(args.pods) != 1 and not args.stop:
        parser.error("this preset expects exactly one pod")
