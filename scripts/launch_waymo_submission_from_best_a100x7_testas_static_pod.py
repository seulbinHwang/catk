...
This launcher targets the existing ``testas`` A100x7 pod. It never
creates, deletes, or restarts pods. By default it reads the summary produced by
``launch_fast_rmm_epoch_sweep_a100x7_testas_static_pod.py``, resolves the
...
DEFAULT_SESSION = "catk-flow-waymo-val-submission-a100x7-testas"
DEFAULT_TASK_NAME = "flow_control_waymo_val_best_rmm_a100x7_testas"
...
def render_worker_script(args: argparse.Namespace) -> str:
    ...
    tags = (
        f"[waymo_submission,a100x7,{args.branch},{args.task_name},"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Waymo validation submission on testas A100x7 from "
    ...
    parser.add_argument("--wandb-group", default="waymo_submission_best_rmm_a100x7_testas")
    parser.add_argument(
    ...
    if args.nproc_per_node != 7:
        parser.error("this testas launcher expects --nproc-per-node 7")
