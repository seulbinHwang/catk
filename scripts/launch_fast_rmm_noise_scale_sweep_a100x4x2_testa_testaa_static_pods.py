#!/usr/bin/env python3
"""Launch a Fast-RMM flow noise-scale sweep on existing testa/testaa A100 4+4 pods.
...
* it fixes ``model.model_config.validation_rollout_sampling.sample_steps=16``;
* it sweeps ``model.model_config.validation_rollout_sampling.noise_scale``
  while keeping the checkpoint and all other validation settings fixed;
* it explicitly keeps stop-motion disabled.

Use this to measure how Fast-RMM changes as the initial Gaussian noise scale
inside Flow closed-loop rollout changes. The similarly named
``eval_sampling_noise.noise_scale`` experiment config is not used by rollout;
the effective override is ``model.model_config.validation_rollout_sampling.noise_scale``.
"""
...
DEFAULT_NOISE_SCALES = "0.7,0.8,0.9,0.95,1.0,1.05,1.1,1.2,1.3"
DEFAULT_SESSION = "fast-rmm-noise-scale-sweep-a100x4x2-testa-testaa"
DEFAULT_SWEEP_NAME = "fast_rmm_noise_scale_sweep_epoch061_x5f9g0ce_a100x4x2"
DEFAULT_WANDB_GROUP = "fast_rmm_noise_scale_sweep_epoch061_x5f9g0ce_a100x4x2_bs42"
DEFAULT_MASTER_PORT = "29910"
...
def parse_noise_scales(value: str) -> list[float]:
    scales: list[float] = []
    for raw_item in value.split(","):
        ...
        try:
            ...
            raise argparse.ArgumentTypeError(f"invalid noise scale: {item!r}") from exc
        if not math.isfinite(scale) or scale <= 0.0:
            raise argparse.ArgumentTypeError("noise scales must be finite positive values")
        scales.append(scale)
    if not scales:
        raise argparse.ArgumentTypeError("--noise-scales must not be empty")
    return scales


def format_noise_scale(value: float) -> str:
    text = f"{value:.12g}"
...
def render_worker_script(
    *,
    ...
    tags = (
        f"[fast_rmm,noise_scale_sweep,a100x4x2,{args.branch},"
...
cd "$PROJECT_ROOT"
echo "[$(date '+%F %T')] sweep start pod={layout.pod} epoch={args.epoch} sample_steps=$SAMPLE_STEPS noise_scales={','.join(format_noise_scale(s) for s in args.noise_scales)} val_batch_size=$VAL_BATCH_SIZE n_rollout_closed_val=$N_ROLLOUT_CLOSED_VAL" | tee -a "$STATUS_FILE"
...
run_one_noise_scale() {{
  local noise_scale="$1"
  local noise_index="$2"
  local master_port="$((BASE_MASTER_PORT + noise_index))"
  local ckpt
  ckpt="$(download_ckpt_if_needed)"
  local padded_epoch noise_slug
  padded_epoch="$(printf '%03d' "$CHECKPOINT_EPOCH")"
  noise_slug="${{noise_scale//./p}}"
  noise_slug="${{noise_slug//-/_neg_}}"
  local task_name="${{SWEEP_NAME}}_epoch_${{padded_epoch}}_noise_${{noise_slug}}_rmm_only_bs${{VAL_BATCH_SIZE}}"
  local task_dir="$RUN_LOG_DIR/$task_name"
  mkdir -p "$task_dir"

  echo "[$(date '+%F %T')] START epoch=$CHECKPOINT_EPOCH sample_steps=$SAMPLE_STEPS noise_scale=$noise_scale task=$task_name ckpt=$ckpt port=$master_port" | tee -a "$STATUS_FILE"
  ...
  export CATK_HYDRA_OVERRIDES="++trainer.strategy._target_=src.smart.utils.heterogeneous_torchelastic.HeterogeneousDDPStrategy ++trainer.strategy.cluster_environment._target_=src.smart.utils.heterogeneous_torchelastic.HeterogeneousTorchElasticEnvironment model.model_config.val_closed_loop=true model.model_config.val_open_loop=false model.model_config.n_rollout_closed_val=${{N_ROLLOUT_CLOSED_VAL}} model.model_config.validation_rollout_sampling.sample_steps=${{SAMPLE_STEPS}} model.model_config.validation_rollout_sampling.noise_scale=${{NOISE_SCALE}} model.model_config.decoder.use_stop_motion=false model.model_config.self_forced.use_stop_motion=false model.model_config.scorer_scene_num=${{SCORER_SCENE_NUM}} logger.wandb.group=${{WANDB_GROUP}} logger.wandb.job_type=fast_rmm_noise_scale_sweep logger.wandb.tags={tags} logger.wandb.log_model=false"
  ...
  set -e
  if [[ "$status" != "0" ]]; then
    echo "[$(date '+%F %T')] FAIL noise_scale=$noise_scale status=$status" | tee -a "$STATUS_FILE"
    return "$status"
  fi
  echo "[$(date '+%F %T')] DONE noise_scale=$noise_scale" | tee -a "$STATUS_FILE"
  sleep "$INTER_RUN_SLEEP_SEC"
}}

IFS=',' read -r -a noise_scale_values <<< {shq(','.join(format_noise_scale(s) for s in args.noise_scales))}
for noise_index in "${{!noise_scale_values[@]}}"; do
  run_one_noise_scale "${{noise_scale_values[$noise_index]}}" "$noise_index"
done
...
def render_start_command(
    *,
    ...
    run_root = f"{args.remote_log_dir.rstrip('/')}/{args.sweep_name}"
    script_path = f"{run_root}/{layout.pod}_run_noise_scale_sweep.sh"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "model.model_config.validation_rollout_sampling.noise_scale on "
    ...
    parser.add_argument(
        ...
        help="Fixed Flow denoising sample_steps value. Default: 16.",
    )
    parser.add_argument(
        "--noise-scales",
        type=parse_noise_scales,
        default=parse_noise_scales(DEFAULT_NOISE_SCALES),
        help=(
            "Comma-separated validation_rollout_sampling.noise_scale sweep, "
            "e.g. 0.7,0.8,0.9,0.95,1.0."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "rendezvous sockets time to settle before the next noise_scale run."
        ),
    )
    ...
    if args.sample_steps < 1:
        parser.error("--sample-steps must be >= 1")
...
def main() -> None:
    ...
    print(
        "[launcher] per-noise-scale ports: "
        f"{args.master_port}..{int(args.master_port) + len(args.noise_scales) - 1}"
    )
    ...
    print(f"[launcher] sample steps: {args.sample_steps}")
    print(f"[launcher] noise scales: {','.join(format_noise_scale(s) for s in args.noise_scales)}")
