#!/usr/bin/env python3
"""Launch H100x4x2 epoch-last Fast WOSAC validation on the RTX 5090 host.

The source checkpoint is copied from the completed ``hsb-npc-training`` pod, then a new
...
DEFAULT_WINDOW_NAME = "fast-wosac-h100x4x2-last"
DEFAULT_TASK_NAME = (
...
def run_shell(
    ...
    *,
    ...
    return run(["bash", "-lc", script], capture=capture, dry_run=dry_run, timeout=timeout)
...
def source_checkpoint_epoch(args: argparse.Namespace, source_ckpt: str) -> int:
    if args.dry_run:
        return args.expected_epoch

    inspect_script = f"""
set -Eeuo pipefail
ckpt={shq(source_ckpt)}
if command -v python >/dev/null 2>&1; then
  PYTHON_BIN="$(command -v python)"
elif [[ -x /mnt/nuplan/miniforge/envs/catk/bin/python ]]; then
...
"""
    ...
    if not lines:
        raise RuntimeError(f"failed to inspect checkpoint epoch: {source_ckpt}")
    return int(lines[-1])


def verify_source_checkpoint_epoch(args: argparse.Namespace, source_ckpt: str) -> int:
    epoch = source_checkpoint_epoch(args, source_ckpt)
    if args.expected_epoch >= 0 and epoch != args.expected_epoch and not args.allow_epoch_mismatch:
        raise RuntimeError(
            f"source checkpoint epoch mismatch: expected {args.expected_epoch}, got {epoch}. "
            "Pass --allow-epoch-mismatch only if this is intentional."
        )
    return epoch


def copy_checkpoint_to_remote_by_chunks(
    ...
    *,
    ...
    tmp_path = args.remote_ckpt_path + ".chunks_tmp"
    run_ssh(args, f"rm -f {shq(tmp_path)} && : > {shq(tmp_path)}")

    chunk_bytes = chunk_mib * 1024 * 1024
    total_chunks = math.ceil(source_size / chunk_bytes)
    for chunk_index in range(total_chunks):
        expected_size = min(source_size, (chunk_index + 1) * chunk_bytes)
        offset_size = chunk_index * chunk_bytes
        for attempt in range(1, max_retries + 1):
            run_ssh(args, f"truncate -s {offset_size} {shq(tmp_path)}")
            pipe = (
                "set -o pipefail; "
                f"kubectl exec -n {shq(args.source_namespace)} {shq(args.source_pod)} "
                f"-c {shq(args.source_container)} -- "
                f"dd if={shq(source_ckpt)} bs={chunk_bytes} skip={chunk_index} count=1 status=none | "
                f"ssh {shq(args.ssh_host)} "
                f"{shq('cat >> ' + shq(tmp_path))}"
            )
            try:
                run_shell(pipe, timeout=15)
                remote_size = remote_file_size(args, tmp_path)
                if remote_size == expected_size:
                    print(
                        f"[launcher] copied chunk {chunk_index + 1}/{total_chunks} "
                        f"({remote_size}/{source_size} bytes)"
                    )
                    break
                raise RuntimeError(
                    f"chunk size mismatch: expected {expected_size}, got {remote_size}"
                )
                ...
                if remote_size == expected_size:
                    print(
                        f"[launcher] copied chunk {chunk_index + 1}/{total_chunks} "
                        f"({remote_size}/{source_size} bytes; accepted after timeout)"
                    )
                    break
                if attempt >= max_retries:
                    raise RuntimeError(
                        f"failed to copy chunk {chunk_index + 1}/{total_chunks}: {exc}"
                    ) from exc
                print(
                    f"[launcher] chunk {chunk_index + 1}/{total_chunks} failed "
                    f"(attempt {attempt}/{max_retries}); retrying: {exc}"
                )
        else:
            raise RuntimeError(f"failed to copy chunk {chunk_index + 1}/{total_chunks}")

    run_ssh(args, f"mv {shq(tmp_path)} {shq(args.remote_ckpt_path)}")
    remote_size = remote_file_size(args, args.remote_ckpt_path)
    if remote_size != source_size:
        raise RuntimeError(
            f"chunked checkpoint copy size mismatch: source={source_size}, remote={remote_size}"
        )
...
def copy_checkpoint_to_remote(args: argparse.Namespace, source_ckpt: str) -> None:
    ...
    print(f"[launcher] copying checkpoint from {args.source_pod}:{source_ckpt}")
    print(
        f"[launcher] remote checkpoint: {args.remote_ckpt_path} ({source_size} bytes)"
    )
    print("[launcher] using chunked copy to avoid long kubectl stream EOFs")
    copy_checkpoint_to_remote_by_chunks(args, source_ckpt, source_size)
...
echo "[fast-wosac-h100x4x2-last] host=$(hostname) task=${{TASK_NAME}}"
echo "[fast-wosac-h100x4x2-last] started at $(date '+%F %T')"
echo "[fast-wosac-h100x4x2-last] repo=$(pwd)"
echo "[fast-wosac-h100x4x2-last] commit=$(git rev-parse --short HEAD 2>/dev/null) $(git log -1 --pretty=%s 2>/dev/null)"
echo "[fast-wosac-h100x4x2-last] python=$(command -v python 2>/dev/null)"
echo "[fast-wosac-h100x4x2-last] cache=${{CACHE_ROOT}}"
echo "[fast-wosac-h100x4x2-last] ckpt=${{CKPT_PATH}}"
echo "[fast-wosac-h100x4x2-last] scorer_scene_num=${{SCORER_SCENE_NUM}} val_batch_size=${{VAL_BATCH_SIZE}} limit_val_batches=${{LIMIT_VAL_BATCHES}}"
echo "[fast-wosac-h100x4x2-last] n_rollout_closed_val=${{N_ROLLOUT_CLOSED_VAL}} nproc=${{NPROC_PER_NODE}} precision=${{PRECISION}}"
echo

if [[ ! -f "$CKPT_PATH" ]]; then
  echo "[fast-wosac-h100x4x2-last] checkpoint does not exist: $CKPT_PATH" >&2
  exec bash
fi
if [[ ! -d "$CACHE_ROOT" ]]; then
  echo "[fast-wosac-h100x4x2-last] CACHE_ROOT does not exist: $CACHE_ROOT" >&2
  exec bash
fi
if ! command -v python >/dev/null 2>&1; then
  echo "[fast-wosac-h100x4x2-last] python is not available after conda activation" >&2
  exec bash
fi
...
ARGS=(
  ...
  "logger.wandb.tags=[fast_wosac,h100x4x2_fullvalid,epoch_last,rtx5090,control_space]"
)

echo "[fast-wosac-h100x4x2-last] output_dir=$OUTPUT_DIR"
echo "[fast-wosac-h100x4x2-last] command: python -m torch.distributed.run --standalone --nproc_per_node=$NPROC_PER_NODE ${{ARGS[*]}}"
python -m torch.distributed.run --standalone --nproc_per_node="$NPROC_PER_NODE" "${{ARGS[@]}}"
status=$?

echo
echo "[fast-wosac-h100x4x2-last] exited with status $status at $(date '+%F %T')"
echo "[fast-wosac-h100x4x2-last] output_dir=$OUTPUT_DIR"
echo "[fast-wosac-h100x4x2-last] leaving shell open for inspection"
exec bash
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Copy the latest H100x4x2 epoch_last.ckpt from hsb-npc-training and launch "
    ...
    parser.add_argument(
        ...
        type=int,
        default=63,
        help="Expected 0-indexed epoch stored in epoch_last.ckpt. Use -1 to skip.",
    )
    parser.add_argument(
        "--allow-epoch-mismatch",
        action="store_true",
        help="Launch even when the source checkpoint epoch differs from --expected-epoch.",
    )
    ...
    if args.expected_epoch < -1:
        parser.error("--expected-epoch must be >= -1")
    return args
...
def main() -> None:
    ...
    source_epoch = verify_source_checkpoint_epoch(args, source_ckpt)
    limit_val_batches = (
        default_limit_val_batches(args)
        if args.limit_val_batches == "auto"
        else args.limit_val_batches
    ...
    print(f"[launcher] source epoch:   {source_epoch}")
