#!/usr/bin/env bash
# Generate and optionally upload a full Waymo Sim Agents 2025 test submission
# on the existing testa/testaa A100 x4 pods.
#
# Required:
...
if [[ -z "${CKPT_PATH:-}" ]]; then
  ...
  echo "  TASK_NAME=smart_waymo_test_a100x4x2 \\" >&2
  echo "  bash scripts/start_smart_a100x4x2_testa_waymo_test_submission.sh" >&2
  exit 2
fi

export ACTION="${ACTION:-test}"
export EXPERIMENT="${EXPERIMENT:-sim_agents_sub}"
export TASK_NAME="${TASK_NAME:-smart_waymo_test_a100x4x2_$(date +%Y%m%d_%H%M%S)}"
export SESSION="${SESSION:-catk-smart-waymo-test-submission-a100x4x2}"
export VAL_BATCH_SIZE="${VAL_BATCH_SIZE:-12}"
export TEST_BATCH_SIZE="${TEST_BATCH_SIZE:-12}"
export LIMIT_TEST_BATCHES="${LIMIT_TEST_BATCHES:-1.0}"
export WANDB_MODE="${WANDB_MODE:-online}"

export CATK_SUBMISSION_STREAM_SHARDS="${CATK_SUBMISSION_STREAM_SHARDS:-1}"
export CATK_SUBMISSION_SHARD_STREAM_PORT="${CATK_SUBMISSION_SHARD_STREAM_PORT:-29641}"
