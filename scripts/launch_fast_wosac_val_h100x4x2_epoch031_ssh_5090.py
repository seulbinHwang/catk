#!/usr/bin/env python3
"""Launch H100x4x2 epoch-31 Fast WOSAC validation on the RTX 5090 host.

The source checkpoint is copied from the running ``hsb-npc-training`` pod,
then a new window is opened inside the existing ``hsb-rl-train`` tmux session
on the SSH host. The pod is not stopped or modified.
"""
...
def copy_checkpoint_to_remote(args: argparse.Namespace, source_ckpt: str) -> None:
    ...
    if remote_size != source_size:
        ...
        with tempfile.TemporaryDirectory(prefix="catk-h100x4x2-ckpt-") as tmpdir:
            local_ckpt = os.path.join(tmpdir, "epoch_031.ckpt")
            run(
                [
                    "kubectl",
                    "cp",
...
echo "[fast-wosac-h100x4x2] host=$(hostname) task=${{TASK_NAME}}"
echo "[fast-wosac-h100x4x2] started at $(date '+%F %T')"
echo "[fast-wosac-h100x4x2] repo=$(pwd)"
echo "[fast-wosac-h100x4x2] commit=$(git rev-parse --short HEAD 2>/dev/null) $(git log -1 --pretty=%s 2>/dev/null)"
echo "[fast-wosac-h100x4x2] python=$(command -v python 2>/dev/null)"
echo "[fast-wosac-h100x4x2] cache=${{CACHE_ROOT}}"
echo "[fast-wosac-h100x4x2] ckpt=${{CKPT_PATH}}"
echo "[fast-wosac-h100x4x2] scorer_scene_num=${{SCORER_SCENE_NUM}} val_batch_size=${{VAL_BATCH_SIZE}} limit_val_batches=${{LIMIT_VAL_BATCHES}}"
echo "[fast-wosac-h100x4x2] n_rollout_closed_val=${{N_ROLLOUT_CLOSED_VAL}} nproc=${{NPROC_PER_NODE}} precision=${{PRECISION}}"
echo

if [[ ! -f "$CKPT_PATH" ]]; then
  echo "[fast-wosac-h100x4x2] checkpoint does not exist: $CKPT_PATH" >&2
  exec bash
fi
if [[ ! -d "$CACHE_ROOT" ]]; then
  echo "[fast-wosac-h100x4x2] CACHE_ROOT does not exist: $CACHE_ROOT" >&2
  exec bash
fi
if ! command -v python >/dev/null 2>&1; then
  echo "[fast-wosac-h100x4x2] python is not available after conda activation" >&2
  exec bash
fi
...
ARGS=(
  ...
  "logger.wandb.tags=[fast_wosac,h100x4x2_fullvalid,epoch031,rtx5090,control_space]"
)

echo "[fast-wosac-h100x4x2] output_dir=$OUTPUT_DIR"
echo "[fast-wosac-h100x4x2] command: python -m torch.distributed.run --standalone --nproc_per_node=$NPROC_PER_NODE ${{ARGS[*]}}"
python -m torch.distributed.run --standalone --nproc_per_node="$NPROC_PER_NODE" "${{ARGS[@]}}"
status=$?

echo
echo "[fast-wosac-h100x4x2] exited with status $status at $(date '+%F %T')"
echo "[fast-wosac-h100x4x2] output_dir=$OUTPUT_DIR"
echo "[fast-wosac-h100x4x2] leaving shell open for inspection"
exec bash
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Copy the H100x4x2 epoch_031.ckpt from hsb-npc-training and launch "
