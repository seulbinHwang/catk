...
def _trajectory_feature(traj: torch.Tensor, heading_weight: float) -> torch.Tensor:
    pos = traj[..., :2]
    head = traj[..., 2]
    return torch.cat(
        [
            pos,
            heading_weight * head.cos().unsqueeze(-1),
            heading_weight * head.sin().unsqueeze(-1),
        ],
        dim=-1,
    ).flatten(1, 2)
...
def minibatch_kmeans(
    ...
    features = _trajectory_feature(trajectories, heading_weight=heading_weight)
    cent_features = _trajectory_feature(centroids, heading_weight=heading_weight)
    for _ in tqdm(range(num_iters), desc="mini-batch k-means"):
        ...
        assign = torch.cdist(batch_features, cent_features).argmin(dim=1)
