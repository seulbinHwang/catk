#!/usr/bin/env python3
"""Launch self-forced V100x3x5 static multi-node training on existing pods."""
...
DEFAULT_EXPERIMENT = "self_forced_npfm_v100x3x5"
...
echo "[self-forced-v100x3x5] pod=$(hostname) rank=${{NODE_RANK}} task=${{TASK_NAME}}"
echo "[self-forced-v100x3x5] started at $(date '+%F %T')"
echo "[self-forced-v100x3x5] experiment=${{EXPERIMENT}} bs=${{INITIAL_BS}} precision=${{PRECISION}}"
echo "[self-forced-v100x3x5] pretrain_artifact=${{WANDB_PRETRAIN_ARTIFACT}}"
echo "[self-forced-v100x3x5] pretrain_ckpt=${{PRETRAIN_CKPT}}"
echo "[self-forced-v100x3x5] attach survives after exit; press Ctrl-b d to detach"
echo
...
PY
  ...
  echo "[self-forced-v100x3x5] retry sync server started on $MASTER_ADDR:$RETRY_SYNC_PORT pid=$RETRY_SYNC_PID"
}}
...
wait_for_retry_sync_server() {{
  ...
  while true; do
    ...
    if (( waited >= timeout_sec )); then
      echo "[self-forced-v100x3x5] retry sync server did not become ready at $RETRY_SYNC_HOST:$RETRY_SYNC_PORT" >&2
      return 1
    fi
    sleep 2
    waited=$(( waited + 2 ))
...
PY
  ...
  echo "[self-forced-v100x3x5] failed to post retry status for $attempt_name after retries" >&2
  return 1
}}

ensure_pretrain_checkpoint() {{
  if [[ -f "$PRETRAIN_CKPT" ]]; then
    echo "[self-forced-v100x3x5] using cached pretrain checkpoint: $PRETRAIN_CKPT"
    return 0
  fi
  ...
  if mkdir "$lock_dir" 2>/dev/null; then
    echo "[self-forced-v100x3x5] downloading W&B artifact: $WANDB_PRETRAIN_ARTIFACT"
    python - <<'PY'
import glob
import os
import shutil
...
PY
    ...
    rm -rf "$lock_dir"
    if (( status != 0 )); then
      echo "[self-forced-v100x3x5] W&B artifact download failed with status $status" >&2
      return "$status"
    fi
  else
    echo "[self-forced-v100x3x5] waiting for checkpoint download lock: $lock_dir"
    for _ in $(seq 1 180); do
      if [[ -f "$PRETRAIN_CKPT" ]]; then
        echo "[self-forced-v100x3x5] checkpoint appeared: $PRETRAIN_CKPT"
        return 0
      fi
      sleep 10
    done
    echo "[self-forced-v100x3x5] timed out waiting for $PRETRAIN_CKPT" >&2
    return 4
  fi
...
wait_for_attempt_statuses() {{
  ...
  while true; do
    ...
    if (( waited >= timeout_sec )); then
      echo "[self-forced-v100x3x5] timed out waiting for attempt $attempt status files: got $count/$NNODES" >&2
      return 1
    fi
    sleep 5
    waited=$(( waited + 5 ))
...
while (( bs >= MIN_BS )); do
  ...
  echo
  echo "[self-forced-v100x3x5] $attempt_tag action=$action ckpt=$ckpt_path"
  printf '[self-forced-v100x3x5] torchrun'
  ...
  if ! wait_for_attempt_statuses; then
    echo "[self-forced-v100x3x5] retry barrier failed for $attempt_tag; see $RETRY_STATE_DIR" >&2
    exit 1
  fi

  if ! global_attempt_has_failure; then
    echo "[self-forced-v100x3x5] training completed successfully at bs=$bs"
    exit 0
  fi

  if global_attempt_has_oom; then
    next_bs=$(( bs - OOM_STEP ))
    echo "[self-forced-v100x3x5] OOM detected on at least one node in attempt $attempt; all nodes lowering bs $bs -> $next_bs"
    bs="$next_bs"
    continue
  fi

  echo "[self-forced-v100x3x5] non-OOM failure status=$status; see $attempt_log and $RETRY_STATE_DIR/$attempt_tag.*.status"
  exit "$status"
done

echo "[self-forced-v100x3x5] reached MIN_BS=$MIN_BS without success"
exit 1
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Launch self-forced V100x3x5 training on existing static pods.",
    )
    ...
    parser.add_argument("--master-port", default="29545")
    parser.add_argument(
        "--retry-sync-port",
        default="29546",
    ...
    parser.add_argument("--scorer-scene-num", type=int, default=300)
    parser.add_argument("--unfrozen-range", default="except_map_encoder")
    parser.add_argument("--estimator-warmup-epochs", type=int, default=4)
