...
while (( bs >= MIN_BS )); do
  ...
  if grep -Eq "$OOM_REGEX" "$RUN_ROOT"/*.${{attempt_tag}}.log 2>/dev/null; then
    next_bs=$(( bs - OOM_STEP ))
    echo "[self-forced-v100x3x5] OOM detected in attempt $attempt; lowering bs $bs -> $next_bs"
    bs="$next_bs"
    continue
  fi

  echo "[self-forced-v100x3x5] non-OOM failure status=$status; see $attempt_log"
  exit "$status"
done
