...
def parse_args() -> argparse.Namespace:
    ...
    if args.detach_block_transition not in {"true", "false"}:
        parser.error("--detach-block-transition must be 'true' or 'false'")
    if args.use_anchor_flow_matching_loss not in {"true", "false"}:
        parser.error("--use-anchor-flow-matching-loss must be 'true' or 'false'")
