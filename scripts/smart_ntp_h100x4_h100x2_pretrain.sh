...
#
# The intended layout is hsb-npc-training(H100x4) + wo-pvc(H100x2). Prefer
...
validate_strict_pretrain_overrides() {
  local experiment="$1"
  local action="$2"
  shift 2
...
main() {
  ...
  local nproc_per_node="${NPROC_PER_NODE:-gpu}"
  ...
  if (( ${#extra_overrides[@]} > 0 )); then
    validate_strict_pretrain_overrides "$experiment" "$action" "${extra_overrides[@]}" "$@"
  else
