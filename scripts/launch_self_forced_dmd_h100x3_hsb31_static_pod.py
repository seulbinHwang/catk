#!/usr/bin/env python3
"""Launch DMD-style self-forced fine-tuning on hsb-npc-training-3-1 H100x3.

The launcher does not create, delete, or restart pods. It prepares a clean
checkout under /tmp inside the existing pod, downloads the pretrained Generator
checkpoint if needed, and starts one tmux session running torchrun on GPUs 0,1,2.
"""
...
DEFAULT_POD = "hsb-npc-training-3-1"
DEFAULT_CONTAINER = "main"
DEFAULT_REPO_URL = "https://github.com/seulbinHwang/catk.git"
DEFAULT_PROJECT_ROOT = "/tmp/catk_self_forced_dmd_h100x3_hsb31"
...
DEFAULT_EXPERIMENT = "self_forced_npfm_h100_3_hsb31"
DEFAULT_WANDB_PRETRAIN_ARTIFACT = (
    "jksg01019-naver-labs/SMART-FLOW/epoch-last-x5f9g0ce:v57"
)
DEFAULT_PRETRAIN_CKPT = (
    "/workspace/flow_self_forced_dmd_h100x3_hsb31_pretrain_epoch061_x5f9g0ce/"
    "v57/epoch_061.ckpt"
)
...
def run_root(args: argparse.Namespace) -> str:
    ...
    return f"{args.log_dir.rstrip('/')}/tmux_self_forced_dmd_h100x3_{safe_pod}/{safe_task}"
...
echo "[self-forced-dmd-h100x3-{pod_label}] pod=$(hostname) task=${{TASK_NAME}}"
echo "[self-forced-dmd-h100x3-{pod_label}] started at $(date '+%F %T')"
echo "[self-forced-dmd-h100x3-{pod_label}] experiment=${{EXPERIMENT}} initial_bs=${{INITIAL_BS}} oom_step=${{OOM_STEP}} min_bs=${{MIN_BS}}"
echo "[self-forced-dmd-h100x3-{pod_label}] lr=${{CATK_LR}} fraction=${{TRAIN_EPOCH_SAMPLE_FRACTION}} memory_balanced=${{TRAIN_MEMORY_BALANCED_BATCHES}} sample_steps=16 backprop_last_k=${{BACKPROP_LAST_K}}"
echo "[self-forced-dmd-h100x3-{pod_label}] pretrain_artifact=${{WANDB_PRETRAIN_ARTIFACT}}"
echo "[self-forced-dmd-h100x3-{pod_label}] pretrain_ckpt=${{PRETRAIN_CKPT}}"
echo

ensure_pretrain_checkpoint() {{
  if [[ -f "$PRETRAIN_CKPT" ]]; then
    echo "[self-forced-dmd-h100x3-{pod_label}] using cached pretrain checkpoint: $PRETRAIN_CKPT"
    return 0
  fi
  ...
  if mkdir "$lock_dir" 2>/dev/null; then
    echo "[self-forced-dmd-h100x3-{pod_label}] downloading W&B artifact: $WANDB_PRETRAIN_ARTIFACT"
    python - <<'PY'
import glob
import os
import shutil
...
PY
    ...
    echo "[self-forced-dmd-h100x3-{pod_label}] waiting for checkpoint download lock: $lock_dir"
    ...
    echo "[self-forced-dmd-h100x3-{pod_label}] timed out waiting for $PRETRAIN_CKPT" >&2
    return 4
  fi
...
status=$?
if (( status != 0 )); then
  echo "[self-forced-dmd-h100x3-{pod_label}] checkpoint preparation failed with status $status"
  exec bash
fi

bash scripts/self_forced_h100_3_with_oom_retry.sh
status=$?

echo
echo "[self-forced-dmd-h100x3-{pod_label}] exited with status $status at $(date '+%F %T')"
echo "[self-forced-dmd-h100x3-{pod_label}] leaving shell open for inspection"
exec bash
"""
...
def render_graceful_stop_block(session: str) -> str:
    return f"""
if tmux has-session -t {shq(session)} 2>/dev/null; then
  tmux send-keys -t {shq(session)} C-c || true
  sleep 20
  if tmux has-session -t {shq(session)} 2>/dev/null; then
    tmux send-keys -t {shq(session)} C-c || true
    sleep 10
...
if [ ! -d {shq(args.project_root)}/.git ]; then
  ...
  git clone --branch {shq(args.branch)} {shq(args.repo_url)} {shq(args.project_root)}
fi
...
"""
    ...
    if args.replace:
        session_block = render_graceful_stop_block(args.session)
    else:
        session_block = f"""
...
def render_stop_command(session: str) -> str:
    return f"""set -Eeuo pipefail
{render_graceful_stop_block(session)}
echo "[launcher] stopped tmux session if it existed: {session}"
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-bs", type=int, default=160)
    ...
    if args.initial_bs != 160:
        parser.error("--initial-bs is fixed at 160 for this requested recipe")
...
def main() -> None:
    ...
    print(f"[launcher] checkout:   {args.project_root}")
    print(f"[launcher] artifact:   {args.wandb_pretrain_artifact}")
    print(f"[launcher] ckpt path:  {args.pretrain_ckpt}")
    print(f"[launcher] batch:      {args.initial_bs} fixed, min {args.min_bs}")
