#!/usr/bin/env python3
"""Launch main-branch RoaD fine-tuning on the static testas A100x7 pod.

The launcher never creates, deletes, or restarts pods. It prepares a clean
main-branch checkout under /tmp on the existing pod, then starts the in-pod
training script in a tmux session.
"""
...
def start_tmux(
    ...
    log_file = f"{log_dir}/{pod}.road_clsft.log"
    ...
    env = {
        ...
        "NPROC_PER_NODE": "7",
        "TRAINER_DEVICES": "7",
        "CKPT_ARTIFACT": "jksg01019-naver-labs/SMART-FLOW/epoch-last-1iapr5ed:v21",
        "ROAD_CACHE_ROOT": f"/workspace/road_cache/{task_name}",
    }
    if smoke:
        env.update(
            {
                "TASK_NAME": task_name,
                "ROAD_DATA_USE_RATIO": "0.0002",
                "MAX_EPOCHS": "1",
                "LIMIT_TRAIN_BATCHES": "1",
                "LIMIT_VAL_BATCHES": "0",
                "ROAD_DELETE_AFTER_USE": "false",
            }
        )
...
tmux new-session -d -s {quote(session)} "cd {quote(project_dir)} && {exports}; bash scripts/start_road_clsft_testas_a100x7_epoch21.sh 2>&1 | tee {quote(log_file)}"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--project-dir", default="/tmp/catk_main_road_clsft_testas")
    parser.add_argument("--session", default="catk-road-clsft-testas-a100x7-main-epoch21")
    parser.add_argument(
        "--task-name",
        default="road_clsft_a100x7_testas_main_1iapr5ed_epoch21_bs12",
    )
...
def main() -> None:
    ...
    if args.smoke and task_name == "road_clsft_a100x7_testas_main_1iapr5ed_epoch21_bs12":
        task_name = f"road_clsft_a100x7_testas_main_smoke_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
