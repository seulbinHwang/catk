#!/usr/bin/env python3
"""Launch the fm-sf-4 H100x8 max-closed-loop-epochs=10 see_all=false run."""
...
def main() -> int:
    ...
    task_name = (
        "flow_closed_loop_self_forced_h100x8_fmsf4_g4_seeallfalse_"
        "1e1dec_lrdecay001_maxcl10_resume_e5_"
        f"{timestamp}"
    )
    args = [
        sys.executable,
        ...
        "main",
        "--project-root",
        "/tmp/catk_self_forcing_closed_loop_fmsf4_seeall_maxcl10",
        "--branch",
        "self_forcing_closed_loop",
        "--pull",
        "--replace",
        "--session",
        "catk-closed-loop-sf-g4-seeallfalse-maxcl10-fmsf4",
        "--task-name",
        task_name,
        "--experiment",
