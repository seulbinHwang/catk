...
Pipeline (per scenario):
  ① N-rollout flow model         →  Official RMM (N_ROLLOUTS rollouts, baseline)
  ② K rollouts (no ascent)       →  Official RMM (K rollouts, fair before)
  ③ Gradient ascent on each of K rollouts independently
  ④ K optimised rollouts         →  Official RMM (K rollouts, after)

K = N_VIS_ROLLOUTS  (video count == ascent count == comparison count)
...
Comparison ②→④ is apples-to-apples (same K rollouts, before/after ascent).
...
N_VIS_ROLLOUTS = 4    # rollouts used for video + gradient ascent (②③④)
N_ASCENT_STEPS = 100  # gradient ascent iterations per rollout
ASCENT_LR      = 3e-3
VERBOSE_EVERY  = 20   # print full sub-metric breakdown every N steps
...
def run_gradient_ascent(
    ...
    log_feat_dict: dict,
    config: sim_agents_metrics_pb2.SimAgentMetricsConfig,
    rollout_idx: int = 0,   # used only for display
    ...
    for step in range(1, n_steps + 1):
        ...
        if should_print:
            ...
            print(
                f"    [r{rollout_idx}] step {step:4d}/{n_steps}"
...
def main():
    ...
    print(f"Scenarios: {N_SCENARIOS}  |  N_ROLLOUTS: {N_ROLLOUTS}  |  "
          f"N_VIS_ROLLOUTS: {N_VIS_ROLLOUTS}  |  ascent steps: {N_ASCENT_STEPS}")
    ...
    for i_sc, (scen_id, tfr) in enumerate(zip(scenario_ids, tfrecord_paths)):
        ...
        K = min(N_VIS_ROLLOUTS, pt.shape[1])   # actual rollouts to use
        ...
        # ── ② Official RMM: K rollouts, before ───────────────────────────────
        print(f"  ② Official RMM (K={K} rollouts, before) ...", end=" ", flush=True)
        rmm_k_before = compute_official_rmm_waymo(
            scenario, pt[:, :K], pz[:, :K], ph[:, :K], aids, wm_cfg
        )
        print(f"{rmm_k_before:.6f}")

        # Per-rollout soft RMM before
        soft_before_list = []
        with torch.no_grad():
            for r in range(K):
                res = compute_soft_rmm(
                    scenario=scenario,
                    x=pt[:, r, :, 0], y=pt[:, r, :, 1],
                    z=pz[:, r, :], head=ph[:, r, :],
                ...
                soft_before_list.append(res)
        soft_before_mean = sum(float(r.metametric) for r in soft_before_list) / K
        print(f"     Soft RMM (K={K} rollouts, before, avg) = {soft_before_mean:.6f}")
        for r in range(K):
            print(f"       rollout-{r}: {float(soft_before_list[r].metametric):.6f}")

        # ── Video before (K rollouts) ─────────────────────────────────────────
        save_before = VIDEO_DIR / f"sc{i_sc:02d}_{scen_id[:8]}_before"
        sr_vis_before = get_scenario_rollouts(
            scenario_id=get_scenario_id_int_tensor([scen_id], DEVICE),
            agent_id=aids,
            agent_batch=torch.zeros(aids.shape[0], dtype=torch.long, device=DEVICE),
            pred_traj=pt[:, :K], pred_z=pz[:, :K], pred_head=ph[:, :K],
        )
        vis_before = VisWaymo(scenario_path=tfr, save_dir=save_before)
        vis_before.save_video_scenario_rollout(sr_vis_before[0], n_vis_rollout=K)
        print(f"     Video before → {save_before.name}/  ({K} rollout videos)")

        # ── ③ Gradient ascent on each of K rollouts independently ─────────────
        print(f"\n  ③ Gradient ascent  ({N_ASCENT_STEPS} steps × {K} rollouts, lr={ASCENT_LR})")
        all_ga_histories: List[List[dict]] = []
        opt_xs, opt_ys, opt_zs, opt_heads = [], [], [], []

        for r in range(K):
            print(f"\n  --- rollout {r}/{K-1} ---")
            ox, oy, oz, oh, hist = run_gradient_ascent(
                scenario=scenario,
                init_x=pt[:, r, :, 0].clone(),
                init_y=pt[:, r, :, 1].clone(),
                init_z=pz[:, r, :].clone(),
                init_head=ph[:, r, :].clone(),
                agent_ids=aids, valid=valid_t,
                log_feat_dict=log_feat_dict, config=wm_cfg,
                rollout_idx=r,
            )
            opt_xs.append(ox); opt_ys.append(oy)
            opt_zs.append(oz); opt_heads.append(oh)
            all_ga_histories.append(hist)

        # Stack into [A, K, 80, *]
        opt_traj = torch.stack(
            [torch.stack([opt_xs[r], opt_ys[r]], dim=-1) for r in range(K)], dim=1
        )   # [A, K, 80, 2]
        opt_z_t  = torch.stack(opt_zs,    dim=1)   # [A, K, 80]
        opt_h_t  = torch.stack(opt_heads, dim=1)   # [A, K, 80]

        # ── ④ Official RMM: K rollouts, after ────────────────────────────────
        print(f"\n  ④ Official RMM (K={K} rollouts, after) ...", end=" ", flush=True)
        rmm_k_after = compute_official_rmm_waymo(scenario, opt_traj, opt_z_t, opt_h_t, aids, wm_cfg)
        print(f"{rmm_k_after:.6f}")
        ...
        with torch.no_grad():
            for r in range(K):
                res = compute_soft_rmm(
                    scenario=scenario,
                    x=opt_xs[r], y=opt_ys[r], z=opt_zs[r], head=opt_heads[r],
                    agent_ids=aids, valid=valid_t,
                    log_feat_dict=log_feat_dict, config=wm_cfg,
                )
                soft_after_list.append(res)
        soft_after_mean = sum(float(r.metametric) for r in soft_after_list) / K
        print(f"     Soft RMM (K={K} rollouts, after,  avg) = {soft_after_mean:.6f}")
        for r in range(K):
            delta_r = float(soft_after_list[r].metametric) - float(soft_before_list[r].metametric)
            sign = "↑" if delta_r > 0 else "↓"
            print(f"       rollout-{r}: {float(soft_before_list[r].metametric):.6f} → "
                  f"{float(soft_after_list[r].metametric):.6f}  ({sign}{delta_r:+.6f})")

        delta_official = rmm_k_after - rmm_k_before
        delta_soft     = soft_after_mean - soft_before_mean
        print(f"\n  ΔOfficial (②→④) : {delta_official:+.6f}  "
              f"({'↑ improved' if delta_official > 0 else '↓ degraded'})")
        print(f"  ΔSoft (avg)      : {delta_soft:+.6f}  "
              f"({'↑ improved' if delta_soft > 0 else '↓ degraded'})")

        # ── Video after (K rollouts) ──────────────────────────────────────────
        save_after = VIDEO_DIR / f"sc{i_sc:02d}_{scen_id[:8]}_after"
        sr_vis_after = get_scenario_rollouts(
            scenario_id=get_scenario_id_int_tensor([scen_id], DEVICE),
            agent_id=aids,
            agent_batch=torch.zeros(aids.shape[0], dtype=torch.long, device=DEVICE),
        ...
        vis_after.save_video_scenario_rollout(sr_vis_after[0], n_vis_rollout=K)
        print(f"     Video after  → {save_after.name}/  ({K} rollout videos)")

        # ── Sub-metric breakdown (avg before → avg after) ─────────────────────
        print(f"\n  Sub-metric likelihoods (avg over K={K} rollouts, before → after):")
        for name, w in _METRIC_WEIGHTS.items():
            key = f"{name}_likelihood"
            v_b = sum(float(r.likelihoods[key]) for r in soft_before_list) / K
            v_a = sum(float(r.likelihoods[key]) for r in soft_after_list) / K
        ...
        summary_rows.append({
            ...
            "rmm_k_before":  rmm_k_before,
            "rmm_k_after":   rmm_k_after,
            "delta_off":     delta_official,
            "soft_before":   soft_before_mean,
            "soft_after":    soft_after_mean,
            "delta_soft":    delta_soft,
            "ga_histories":  all_ga_histories,   # list of K histories
        })

    # ── Summary table ──────────────────────────────────────────────────────────
    K_disp = min(N_VIS_ROLLOUTS, summary_rows[0]["ga_histories"].__len__() if summary_rows else 1)
    print(f"\n{'='*95}")
    print(f"SUMMARY  (K={K_disp} rollouts per scenario)")
    print(f"{'='*95}")
    hdr = (
        f"{'Scenario':<16}  {'RMM(N-roll)':>11}  {'RMM(K,bfr)':>10}  "
        f"{'RMM(K,aft)':>10}  {'Δ(K-roll)':>9}  {'Δsoft(avg)':>10}"
    )
    print(hdr)
    print("-" * len(hdr))
    for r in summary_rows:
        print(
            ...
            f"{r['rmm_k_before']:>10.6f}  "
            f"{r['rmm_k_after']:>10.6f}  "
            f"{r['delta_off']:>+9.6f}  "
            f"{r['delta_soft']:>+10.6f}"
        )

    # Gradient ascent curves for first scenario, all rollouts
    if summary_rows:
        print(f"\nGradient ascent curves  (scenario 0, each rollout):")
        for r_idx, hist in enumerate(summary_rows[0]["ga_histories"]):
            print(f"\n  rollout-{r_idx}:")
