#!/usr/bin/env python3
"""Launch self-forced H100x4 training in tmux on the existing hsb-npc-training pod.
...
DEFAULT_POD = "hsb-npc-training"
DEFAULT_CONTAINER = "main"
DEFAULT_PROJECT_ROOT = "/mnt/nuplan/projects/catk"
DEFAULT_BRANCH = "self_forcing_w_track_loss"
DEFAULT_CACHE_ROOT = "/mnt/nuplan/womd_v1_3/SMART_cache"
DEFAULT_LOG_DIR = "/mnt/nuplan/projects/catk/logs"
DEFAULT_EXPERIMENT = "self_forced_npfm_h100x4_hsb_npc_training"
...
DEFAULT_TASK_NAME = (
    "flow_self_forced_h100x4_hsb_npc_training_"
    "use_stop_motion_false_estimator_warmup_0_lr1e-6_bs28"
)
DEFAULT_SESSION = "catk-sf-h100x4-hsb-npc-training"
...
def run_root(args: argparse.Namespace) -> str:
    ...
    return f"{args.log_dir.rstrip('/')}/tmux_self_forced_h100x4_hsb_npc_training/{safe_task}"
...
echo "[self-forced-h100x4-hsb-npc-training] pod=$(hostname) task=${{TASK_NAME}}"
echo "[self-forced-h100x4-hsb-npc-training] started at $(date '+%F %T')"
echo "[self-forced-h100x4-hsb-npc-training] experiment=${{EXPERIMENT}} initial_bs=${{INITIAL_BS}}"
echo "[self-forced-h100x4-hsb-npc-training] lr=${{CATK_LR}} estimator_warmup=${{ESTIMATOR_WARMUP_EPOCHS}} self_forced_use_stop_motion=${{SELF_FORCED_USE_STOP_MOTION}}"
echo "[self-forced-h100x4-hsb-npc-training] pretrain_artifact=${{WANDB_PRETRAIN_ARTIFACT}}"
echo "[self-forced-h100x4-hsb-npc-training] pretrain_ckpt=${{PRETRAIN_CKPT}}"
echo "[self-forced-h100x4-hsb-npc-training] attach survives after exit; press Ctrl-b d to detach"
echo

ensure_pretrain_checkpoint() {{
  if [[ -f "$PRETRAIN_CKPT" ]]; then
    echo "[self-forced-h100x4-hsb-npc-training] using cached pretrain checkpoint: $PRETRAIN_CKPT"
    return 0
  fi
  ...
  if mkdir "$lock_dir" 2>/dev/null; then
    echo "[self-forced-h100x4-hsb-npc-training] downloading W&B artifact: $WANDB_PRETRAIN_ARTIFACT"
    python - <<'PY'
import glob
import os
import shutil
...
PY
    ...
    rm -rf "$lock_dir"
    if (( status != 0 )); then
      echo "[self-forced-h100x4-hsb-npc-training] W&B artifact download failed with status $status" >&2
      return "$status"
    fi
  else
    echo "[self-forced-h100x4-hsb-npc-training] waiting for checkpoint download lock: $lock_dir"
    for _ in $(seq 1 180); do
      if [[ -f "$PRETRAIN_CKPT" ]]; then
        echo "[self-forced-h100x4-hsb-npc-training] checkpoint appeared: $PRETRAIN_CKPT"
        return 0
      fi
      sleep 10
    done
    echo "[self-forced-h100x4-hsb-npc-training] timed out waiting for $PRETRAIN_CKPT" >&2
    return 4
  fi
...
status=$?
if (( status != 0 )); then
  echo "[self-forced-h100x4-hsb-npc-training] checkpoint preparation failed with status $status"
  echo "[self-forced-h100x4-hsb-npc-training] leaving shell open for inspection"
  exec bash
fi
...
echo
echo "[self-forced-h100x4-hsb-npc-training] exited with status $status at $(date '+%F %T')"
echo "[self-forced-h100x4-hsb-npc-training] leaving shell open for inspection"
exec bash
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Launch H100x4 self-forced training on the existing hsb-npc-training pod.",
    )
    ...
    parser.add_argument("--estimator-warmup-epochs", default="0")
    ...
    if args.self_forced_use_stop_motion != "false":
        parser.error("--self-forced-use-stop-motion is globally disabled; use 'false'")
    if args.decoder_use_stop_motion not in {"", "false"}:
        parser.error("--decoder-use-stop-motion is globally disabled; leave empty or use 'false'")
