#!/usr/bin/env python3
"""Launch the testa/testaa A100x4x2 see-all-false injection-ramp gradually-see run."""
...
def split_wrapper_args(argv: list[str]) -> tuple[list[str], str, int, bool, int]:
    ...
    variant_overrides = " ".join(
        [
            ...
            "model.model_config.self_forced.closed_loop_see_all=false",
            "model.model_config.self_forced.gradually_see=true",
            "model.model_config.self_forced.dmd_use_injection_ramp=true",
        ]
    )
...
def main() -> int:
    ...
    if skip_initial_stage_checkpoint:
        ...
        checkpoint_path = (
            "/workspace/flow_closed_loop_self_forced_a100x4x2_testa_testaa_pretrain_epoch061_x5f9g0ce/"
            "v57/epoch_061.ckpt"
        )
    ...
    task_name = (
        f"flow_closed_loop_self_forced_dmd_{pod_label}_sfanchor_seeallfalse_gradual_injramp_"
    ...
    args = [
        ...
        task_name,
        "--session",
        f"catk-clsf-a100x4x2-testa-testaa-gradual-injramp-seeallfalse-stride{rollout_anchor_stride}",
        "--initial-bs",
        "24",
        "--oom-step",
        "4",
