...
``semi_control_stable_w_val`` run does not accidentally resume or mix with the older
W&B run:
...
DEFAULT_METADATA_CACHE_BASENAME = (
    "womd_training_validation_memory_balance_v1_stable_latest.pt"
)
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(
        description=(
            "Launch the latest semi_control_stable_w_val V100x47 tail-prefix Flow "
