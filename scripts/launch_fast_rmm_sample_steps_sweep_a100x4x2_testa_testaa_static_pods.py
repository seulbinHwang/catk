#!/usr/bin/env python3
"""Launch a Fast-RMM flow sample-step sweep on existing testa/testaa A100 4+4 pods.
...
inside each closed-loop rollout changes. ``n_rollout_closed_val`` stays fixed;
it controls the number of rollout samples per scene, not the denoising depth.
"""
...
DEFAULT_SAMPLE_STEPS = "4,8,12,16,20,24,32,40,48,56,64"
DEFAULT_SESSION = "fast-rmm-sample-steps-sweep-a100x4x2-testa-testaa"
...
def render_worker_script(
    *,
    ...
    tags = (
        f"[fast_rmm,sample_steps_sweep,a100x4x2,{args.branch},"
...
cd "$PROJECT_ROOT"
echo "[$(date '+%F %T')] sweep start pod={layout.pod} epoch={args.epoch} sample_steps={','.join(str(s) for s in args.sample_steps)} val_batch_size=$VAL_BATCH_SIZE n_rollout_closed_val=$N_ROLLOUT_CLOSED_VAL" | tee -a "$STATUS_FILE"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "model.model_config.validation_rollout_sampling.sample_steps on "
    ...
    parser.add_argument(
        ...
        help="Comma-separated Flow denoising sample_steps sweep, e.g. 4,8,16,32.",
    )
