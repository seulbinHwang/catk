...
FALLBACK_VARIANTS: tuple[Variant, ...] = (
    Variant(
        name="ocsc_cleanmatch_lr1e6_noshuffle_trainselect",
        note="Closest OCSC-clean OL2hz M8 recipe: lr1e-6, no shuffle, train target builder, fp32, no clip.",
        env={
            "LR": "1.0e-6",
            "DATA_SHUFFLE": "false",
            "TRAIN_USE_EVAL_AGENT_SELECTION": "false",
            "TRAIN_EPOCH_SAMPLE_FRACTION": "1.0",
            "PRECISION": "32-true",
            "GRADIENT_CLIP_VAL": "0",
            "NUM_WORKERS": "12",
            "PREFETCH_FACTOR": "4",
    ...
    Variant(
        name="ocsc_cleanmatch_lr2e6_noshuffle_trainselect",
        note="Same clean-match data/training knobs but keep current lr2e-6.",
        env={
            "LR": "2.0e-6",
            "DATA_SHUFFLE": "false",
            "TRAIN_USE_EVAL_AGENT_SELECTION": "false",
    ...
    Variant(
        name="ocsc_cleanmatch_m16_lr1e6",
        note="Increase OL target pool M to 16 while keeping the clean-match recipe.",
        env={
            "LR": "1.0e-6",
