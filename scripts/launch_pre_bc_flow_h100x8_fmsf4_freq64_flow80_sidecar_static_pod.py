#!/usr/bin/env python3
"""Launch an H100x8 freq64/flow80 sidecar Flow pretrain on fm-sf-4.
...
DEFAULT_TASK_NAME = (
    "flow_open_loop_pretrain_freq64_flow80_sidecar_metricoff_"
    "h100x8_fmsf4_bs18_lr6p5e-4_warm5_val8_membal"
)
DEFAULT_SESSION = "catk-pretrain-freq64-flow80-sidecar-h100x8-fmsf4"
...
def run_root(args: argparse.Namespace) -> str:
    ...
    return f"{args.log_dir.rstrip('/')}/tmux_pre_bc_flow_h100x8_fmsf4/{safe_task}"
...
RUN_ROOT="${{CATK_LOG_DIR%/}}/tmux_pre_bc_flow_h100x8_fmsf4/${{TASK_NAME//\\//_}}"
mkdir -p "$RUN_ROOT"

echo "[pretrain-h100x8-fmsf4] pod=$(hostname) task=${{TASK_NAME}}"
echo "[pretrain-h100x8-fmsf4] started at $(date '+%F %T')"
echo "[pretrain-h100x8-fmsf4] git=$(git rev-parse --short HEAD 2>/dev/null || true)"
echo "[pretrain-h100x8-fmsf4] experiment=${{EXPERIMENT}} initial_bs=${{INITIAL_BS}} oom_step=${{OOM_STEP}} min_bs=${{MIN_BS}}"
echo "[pretrain-h100x8-fmsf4] nproc=${{NPROC_PER_NODE}} cache=${{CACHE_ROOT}}"
echo "[pretrain-h100x8-fmsf4] run_root=${{RUN_ROOT}}"
echo "[pretrain-h100x8-fmsf4] attach survives after exit; press Ctrl-b d to detach"
echo

if [[ "${{SIDECAR_PREBUILD}}" == "true" ]]; then
  echo "[pretrain-h100x8-fmsf4] prebuilding flow target sidecars dir=${{FLOW_TARGET_SIDECAR_DIR}} max_samples=${{SIDECAR_PREBUILD_MAX_SAMPLES}} nproc=${{NPROC_PER_NODE}}"
  ...
  echo "[pretrain-h100x8-fmsf4] sidecar prebuild exited with status $sidecar_status log=$sidecar_log"
  if (( sidecar_status != 0 )); then
    echo "[pretrain-h100x8-fmsf4] sidecar prebuild failed; aborting before training"
    exec bash
  fi
fi
...
PY
  ...
  echo "[pretrain-h100x8-fmsf4] dataloader sidecar root=${{FLOW_TARGET_SIDECAR_ROOT}}"
fi
...
terminate_task_processes() {{
  ...
  echo "[pretrain-h100x8-fmsf4] terminating task processes for $reason: ${{pids[*]}}"
  kill -TERM "${{pids[@]}}" 2>/dev/null || true
  sleep 15
  mapfile -t pids < <(task_process_pids || true)
  if (( ${{#pids[@]}} > 0 )); then
    echo "[pretrain-h100x8-fmsf4] force killing task processes for $reason: ${{pids[*]}}"
...
while (( bs >= MIN_BS )); do
  ...
  if [[ -n "$latest_ckpt" ]]; then
    echo "[pretrain-h100x8-fmsf4] attempt #${{attempt}} bs=${{bs}} resume=${{latest_ckpt}}"
    ckpt_override="ckpt_path=${{latest_ckpt}}"
  else
    echo "[pretrain-h100x8-fmsf4] attempt #${{attempt}} bs=${{bs}} fresh fit"
    ckpt_override="ckpt_path=null"
  fi
  echo "[pretrain-h100x8-fmsf4] attempt log: $attempt_log"
  ...
  printf '[pretrain-h100x8-fmsf4] torchrun command:'
  ...
  echo "[pretrain-h100x8-fmsf4] attempt #${{attempt}} exited with status $exit_code"
  ...
  if grep -Eq "$OOM_REGEX" "$attempt_log"; then
    non_oom_retry_count=0
    new_bs=$(( bs - OOM_STEP ))
    echo "[pretrain-h100x8-fmsf4] OOM detected at bs=${{bs}}; lowering to bs=${{new_bs}}"
    if (( new_bs < MIN_BS )); then
      echo "[pretrain-h100x8-fmsf4] next bs=${{new_bs}} is below MIN_BS=${{MIN_BS}}; aborting"
      final_status=1
      break
    fi
    bs="$new_bs"
  ...
  if is_retryable_non_oom_exit "$exit_code" && (( non_oom_retry_count < MAX_NON_OOM_RETRIES )); then
    ...
    echo "[pretrain-h100x8-fmsf4] retryable non-OOM exit=${{exit_code}}; retrying bs=${{bs}} (${{non_oom_retry_count}}/${{MAX_NON_OOM_RETRIES}})"
    continue
  fi

  echo "[pretrain-h100x8-fmsf4] non-OOM failure; see $attempt_log"
  final_status="$exit_code"
  break
done

echo
echo "[pretrain-h100x8-fmsf4] finished with status $final_status at $(date '+%F %T')"
echo "[pretrain-h100x8-fmsf4] leaving shell open for inspection"
exec bash
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Launch fm-sf-4 H100x8 open-loop pretrain with sidecars for freq64 + flow80."
    )
