#!/usr/bin/env python3
"""Launch H100 4+2 prefix-valid default no-slip pose-space Flow pretrain.

This preset mirrors
``launch_pre_bc_flow_control_h100x4_h100x2_prefix_default_noslip_static_pods.py``
but forces ``model.model_config.token_processor.use_kinematic_control_flow=false``.
...
DEFAULT_TASK_NAME = (
    "flow_pose_space_pretrain_h100x4_h100x2_prefix_default_noslip_"
    "lr6e-4_bs20"
)
DEFAULT_SESSION = "catk-pose-pretrain-h100x4-h100x2-prefix-default-noslip"
...
def launcher_env() -> dict[str, str]:
    env = os.environ.copy()
    shim_dir = Path(tempfile.gettempdir()) / "catk_launcher_python_shim"
    shim_dir.mkdir(parents=True, exist_ok=True)
    python_shim = shim_dir / "python"
    python_shim.write_text(
        f"#!/usr/bin/env bash\nexec {shq(sys.executable)} \"$@\"\n",
        encoding="utf-8",
    )
...
def run_stop(args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        ...
        return 0
    return subprocess.call(command, env=launcher_env())


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Launch semi_control_pose H100x4 + H100x2 prefix-valid default "
            "no-slip pose-space Flow pretrain."
        )
    )
    ...
    parser.add_argument("--branch", default=os.environ.get("CATK_BRANCH") or "semi_control_pose")
    parser.add_argument(
        "--git-ref",
