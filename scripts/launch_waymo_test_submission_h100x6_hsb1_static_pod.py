#!/usr/bin/env python3
"""Launch a guarded H100x6 Waymo test-set submission export on hsb-npc-training-1.

The default mode is safe: it generates and verifies a test-set submission
archive but does not upload to the Waymo test leaderboard. Passing
``--submit-test`` explicitly arms the final upload.
...
def render_remote_script(args: argparse.Namespace) -> str:
    # `--verify-waymo-ui` must arm the test submission code path so that the
    # Waymo test upload form is located, but `dry_run=true` returns before
    # attaching the archive or clicking submit.
    submit_test = "true" if (args.submit_test or args.verify_waymo_ui) else "false"
    waymo_enabled = "true" if (args.submit_test or args.verify_waymo_ui) else "false"
    waymo_dry_run = "true" if args.verify_waymo_ui and not args.submit_test else "false"
    limit_test_batches = "1" if args.smoke_test else str(args.limit_test_batches)
    test_batch_size = (
        args.smoke_test_batch_size if args.smoke_test else args.test_batch_size
    )
    expected_test_scenarios = (
        ""
        if args.smoke_test or args.expected_test_scenarios is None
        else str(int(args.expected_test_scenarios))
    )
    ...
    hydra_overrides = [
        ...
        "logger.wandb.job_type=waymo_test_submission",
        ...
        "waymo_submission.submit_validate=false",
        f"waymo_submission.submit_test={submit_test}",
        "waymo_submission.evaluation_set=test",
    ...
    download_python = textwrap.dedent(
        ...
        run = wandb.init(
            ...
            job_type="download_waymo_test_checkpoint",
            mode="online",
        )
    ...
    script = strip_template_indent(
        ...
        LIMIT_TEST_BATCHES={shq(limit_test_batches)}
        ...
        EXPECTED_TEST_SCENARIOS={shq(expected_test_scenarios)}

        cd "$PROJECT_ROOT"
        mkdir -p "$RUN_DIR" "$CKPT_DIR" "$(dirname "$STATUS_FILE")"

        echo "[$(date '+%F %T')] start waymo test submission export"
        echo "  branch=$(git rev-parse --abbrev-ref HEAD) commit=$(git rev-parse --short HEAD)"
        echo "  artifact=$CHECKPOINT_ARTIFACT"
        echo "  run_dir=$RUN_DIR"
        echo "  submit_test={submit_test} waymo_enabled={waymo_enabled} waymo_dry_run={waymo_dry_run}"
        echo "  smoke_test={str(args.smoke_test).lower()} limit_test_batches=$LIMIT_TEST_BATCHES test_batch_size=$TEST_BATCH_SIZE"

        if [[ ! -d "$CACHE_ROOT/testing" ]]; then
          echo "ERROR: missing test cache directory: $CACHE_ROOT/testing" >&2
          exit 2
        fi
        if [[ "{str(args.smoke_test).lower()}" != "true" && -z "$EXPECTED_TEST_SCENARIOS" ]]; then
          EXPECTED_TEST_SCENARIOS=$(find "$CACHE_ROOT/testing" -type f -name '*.pkl' | wc -l | tr -d ' ')
          echo "  auto expected_test_scenarios=$EXPECTED_TEST_SCENARIOS"
        fi
        ...
        if [[ -n "$EXPECTED_TEST_SCENARIOS" ]]; then
          VERIFY_SCENARIOS_ARG=(--expected-scenarios "$EXPECTED_TEST_SCENARIOS")
        fi
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--expected-test-scenarios", type=int, default=None)
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "After archive generation, open the Waymo test submission UI and "
    ...
    if args.submit_test and args.smoke_test:
        parser.error("--submit-test is not allowed together with --smoke-test")
    if args.submit_test and args.verify_waymo_ui:
        parser.error("--submit-test and --verify-waymo-ui are mutually exclusive")
    if args.test_batch_size < 1 or args.smoke_test_batch_size < 1:
