#!/usr/bin/env python3
"""Launch an H100x8 open-loop Flow pretrain on the existing fm-sf-3 pod.
...
def run_root(args: argparse.Namespace) -> str:
    ...
    return f"{args.log_dir.rstrip('/')}/tmux_pre_bc_flow_h100x8_fmsf3/{safe_task}"
...
echo "[pretrain-h100x8-fmsf3] pod=$(hostname) task=${{TASK_NAME}}"
echo "[pretrain-h100x8-fmsf3] started at $(date '+%F %T')"
echo "[pretrain-h100x8-fmsf3] git=$(git rev-parse --short HEAD 2>/dev/null || true)"
echo "[pretrain-h100x8-fmsf3] experiment=${{EXPERIMENT}} initial_bs=${{INITIAL_BS}} oom_step=${{OOM_STEP}} min_bs=${{MIN_BS}}"
echo "[pretrain-h100x8-fmsf3] nproc=${{NPROC_PER_NODE}} cache=${{CACHE_ROOT}}"
echo "[pretrain-h100x8-fmsf3] run_root=${{RUN_ROOT}}"
echo "[pretrain-h100x8-fmsf3] attach survives after exit; press Ctrl-b d to detach"
echo
...
terminate_task_processes() {{
  ...
  echo "[pretrain-h100x8-fmsf3] terminating task processes for $reason: ${{pids[*]}}"
  kill -TERM "${{pids[@]}}" 2>/dev/null || true
  sleep 15
  mapfile -t pids < <(task_process_pids || true)
  if (( ${{#pids[@]}} > 0 )); then
    echo "[pretrain-h100x8-fmsf3] force killing task processes for $reason: ${{pids[*]}}"
...
while (( bs >= MIN_BS )); do
  ...
  echo "[pretrain-h100x8-fmsf3] attempt log: $attempt_log"
  ...
  printf '[pretrain-h100x8-fmsf3] torchrun command:'
  ...
  echo "[pretrain-h100x8-fmsf3] attempt #${{attempt}} exited with status $exit_code"
  ...
  if grep -Eq "$OOM_REGEX" "$attempt_log"; then
    non_oom_retry_count=0
    new_bs=$(( bs - OOM_STEP ))
    echo "[pretrain-h100x8-fmsf3] OOM detected at bs=${{bs}}; lowering to bs=${{new_bs}}"
    if (( new_bs < MIN_BS )); then
      echo "[pretrain-h100x8-fmsf3] next bs=${{new_bs}} is below MIN_BS=${{MIN_BS}}; aborting"
      final_status=1
      break
    fi
    bs="$new_bs"
  ...
  echo "[pretrain-h100x8-fmsf3] non-OOM failure; see $attempt_log"
  final_status="$exit_code"
  break
done

echo
echo "[pretrain-h100x8-fmsf3] finished with status $final_status at $(date '+%F %T')"
echo "[pretrain-h100x8-fmsf3] leaving shell open for inspection"
exec bash
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Launch fm-sf-3 H100x8 open-loop pretrain for a343315f GELU test."
    )
