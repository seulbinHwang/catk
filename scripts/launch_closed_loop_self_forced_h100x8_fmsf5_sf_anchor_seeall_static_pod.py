#!/usr/bin/env python3
"""Launch stride-1 see-all multi-anchor closed-loop DMD self-forcing on fm-sf-5 H100x8."""
...
def split_wrapper_args(argv: list[str]) -> tuple[list[str], str]:
    ...
    parser.add_argument("--closed-loop-see-all", action=argparse.BooleanOptionalAction, default=True)
...
def main() -> int:
    ...
    args = [
        ...
        "--pretrain-ckpt",
        (
            "/workspace/flow_closed_loop_self_forced_h100x8_fmsf5_pretrain_epoch061_x5f9g0ce/"
            "v57/epoch_061.ckpt"
        ),
