...
## testsv + svvv V100x4x2 실행

`testsv`, `svvv`는 각각 V100 32GB 4장이라 H100 preset을 그대로 쓰지 않습니다. 전용 preset은
`self_forced_gan_v100x4x2_testsv_svvv`이고, 핵심 차이는 다음입니다.
...
| D/G fake rollout | V100에서는 D용 no-grad fake와 G용 grad fake를 분리 생성 |
| validation rollout | 8 |

아래 두 메모리 안정화는 `testsv + svvv` V100 멀티노드 환경에서도
...
map sender token도 한 번에 전부 보지 않고 `512`개 단위 streaming softmax로 처리합니다.
...
V100 32GB 안으로 낮춥니다.
D update 중에는 G update용 fake rollout graph를 들고 있지 않도록,
V100 preset은 D용 fake rollout을 no-grad로 먼저 만들고 G update 직전에 fake rollout을
다시 샘플링합니다.

| 항목 | testsv + svvv 특화 대응 |
|---|---|
...
먼저 teacher cache가 없으면 한 번 생성합니다. `testsv + svvv`에서는 8개 V100을 모두 cache
...
```bash
python scripts/launch_self_forced_gan_v100x4x2_testsv_svvv_static_pods.py \
  --build-teacher-cache \
  --build-cache-only \
  --parallel-teacher-cache \
  --sync-teacher-cache \
...
최신 측정은 `testsv + svvv`의 V100 8장, `batch_size=32`, `rollout_batch_size=32`,
...
python scripts/launch_self_forced_gan_v100x4x2_testsv_svvv_static_pods.py \
  ...
  --max-epochs 1 \
  --extra-hydra-overrides "data.shuffle=false data.train_epoch_sample_fraction=1.0" \
  --task-name sf_gan_k16_v100x4x2_testsv_svvv_smoke \
  --session catk-sf-gan-v100x4x2-testsv-svvv-smoke \
  --replace
```
...
```bash
python scripts/launch_self_forced_gan_v100x4x2_testsv_svvv_static_pods.py \
  --task-name sf_gan_k16_v100x4x2_testsv_svvv \
  --session catk-sf-gan-v100x4x2-testsv-svvv \
  --stop
```
