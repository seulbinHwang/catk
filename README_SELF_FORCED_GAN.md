...
## testsv + svv V100x4x2 실행

`testsv`, `svv`는 각각 V100 32GB 4장이라 H100 preset을 그대로 쓰지 않습니다. 전용 preset은
`self_forced_gan_v100x4x2_testsv_svv`이고, 핵심 차이는 다음입니다.
...
아래 두 메모리 안정화는 `testsv + svv` V100 멀티노드 환경에서도
...
먼저 teacher cache가 없으면 한 번 생성합니다. `testsv + svv`에서는 8개 V100을 모두 cache
...
```bash
python scripts/launch_self_forced_gan_v100x4x2_testsv_svv_static_pods.py \
  --build-teacher-cache \
  --build-cache-only \
  --parallel-teacher-cache \
  --sync-teacher-cache \
...
최신 측정은 `testsv + svv`의 V100 8장, `batch_size=32`, `rollout_batch_size=32`,
...
python scripts/launch_self_forced_gan_v100x4x2_testsv_svv_static_pods.py \
  ...
  --max-epochs 1 \
  --extra-hydra-overrides "data.shuffle=false data.train_epoch_sample_fraction=1.0" \
  --task-name sf_gan_k16_v100x4x2_testsv_svv_smoke \
  --session catk-sf-gan-v100x4x2-testsv-svv-smoke \
  --replace
```
...
```bash
python scripts/launch_self_forced_gan_v100x4x2_testsv_svv_static_pods.py \
  --task-name sf_gan_k16_v100x4x2_testsv_svv \
  --session catk-sf-gan-v100x4x2-testsv-svv \
  --stop
```
