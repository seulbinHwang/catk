...
# temp/ 복사본 — feature 추출 helper 자체는 length-agnostic 이므로 src/ 의
# 원본 모듈을 그대로 재사용해 정합성을 깨지 않는다 (상대 import → 절대 import).
from src.smart.metrics.wosac_metric_features_torch.types import MetricFeaturesTorch
from src.smart.metrics.wosac_metric_features_torch import trajectory_features_torch as traj
from src.smart.metrics.wosac_metric_features_torch import interaction_features_torch as inter
from src.smart.metrics.wosac_metric_features_torch import map_metric_features_torch as map_feat
from src.smart.metrics.wosac_metric_features_torch import traffic_light_features_torch as tl_feat
...
def scenario_to_joint_scene(
    ...
    *,
    ...
    ``n_steps_override`` 가 None 이면 기본 8초 (``cfg.n_simulation_steps``=80) 그대로,
    값이 주어지면 GT future 의 앞 ``n_steps_override`` step 만 사용한다 (예: 20=2초).
    """
    ...
    n_steps = int(n_steps_override) if n_steps_override is not None else int(cfg.n_simulation_steps)
    ...
    for oid in sim_ids:
        ...
        trajs.append(
            sim_agents_submission_pb2.SimulatedTrajectory(
                object_id=int(oid),
                center_x=[tr.states[ti].center_x for ti in range(t0, t0 + n_steps)],
                center_y=[tr.states[ti].center_y for ti in range(t0, t0 + n_steps)],
                center_z=[tr.states[ti].center_z for ti in range(t0, t0 + n_steps)],
                heading=[tr.states[ti].heading for ti in range(t0, t0 + n_steps)],
            )
        )
...
def joint_scene_to_trajectories(
    ...
    *,
    ...
    if use_log_validity:
        ...
        # 짧은 horizon (예: 2초=20) 인 경우 sim_x_t 의 시간축 길이만큼만 valid 추출.
        _T_pred = int(sim_x_t.shape[-1])
        logged_future = logged_full_aligned.slice_time(
            cfg.current_time_index + 1, cfg.current_time_index + 1 + _T_pred,
        )
...
def compute_metric_features(
    ...
    *,
    ...
    # ── 시간축 정렬 (short-horizon 대응) ─────────────────────────────────────
    # 원본 hard RMM 은 sim 이 항상 [history+80] 라고 가정하지만, 본 2초 변형은
    # sim 이 [history+T_pred] (T_pred=20) 일 수 있다.  ``logged_full`` 은
    # scenario 의 full 91 step 을 들고있으므로 sim 길이로 맞춰 슬라이스해야
    # ``compute_displacement_error`` (evaluated.x vs eval_logged.x) 가 깨지지
    # 않는다.  (8초 = 91 일 땐 no-op.)
    _t_sim = int(simulated.x.shape[1])
    _t_log = int(logged_full.x.shape[1])
    if _t_sim < _t_log:
        logged_full = logged_full.slice_time(0, _t_sim)
...
def compute_scenario_rollouts_features(
    ...
    *,
    ...
    ``n_steps_override`` 가 주어지면 GT log 의 future 도 그 길이로 자른다 — sim
    쪽 ``ScenarioRollouts.joint_scenes`` 의 각 ``SimulatedTrajectory.center_x``
    길이는 caller 가 같은 값으로 만들어 둔 다고 가정 (그래야 log/sim T 일치).
    """
    log_joint = scenario_to_joint_scene(
        scenario, challenge_type, n_steps_override=n_steps_override,
    )
