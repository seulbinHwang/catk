...
def _token_processor_stub(**overrides):
    values = {
        ...
        "control_no_slip_point_ratio": 0.25,
...
def test_decoder_config_uses_token_processor_as_single_source_of_truth() -> None:
    ...
    assert synced["control_no_slip_point_ratio"] == 0.25
...
def test_decoder_config_rejects_control_space_mismatch() -> None:
    decoder_config = {
        "hidden_dim": 128,
        "control_no_slip_point_ratio": 0.0,
    }

    with pytest.raises(ValueError, match="token_processor.control_no_slip_point_ratio"):
        _build_decoder_config_from_token_processor(
            decoder_config=decoder_config,
            token_processor=_token_processor_stub(control_no_slip_point_ratio=0.25),
        )
