...
def test_type_aware_normalize_and_denormalize_round_trip() -> None:
    ...
    agent_type = torch.tensor([VEHICLE_TYPE_ID, PEDESTRIAN_TYPE_ID, CYCLIST_TYPE_ID])
    control = torch.zeros((3, 1, 2), dtype=torch.float32)
    control[:, 0, 0] = torch.tensor([1.0, 1.0, 1.0])
    control[:, 0, 1] = torch.tensor(
        [
            CONTROL_YAW_SCALE_KWARGS["vehicle_yaw_scale_rad"],
            CONTROL_YAW_SCALE_KWARGS["pedestrian_yaw_scale_rad"],
            CONTROL_YAW_SCALE_KWARGS["cyclist_yaw_scale_rad"],
    ...
    torch.testing.assert_close(control_norm[:, 0, 1], torch.ones(3))
...
def test_agent_type_is_required_for_control_normalization() -> None:
    ...
    control = torch.zeros((1, 1, 2), dtype=torch.float32)
    control[..., 1] = CONTROL_YAW_SCALE_KWARGS["pedestrian_yaw_scale_rad"]
...
def test_control_norm_to_pose_norm_uses_type_aware_yaw_scale() -> None:
    ...
    agent_type = torch.tensor([VEHICLE_TYPE_ID, PEDESTRIAN_TYPE_ID, CYCLIST_TYPE_ID])
    control_norm = torch.zeros((3, 1, 2), dtype=torch.float32)
    control_norm[:, 0, 1] = 1.0
    ...
    expected_yaw = torch.tensor(
        [
            CONTROL_YAW_SCALE_KWARGS["vehicle_yaw_scale_rad"] * 0.1,
            CONTROL_YAW_SCALE_KWARGS["pedestrian_yaw_scale_rad"] * 0.1,
            CONTROL_YAW_SCALE_KWARGS["cyclist_yaw_scale_rad"] * 0.1,
        ],
        dtype=torch.float32,
    )
