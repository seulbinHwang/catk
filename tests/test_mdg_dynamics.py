...
def loop_reference_dynamics(dynamics, action, current_pos, current_heading, current_speed, current_velocity=None):
    ...
    for idx in range(int(action.shape[2])):
        for _ in range(dynamics.action_chunk):
            speed = torch.clamp(speed + acc[:, :, idx] * dynamics.dt, min=0.0)
            gated_yaw_rate = torch.where(
                speed > dynamics.YAW_RATE_SPEED_THRESHOLD_MPS,
                yaw_rate[:, :, idx],
                torch.zeros_like(yaw_rate[:, :, idx]),
            )
            heading = wrap_angle(heading + gated_yaw_rate * dynamics.dt)
...
def test_kinematic_dynamics_uses_vbd_update_order():
    ...
    torch.testing.assert_close(pos[0, 0, 0], torch.tensor([0.0, 2.0]), atol=1e-6, rtol=1e-6)
    torch.testing.assert_close(heading[0, 0, 0], torch.tensor(math.pi / 2.0))
    torch.testing.assert_close(speed[0, 0, 0], torch.tensor(2.0))


def test_kinematic_dynamics_clamps_speed_and_gates_low_speed_yaw_rate():
    dynamics = KinematicDynamics(
        action_chunk=1,
        dt=1.0,
        action_mean=(0.0, 0.0),
    ...
    pos, heading, speed, _, _ = dynamics(
        action=torch.tensor([[[[-10.0, math.pi]]]]),
        current_pos=torch.tensor([[[0.0, 0.0]]]),
        current_heading=torch.tensor([[0.5]]),
        current_speed=torch.tensor([[0.5]]),
    )

    torch.testing.assert_close(speed[0, 0, 0], torch.tensor(0.0))
    torch.testing.assert_close(heading[0, 0, 0], torch.tensor(0.5))
    torch.testing.assert_close(pos[0, 0, 0], torch.tensor([0.0, 0.0]))


def test_trajectory_to_actions_uses_logged_velocity_norm_speed():
    dynamics = KinematicDynamics(
        action_chunk=2,
        dt=1.0,
        action_mean=(0.0, 0.0),
    ...
    future_pos = torch.tensor([[[[20.0, 0.0], [50.0, 0.0], [90.0, 0.0], [140.0, 0.0]]]])
    future_velocity = torch.tensor([[[[2.0, 0.0], [3.0, 0.0], [4.0, 0.0], [5.0, 0.0]]]])
