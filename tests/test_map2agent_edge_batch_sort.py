...
def test_map2agent_edge_relation_uses_geometry_only_signature() -> None:
    signature = inspect.signature(SMARTAgentEncoder.build_map2agent_edge)
    assert "light_type" not in signature.parameters
    assert "light_time_delta_norm" not in signature.parameters
