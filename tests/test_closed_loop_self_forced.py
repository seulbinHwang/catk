...
def test_gradually_see_keeps_default_closed_loop_prefix_range(monkeypatch) -> None:
    model = _make_closed_loop_model()
    model.gradually_see = False
    model.closed_loop_see_all = False
    model.closed_loop_sf_global_max_step = 3
    ...
    assert requested_ranges == [(1, 5), (1, 5), (1, 5)]


def test_gradually_see_opens_multi_anchor_stage_local_window_over_epochs(monkeypatch) -> None:
    model = _make_closed_loop_model()
...
def test_gradually_see_all_opens_multi_anchor_cumulative_window_over_epochs(monkeypatch) -> None:
    model = _make_closed_loop_model()
    model.gradually_see = True
    model.closed_loop_see_all = True
    model.self_forced_estimator_warmup_epochs = 0
