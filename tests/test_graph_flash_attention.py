...
from src.smart.layers.graph_flash_attention import (
...
def test_graph_flash_attention_matches_manual_backward() -> None:
...
@pytest.mark.skipif(
    ...
    reason="CUDA and flash-attn are required for the fused kernel smoke test.",
)
def test_cuda_flash_varlen_kernel_matches_cpu_reference() -> None:
    q, k, v, edge_index, r, relation_key_weight, relation_value_weight, relation_value_bias = _make_inputs(dtype=torch.float32)
    scale = q.size(-1) ** -0.5
    expected = graph_flash_attention(
        q=q,
        k=k,
        v=v,
        edge_index=edge_index,
    ...
    actual_cuda = graph_flash_attention(
        q=q_cuda,
        k=k_cuda,
        v=v_cuda,
