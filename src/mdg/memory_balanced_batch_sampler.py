...
class MemoryBalanceMetadata:
    ...
    def from_samples(cls, samples: Iterable[SampleMemoryMetadata]) -> "MemoryBalanceMetadata":
        ...
        return cls(
            ...
            current_valid_agent_count=torch.as_tensor(current_valid_agent_count, dtype=torch.int64),
...
def _extract_sample_metadata(path: str) -> SampleMemoryMetadata:
    ...
    mdg_map = _get_storage(data, "mdg_map")
    map_valid = _get_field(mdg_map, "valid")
    map_position = _get_field(mdg_map, "position")
    if map_valid is not None:
        map_count = _sum_bool(map_valid)
    elif map_position is not None:
        map_count = _shape0(map_position)
    else:
        map_count = 0
...
def fingerprint_raw_paths(raw_paths: Sequence[str | Path]) -> str:
    digest = hashlib.sha1()
...
def _default_metadata_cache_path(raw_paths: Sequence[str | Path]) -> Path:
    raw_dir = Path(raw_paths[0]).parent if raw_paths else Path(".")
    filename = f"{raw_dir.name}_mdg_memory_balance_v{METADATA_VERSION}.pt"
...
def _start_lock_heartbeat(
    ...
    thread = threading.Thread(
        target=_heartbeat,
        name="mdg-memory-balance-metadata-lock-heartbeat",
        daemon=True,
    )
    thread.start()
    return thread
...
def _metadata_payload(metadata: MemoryBalanceMetadata, raw_paths: Sequence[str | Path]) -> dict[str, object]:
    return {
...
def _build_metadata(raw_paths: Sequence[str | Path], num_workers: int) -> MemoryBalanceMetadata:
    path_strings = [str(path) for path in raw_paths]
    if num_workers <= 1:
        return MemoryBalanceMetadata.from_samples(
            _extract_sample_metadata(path) for path in path_strings
        )
    with ProcessPoolExecutor(max_workers=int(num_workers), initializer=_init_metadata_worker) as executor:
        samples = executor.map(_extract_sample_metadata, path_strings, chunksize=128)
...
def _load_metadata_file(
    ...
    if payload.get("num_samples") != len(raw_paths):
        return None
    if payload.get("fingerprint") != fingerprint_raw_paths(raw_paths):
        return None
    try:
    ...
    if isinstance(metadata, MemoryBalanceMetadata):
        return (
            ...
            * metadata.current_valid_agent_count.to(torch.float64)
            + float(valid_agent_step_weight) * metadata.valid_agent_step_count.to(torch.float64)
...
class MemoryBalancedDistributedBatchSampler(Sampler[list[int]]):
    """Distributed batch sampler that spreads high-memory scenes across ranks."""
    ...
    def __init__(
        ...
        *,
        ...
        self.num_global_batches = int(math.ceil(self.dataset_size / self.global_batch_size))
    ...
    def __iter__(self) -> Iterator[list[int]]:
        ...
        for bin_idx in range(self.rank, stop, self.num_replicas):
