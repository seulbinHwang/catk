...
def compute_differentiable_scenario_metrics_for_bundle(
    config: sim_agents_metrics_pb2.SimAgentMetricsConfig,
    gt_scenario: dict,
    scenario_rollouts: dict,
    version: str = '2025',
    *,
    enabled_metric_fields: Sequence[str] | str | None = None,
    renormalize_metric_weights: bool = False,
    histogram_temperature: float = 1.0,
    soft_indicator_temperature: float = 0.25,
    eps: float = 1.0e-6,
) -> dict[str, torch.Tensor]:
    """Differentiable soft approximation of the fast Sim Agents RMM.

    The official fast scorer is intentionally hard and non-differentiable:
    histograms use ``torch.histc``, KDE goes through sklearn, and event metrics
    use boolean thresholds.  This function keeps the same feature pipeline and
    metric weights, but swaps likelihood estimators for torch-only soft versions.
    Collision/offroad event likelihoods are based on sigmoid relaxations of the
    continuous distance features.  Traffic-light events remain hard because the
    lane-state association is discrete.
    """
    enabled_fields = _normalize_enabled_metric_fields(enabled_metric_fields)
    log_features, sim_features, valid_masks = (
        metric_features.compute_scenario_rollouts_features(
            gt_scenario,
            scenario_rollouts,
            version,
            required_metric_fields=enabled_fields,
        )
    )
    zero = sim_features['linear_speed'].sum() * 0.0

    def enabled(field_name: str) -> bool:
        return enabled_fields is None or field_name in enabled_fields

    likelihood_metrics: dict[str, torch.Tensor] = {}
    if enabled('linear_speed'):
        linear_speed_log_likelihood = estimators.soft_log_likelihood_estimate_timeseries(
            feature_config=config.linear_speed,
            log_values=log_features['linear_speed'][0],
            sim_values=sim_features['linear_speed'],
        ...
        likelihood_metrics['linear_speed_likelihood'] = torch.exp(_reduce_average_with_validity_safe(
            linear_speed_log_likelihood, log_features['speed_validity'], eps=eps))
    if enabled('angular_speed'):
        angular_speed_log_likelihood = estimators.soft_log_likelihood_estimate_timeseries(
            feature_config=config.angular_speed,
            log_values=log_features['angular_speed'][0],
            sim_values=sim_features['angular_speed'],
            histogram_temperature=histogram_temperature,
        ...
        likelihood_metrics['angular_speed_likelihood'] = torch.exp(_reduce_average_with_validity_safe(
            angular_speed_log_likelihood, log_features['speed_validity'], eps=eps))

    if enabled('linear_acceleration'):
        linear_accel_log_likelihood = estimators.soft_log_likelihood_estimate_timeseries(
            feature_config=config.linear_acceleration,
            log_values=log_features['linear_acceleration'][0],
            sim_values=sim_features['linear_acceleration'],
            histogram_temperature=histogram_temperature,
        ...
        likelihood_metrics['linear_acceleration_likelihood'] = torch.exp(_reduce_average_with_validity_safe(
            linear_accel_log_likelihood, log_features['acceleration_validity'], eps=eps))
    if enabled('angular_acceleration'):
        angular_accel_log_likelihood = estimators.soft_log_likelihood_estimate_timeseries(
            feature_config=config.angular_acceleration,
            log_values=log_features['angular_acceleration'][0],
            sim_values=sim_features['angular_acceleration'],
            histogram_temperature=histogram_temperature,
        ...
        likelihood_metrics['angular_acceleration_likelihood'] = torch.exp(_reduce_average_with_validity_safe(
            angular_accel_log_likelihood, log_features['acceleration_validity'], eps=eps))

    if enabled('collision_indication') or enabled('distance_to_nearest_object'):
        soft_collision_per_step = torch.sigmoid(
            (interaction_features.COLLISION_DISTANCE_THRESHOLD - sim_features['distance_to_nearest_object'])
            / max(float(soft_indicator_temperature), eps)
        )
        soft_collision_per_step = torch.where(
            valid_masks.unsqueeze(0),
            soft_collision_per_step,
            torch.zeros_like(soft_collision_per_step),
        )
        sim_collision_indication = 1.0 - torch.prod(
            (1.0 - soft_collision_per_step).clamp(eps, 1.0),
            dim=2,
        )
        log_collision_indication = torch.any(
            torch.where(valid_masks, log_features['collision_per_step'],
            torch.zeros_like(log_features['collision_per_step'], dtype=torch.bool)), dim=2)
        if enabled('collision_indication'):
            collision_score = estimators.soft_log_likelihood_estimate_scenario_level(
                feature_config=config.collision_indication,
                log_values=log_collision_indication[0],
                sim_values=sim_collision_indication,
                histogram_temperature=histogram_temperature,
            ...
            likelihood_metrics['collision_indication_likelihood'] = torch.exp(torch.mean(collision_score))
        if enabled('distance_to_nearest_object'):
            distance_to_objects_log_likelihood = estimators.soft_log_likelihood_estimate_timeseries(
                feature_config=config.distance_to_nearest_object,
                log_values=log_features['distance_to_nearest_object'][0],
                sim_values=sim_features['distance_to_nearest_object'],
                histogram_temperature=histogram_temperature,
            ...
            likelihood_metrics['distance_to_nearest_object_likelihood'] = torch.exp(
                _reduce_average_with_validity_safe(distance_to_objects_log_likelihood, valid_masks, eps=eps)
            )

    if enabled('time_to_collision'):
        ttc_log_likelihood = estimators.soft_log_likelihood_estimate_timeseries(
            feature_config=config.time_to_collision,
            log_values=log_features['time_to_collision'][0],
            sim_values=sim_features['time_to_collision'],
            histogram_temperature=histogram_temperature,
        ...
        if version == '2025':
            ...
            ttc_validity = valid_masks
        likelihood_metrics['time_to_collision_likelihood'] = torch.exp(
            _reduce_average_with_validity_safe(ttc_log_likelihood, ttc_validity, eps=eps)
        )

    if enabled('offroad_indication') or enabled('distance_to_road_edge'):
        soft_offroad_per_step = torch.sigmoid(
            (sim_features['distance_to_road_edge'] - map_metric_features.OFFROAD_DISTANCE_THRESHOLD)
            / max(float(soft_indicator_temperature), eps)
        )
        soft_offroad_per_step = torch.where(
            valid_masks.unsqueeze(0),
            soft_offroad_per_step,
            torch.zeros_like(soft_offroad_per_step),
        )
        sim_offroad_indication = 1.0 - torch.prod(
            (1.0 - soft_offroad_per_step).clamp(eps, 1.0),
            dim=2,
        )
        log_offroad_indication = torch.any(
            torch.where(valid_masks, log_features['offroad_per_step'],
            torch.zeros_like(log_features['offroad_per_step'], dtype=torch.bool)),dim=2)
        if enabled('offroad_indication'):
            offroad_score = estimators.soft_log_likelihood_estimate_scenario_level(
                feature_config=config.offroad_indication,
                log_values=log_offroad_indication[0],
                sim_values=sim_offroad_indication,
                histogram_temperature=histogram_temperature,
            ...
            likelihood_metrics['offroad_indication_likelihood'] = torch.exp(torch.mean(offroad_score))
        if enabled('distance_to_road_edge'):
            distance_to_road_edge_log_likelihood = estimators.soft_log_likelihood_estimate_timeseries(
                feature_config=config.distance_to_road_edge,
                log_values=log_features['distance_to_road_edge'][0],
                sim_values=sim_features['distance_to_road_edge'],
                histogram_temperature=histogram_temperature,
            ...
            likelihood_metrics['distance_to_road_edge_likelihood'] = torch.exp(
                _reduce_average_with_validity_safe(distance_to_road_edge_log_likelihood, valid_masks, eps=eps)
            )

    if version == '2025' and enabled('traffic_light_violation'):
        if 'is_vehicle' not in locals():
            is_vehicle = torch.eq(log_features['object_type'], scenario_pb2.Track.ObjectType.TYPE_VEHICLE)
        tl_validity = torch.logical_and(valid_masks, is_vehicle.unsqueeze(1))
        log_traffic_light_violation_indication = torch.any(
            torch.logical_and(log_features['traffic_light_violation_per_step'], tl_validity.unsqueeze(0)),
            dim=2,
        )
        sim_traffic_light_violation_indication = torch.any(
            torch.logical_and(sim_features['traffic_light_violation_per_step'], tl_validity.unsqueeze(0)),
            dim=2,
        )
        tl_score = estimators.soft_log_likelihood_estimate_scenario_level(
            feature_config=config.traffic_light_violation,
            log_values=log_traffic_light_violation_indication[0],
            sim_values=sim_traffic_light_violation_indication,
            histogram_temperature=histogram_temperature,
        ...
        likelihood_metrics['traffic_light_violation_likelihood'] = torch.exp(torch.mean(tl_score))

    metametric = _compute_differentiable_metametric(
        config=config,
        likelihood_metrics=likelihood_metrics,
        enabled_fields=enabled_fields,
        renormalize_metric_weights=renormalize_metric_weights,
        zero=zero,
    )
    average_displacement_error = torch.mean(sim_features['average_displacement_error'])
    min_average_displacement_error = torch.min(
        torch.mean(sim_features['average_displacement_error'], dim=1))
    likelihood_metrics.update({
        'metametric': metametric,
...
def _reduce_average_with_validity_safe(
        tensor: torch.Tensor,
        validity: torch.Tensor,
        *,
        eps: float = 1.0e-6,
) -> torch.Tensor:
    """Differentiable reduce that stays finite when short horizons have no support."""
    if tensor.shape != validity.shape:
        raise ValueError('Shapes of `tensor` and `validity` must be the same.'
                                         f'(Actual: {tensor.shape}, {validity.shape}).')
    validity_bool = validity.to(device=tensor.device, dtype=torch.bool)
    validity_float = validity_bool.to(dtype=tensor.dtype)
    cond_sum = torch.sum(torch.where(validity_bool, tensor, torch.zeros_like(tensor)))
    valid_sum = torch.sum(validity_float)
    return cond_sum / valid_sum.clamp_min(eps)


def _normalize_enabled_metric_fields(
    enabled_metric_fields: Sequence[str] | str | None,
) -> set[str] | None:
    if enabled_metric_fields is None:
        return None
    if isinstance(enabled_metric_fields, str):
        if enabled_metric_fields.strip().lower() in {"", "all", "*"}:
            return None
        raw_fields = [enabled_metric_fields]
    else:
        raw_fields = list(enabled_metric_fields)
        if any(str(field).strip().lower() in {"all", "*"} for field in raw_fields):
            return None
    ...
    for field in raw_fields:
        ...
        if name.endswith("_likelihood"):
            name = name[: -len("_likelihood")]
        normalized.add(name)
    unknown_fields = sorted(normalized - set(_METRIC_FIELD_NAMES))
    if unknown_fields:
        raise ValueError(f"Unknown Sim Agents metric fields: {unknown_fields}.")
    return normalized


def _compute_differentiable_metametric(
        *,
        config: sim_agents_metrics_pb2.SimAgentMetricsConfig,
        likelihood_metrics: dict[str, torch.Tensor],
        enabled_fields: set[str] | None,
        renormalize_metric_weights: bool,
        zero: torch.Tensor,
) -> torch.Tensor:
    metametric = zero
    weight_sum = 0.0
    for field_name in _METRIC_FIELD_NAMES:
        if enabled_fields is not None and field_name not in enabled_fields:
            continue
        likelihood_field_name = field_name + '_likelihood'
        if likelihood_field_name not in likelihood_metrics:
            continue
        weight = float(getattr(config, field_name).metametric_weight)
        metametric = metametric + weight * likelihood_metrics[likelihood_field_name]
        weight_sum += weight
    if renormalize_metric_weights and weight_sum > 0.0:
        metametric = metametric / weight_sum
    return metametric
