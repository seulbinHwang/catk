...
"""HardSimAgentsMetrics: pure-PyTorch hard RMM 계산기.

OCSC_clean 측 src/smart/metrics/__init__.py 에서 `HardSimAgentsMetrics`,
`_hard_load_and_log_feat_worker`, `_LIKELIHOOD_NAMES` 만 분리해서 옮긴 파일입니다.
나머지 `SimAgentsMetrics` / `SimAgentsSubmission` / worker 들은
project_3 측 `sim_agents_metrics.py` / `sim_agents_submission.py` 를 그대로 사용합니다.
"""
