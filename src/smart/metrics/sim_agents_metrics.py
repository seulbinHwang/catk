...
from src.smart.metrics.sim_agents_fast_eval_tool.fast_sim_agents_metrics import (
    metrics as fast_sim_agents_metrics,
)
from src.smart.metrics.sim_agents_fast_eval_tool.scenario_gt_converter import (
    extract_gt_scenario,
)
...
_FAST_SIM_AGENTS_2025_CONFIG_PATH = (
    Path(__file__).resolve().parent
    / "sim_agents_fast_eval_tool"
...
def _load_waymo_sim_agents_2025_config(
    ...
    if _SIM_AGENTS_2025_CHALLENGE_TYPE is None:
        raise RuntimeError(
            ...
            f"현재 버전={_get_waymo_version_string()}. "
            "legacy Sim Agents 평가는 허용되지 않으며, README 기준으로 "
            "waymo-open-dataset-tf-2-12-0==1.6.7 이상이 필요합니다."
        )
    if not _FAST_SIM_AGENTS_2025_CONFIG_PATH.exists():
        raise RuntimeError(f"Fast Sim Agents 2025 config 파일을 찾지 못했습니다: {_FAST_SIM_AGENTS_2025_CONFIG_PATH}")

    config = sim_agents_metrics_pb2.SimAgentMetricsConfig()
    with _FAST_SIM_AGENTS_2025_CONFIG_PATH.open("r", encoding="utf-8") as handle:
    ...
    if traffic_light_weight <= 0.0:
        raise RuntimeError(
            "Fast Sim Agents 2025 설정을 읽었지만 traffic_light_violation 가중치가 0입니다. "
...
class SimAgentsMetrics(Metric):
    """TrajTok Fast Sim Agents 2025 evaluator wrapped as a torchmetrics Metric."""
