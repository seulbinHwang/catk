...
def _as_float_tensor(value: Tensor, *, device: torch.device) -> Tensor:
    """입력 텐서를 계산용 실수 텐서로 바꿉니다.

    Args:
        value: 변환할 텐서입니다. shape은 입력 그대로 유지됩니다.
        device: 결과 텐서를 둘 장치입니다.

    Returns:
        Tensor: ``float32`` 타입 텐서입니다. shape은 입력과 같습니다.
    """
...
def _as_bool_tensor(value: Tensor, *, device: torch.device) -> Tensor:
    ...
    Returns:
        Tensor: ``bool`` 타입 텐서입니다. shape은 입력과 같습니다.
    """
...
def _as_long_tensor(value: Tensor, *, device: torch.device) -> Tensor:
    ...
    Returns:
        Tensor: ``long`` 타입 텐서입니다. shape은 입력과 같습니다.
    """
...
def _safe_cat_state(
    ...
    *,
    ...
    Args:
        value: 텐서이거나 텐서 목록입니다. 각 텐서는 첫 번째 차원 기준으로 합칩니다.
        device: 결과 텐서를 둘 장치입니다.
...
def _trim_prediction_and_gt(
    ...
    Returns:
        tuple[Tensor, Optional[Tensor], Optional[Tensor]]: 미래 길이를 맞춘 예측,
        정답, valid mask입니다.
    """
...
def _compute_pair_square_summaries(
    ...
    *,
    ...
    """같은 scenario 안 rollout 쌍별 거리 제곱합을 계산합니다.
...
class WOSACDistributionMetrics(Metric):
    ...
    def update(
        ...
        """한 batch의 closed-loop rollout을 누적합니다.
        ...
        current_pos = _as_float_tensor(current_pos, device=device) if current_pos is not None else None
        gt_traj = _as_float_tensor(gt_traj, device=device) if gt_traj is not None else None
        gt_valid_mask = _as_bool_tensor(gt_valid_mask, device=device) if gt_valid_mask is not None else None
        agent_valid_mask = (
        ...
        pair_sq_chunks: list[Tensor] = []
        pair_count_chunks: list[Tensor] = []
        gt_sq_chunks: list[Tensor] = []
        gt_count_chunks: list[Tensor] = []
    ...
    def _update_type_scale(
        ...
        for type_index in range(self.num_agent_types):
            ...
            if bool(type_valid.any()):
                self.scale_sq_sum[type_index] += square_distance[type_valid].sum(dtype=torch.float64)
    ...
    def _parse_fixed_type_scale(
        ...
        Args:
            type_scale: agent 종류별 positive finite scale입니다.

        Returns:
            Optional[Tensor]: 고정 scale 텐서입니다. 값이 없으면 ``None`` 입니다.
        """
    ...
    def compute(self) -> Dict[str, Tensor]:
        ...
        scenario_pair_sum = torch.where(pair_valid, pair_distance, torch.zeros_like(pair_distance)).sum(dim=1)
    ...
    def _compute_ces(
        ...
        pair_sum = torch.where(pair_valid, pair_distance, torch.zeros_like(pair_distance)).sum(dim=1)
...
def update_wosac_distribution_metric_from_model(
    ...
    """LightningModule의 history 길이를 사용해 WOSAC 분포 metric을 갱신합니다.

    Args:
        metric: 갱신할 WOSAC 분포 metric입니다.
        model: ``num_historical_steps`` 값을 가진 LightningModule입니다.
        data: validation 또는 test batch입니다.
        pred_traj: 모델이 만든 closed-loop 미래 위치입니다.
            shape은 ``[n_agent, n_rollout, n_step, 2]`` 입니다.
        include_gt: ``True``이면 validation GT로 CES와 type scale을 계산합니다.
...
def log_and_reset_wosac_distribution_metric(
    ...
    """WOSAC 분포 metric을 계산하고 상태를 초기화합니다.

    Args:
        metric: 계산 후 초기화할 WOSAC 분포 metric입니다.

    Returns:
        Dict[str, Tensor]: 계산된 metric 사전입니다. 저장된 rollout이 없으면 빈 사전입니다.
    """
