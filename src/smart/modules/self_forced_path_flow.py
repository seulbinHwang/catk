...
def build_anchor0_prev_control(
    tokenized_agent: Dict[str, Tensor],
    anchor_mask: Tensor,
) -> tuple[Tensor, Tensor]:
    """physics loss가 쓸 첫 anchor 직전 제어값을 만듭니다.

    Args:
        tokenized_agent: 평가 모드 토큰 사전입니다. 가능하면 ``gt_pos_raw``,
            ``gt_head_raw``, ``gt_valid_raw`` 를 사용합니다.
        anchor_mask: self-forced 학습에 쓰는 agent 마스크입니다. shape은 ``[n_agent]`` 입니다.

    Returns:
        tuple[Tensor, Tensor]: 직전 제어값과 유효 마스크입니다. 직전 제어값의 shape은
            ``[n_valid_agent, 3]`` 이고 마지막 차원은 ``[v_x_local, v_y_local, yaw_rate]`` 입니다.
            유효 마스크 shape은 ``[n_valid_agent]`` 입니다.
    """
    num_valid = int(anchor_mask.sum().item())
    device = anchor_mask.device
    dtype = tokenized_agent["ctx_sampled_pos"].dtype
    if num_valid == 0:
        return torch.zeros((0, 3), device=device, dtype=dtype), torch.zeros(
            (0,), device=device, dtype=torch.bool
        )

    prev_control = torch.zeros((num_valid, 3), device=device, dtype=dtype)
    prev_valid = torch.zeros((num_valid,), device=device, dtype=torch.bool)
    required_keys = {"gt_pos_raw", "gt_head_raw", "gt_valid_raw", "ctx_sampled_pos", "ctx_sampled_heading"}
    if not required_keys.issubset(tokenized_agent.keys()):
        return prev_control, prev_valid

    raw_step = 10
    pos_raw = tokenized_agent["gt_pos_raw"]
    head_raw = tokenized_agent["gt_head_raw"]
    valid_raw = tokenized_agent["gt_valid_raw"]
    if pos_raw.shape[1] <= raw_step or head_raw.shape[1] <= raw_step or valid_raw.shape[1] <= raw_step:
        return prev_control, prev_valid

    current_pos = tokenized_agent["ctx_sampled_pos"][anchor_mask, 1]
    current_head = tokenized_agent["ctx_sampled_heading"][anchor_mask, 1]
    pos_pair = pos_raw[anchor_mask, raw_step - 1 : raw_step + 1, :2]
    head_pair = head_raw[anchor_mask, raw_step - 1 : raw_step + 1]
    prev_valid = valid_raw[anchor_mask, raw_step - 1] & valid_raw[anchor_mask, raw_step]
    ...
    delta_head = torch.atan2(
        (head_pair_local[:, 1] - head_pair_local[:, 0]).sin(),
        (head_pair_local[:, 1] - head_pair_local[:, 0]).cos(),
    )
    ...
    prev_control[~prev_valid] = 0.0
    return prev_control, prev_valid


def build_anchor0_agent_length(
    tokenized_agent: Dict[str, Tensor],
    anchor_mask: Tensor,
) -> Tensor:
    """첫 anchor에 해당하는 agent box length를 가져옵니다.

    Args:
        tokenized_agent: 평가 모드 토큰 사전입니다. ``shape`` 또는
            ``token_agent_shape`` 을 포함해야 합니다.
        anchor_mask: self-forced 학습에 쓰는 agent 마스크입니다. shape은 ``[n_agent]`` 입니다.

    Returns:
        Tensor: 선택된 agent의 길이입니다. shape은 ``[n_valid_agent]`` 입니다.
    """
    if "shape" in tokenized_agent:
        shape = tokenized_agent["shape"].to(dtype=torch.float32)
        if shape.dim() >= 2 and shape.shape[-1] >= 2:
            return shape[..., :2].amax(dim=-1)[anchor_mask]
    token_shape = tokenized_agent["token_agent_shape"].to(dtype=torch.float32)
    return token_shape[..., 1][anchor_mask]


def build_anchor0_physics_inputs(
    tokenized_agent: Dict[str, Tensor],
    anchor_mask: Tensor,
) -> Dict[str, Tensor]:
    """committed rollout physics loss에 필요한 agent별 메타데이터를 모읍니다.

    Args:
        tokenized_agent: 평가 모드 토큰 사전입니다.
        anchor_mask: self-forced 학습에 쓰는 agent 마스크입니다. shape은 ``[n_agent]`` 입니다.

    Returns:
        Dict[str, Tensor]: ``agent_type``, ``agent_length``, ``prev_control``,
            ``prev_control_valid`` 를 담은 사전입니다. 첫 차원은 모두 ``n_valid_agent`` 입니다.
    """
    prev_control, prev_control_valid = build_anchor0_prev_control(
        tokenized_agent=tokenized_agent,
        anchor_mask=anchor_mask,
    )
    return {
        "agent_type": tokenized_agent["type"][anchor_mask],
        "agent_length": build_anchor0_agent_length(tokenized_agent, anchor_mask).to(
            device=anchor_mask.device,
            dtype=tokenized_agent["ctx_sampled_pos"].dtype,
        ),
