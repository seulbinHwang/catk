...
class SMARTFlowAgentDecoder(SMARTAgentEncoder):
    ...
    def prepare_inference_cache_anchor_batched(
        ...
        """여러 anchor 의 sliding-window cache 를 한 번의 encoder forward 로 생성.

        OCSC 학습에서 ``prepare_inference_cache`` 가 anchor 별로 매번 호출되어
        encoder forward 가 N_anchor 회 반복되는 부분 (~33.8% wallclock) 을 한 번의
        big batched forward 로 압축한다.  각 anchor i 의
        ``[:, hist_start_i : anchor_idx_i + 1]`` 윈도우를 ``step_current_2hz`` 길이로
        front-padding (``valid_mask=False``) 한 뒤 agent 축으로 stack 한다.
        ``valid_mask=False`` padding 은 ``build_temporal_edge`` /
        ``build_map2agent_edge`` / ``build_interaction_edge`` 에서 mask AND 조건으로
        edge 가 빌드되지 않으므로, anchor 별 sequential forward 와 numerically
        동일한 hidden state 가 anchor 의 마지막 (실제) 위치에서 나온다.

        map feature 도 anchor 수만큼 batch-offset 으로 replicate 해 anchor 간
        cross-mix 를 방지.  결과 cache 는 anchor 별로 split 되어 list 로 반환된다
        (dict 구조 / shape 은 단일 ``prepare_inference_cache`` 와 동일).

        Args:
            tokenized_agent: 원본 (전체 시퀀스) tokenized agent 사전.
            map_feature: 지도 인코더 출력.
            anchor_indices: 처리할 anchor index 리스트 (2Hz 기준).
            step_current_2hz: sliding window 길이 (보통 2).
            seq_keys: 시간 축으로 슬라이스할 key 집합.

        Returns:
            List[Dict[str, object]]: anchor 별 cache 사전 리스트.  길이는
            ``len(anchor_indices)`` 와 같고, 각 원소는 단일
            ``prepare_inference_cache`` 반환 dict 와 같은 키를 갖는다.
        """
        ...
        num_graphs_orig = (
            int(tokenized_agent["num_graphs"])
            if "num_graphs" in tokenized_agent
            else int(tokenized_agent["batch"].max().item()) + 1
        )

        # ── Build batched tokenized_agent ────────────────────────────────────
        batched_tokenized: Dict[str, object] = {}
        for k, v in tokenized_agent.items():
            if (
                ...
                for anchor_idx in anchor_indices:
                    hist_start = max(0, int(anchor_idx) + 1 - int(step_current_2hz))
                    window = v[:, hist_start : int(anchor_idx) + 1]
                    pad_len = int(step_current_2hz) - int(window.shape[1])
                    if pad_len > 0:
                        pad_shape = list(window.shape)
                        pad_shape[1] = pad_len
                        # zeros: valid_mask=False (bool zero) 와 numeric 0 모두 적합.
                        pad = torch.zeros(
                            *pad_shape, dtype=window.dtype, device=window.device,
                        )
                        window = torch.cat([pad, window], dim=1)
                    slices.append(window)
                batched_tokenized[k] = torch.cat(slices, dim=0).contiguous()
            elif k == "batch" and isinstance(v, torch.Tensor):
                offset_batches = [v + i * num_graphs_orig for i in range(num_anchors)]
                batched_tokenized[k] = torch.cat(offset_batches, dim=0).contiguous()
            elif k == "num_graphs":
                batched_tokenized[k] = num_graphs_orig * num_anchors
            elif (
                isinstance(v, torch.Tensor)
                and v.dim() >= 1
                and v.shape[0] == n_agent
            ):
                # Per-agent attribute (type, shape, fine history 등) — replicate.
                batched_tokenized[k] = (
                    v.repeat(num_anchors, *([1] * (v.dim() - 1))).contiguous()
                )
            else:
                # Fixed token vocabularies / non-tensor / 다른 shape — share.
                batched_tokenized[k] = v

        # ── Build batched map_feature ────────────────────────────────────────
        pt_token = map_feature.get("pt_token", None)
        n_pl = int(
            (pt_token if pt_token is not None else map_feature["position"]).shape[0]
        )
        batched_map: Dict[str, object] = {}
        for k, v in map_feature.items():
            if k == "batch" and isinstance(v, torch.Tensor):
                offset_batches = [v + i * num_graphs_orig for i in range(num_anchors)]
                batched_map[k] = torch.cat(offset_batches, dim=0).contiguous()
            elif (
                isinstance(v, torch.Tensor)
                and v.dim() >= 1
                and v.shape[0] == n_pl
        ...
        # ── Run encoder ONCE on batched inputs ───────────────────────────────
        big_cache = self._prepare_rollout_cache_impl(
            tokenized_agent=batched_tokenized,
            map_feature=batched_map,
        )

        # ── Split per-anchor ─────────────────────────────────────────────────
        expected_big_n_agent = n_agent * num_anchors
        caches: List[Dict[str, object]] = []
        for i in range(num_anchors):
            start = i * n_agent
            end = (i + 1) * n_agent
            sliced: Dict[str, object] = {}
            for k, v in big_cache.items():
                if k == "n_agent":
                    sliced[k] = n_agent
                elif (
                    isinstance(v, torch.Tensor)
                    and v.dim() >= 1
                    and v.shape[0] == expected_big_n_agent
                ):
                    sliced[k] = v[start:end].contiguous()
                elif isinstance(v, dict):
                    # feat_a_t_dict: layer_idx -> [big_n_agent, step, D]
                    sliced[k] = {
                        layer_idx: (
                            t[start:end].contiguous()
                            if isinstance(t, torch.Tensor) and t.dim() >= 1 and t.shape[0] == expected_big_n_agent
                            else t
                        )
                        for layer_idx, t in v.items()
                    }
                elif isinstance(v, list):
                    # categorical_embs: [type_a_emb, shape_emb] — each [big_n_agent, D].
                    sliced[k] = [
                        (
                            t[start:end].contiguous()
                            if isinstance(t, torch.Tensor)
                            and t.dim() >= 1
                            and t.shape[0] == expected_big_n_agent
                            else t
                        )
                        for t in v
                    ]
                else:
                    # Embedding tables (agent_token_emb_*), int scalars 등 — share.
                    sliced[k] = v
            caches.append(sliced)
    ...
    def training_rollout_from_cache(
        ...
        Returns:
            Dict[str, torch.Tensor]: N초 committed rollout 결과입니다.
        """
        return self._rollout_from_cache_impl(
            rollout_cache=rollout_cache,
            tokenized_agent=tokenized_agent,
