...
class EgoGMMAgentDecoder(nn.Module):
    ...
    def __init__(
        ...
        k_ego_gmm: int,
        cov_ego_gmm: ListConfig[float],
        cov_learnable: bool,
    ) -> None:
        super(EgoGMMAgentDecoder, self).__init__()
    ...
    def forward(
        ...
        feat_a = feat_a[ego_mask]
        ego_next_logits = self.gmm_logits_head(feat_a)
        ego_next_poses = self.gmm_pose_head(feat_a).view(*ego_next_logits.shape, 3)
        return {
            # action that goes from [(10->15), ..., (85->90)]
            "ego_next_logits": ego_next_logits[:, 1:-1],  # [n_batch, 16, n_k_ego_gmm]
            "ego_next_poses": ego_next_poses[:, 1:-1],  # [n_batch, 16, n_k_ego_gmm, 3]
            "ego_next_valid": tokenized_agent["valid_mask"][ego_mask][:, 1:-1],
            "ego_next_cov": self.gmm_cov,  # [2], one for pos, one for heading.
            # for step {5, 10, ..., 90} and act [(0->5), (5->10), ..., (85->90)]
            "pred_pos": tokenized_agent["sampled_pos"][ego_mask],  # [n_batch, 18, 2]
            "pred_head": tokenized_agent["sampled_heading"][ego_mask],  # [n_batch, 18]
            "pred_valid": tokenized_agent["valid_mask"][ego_mask],  # [n_batch, 18]
            # for step {5, 10, ..., 90}
            "gt_pos_raw": tokenized_agent["gt_pos_raw"][ego_mask],  # [n_batch, 18, 2]
            "gt_head_raw": tokenized_agent["gt_head_raw"][ego_mask],  # [n_batch, 18]
            "gt_valid_raw": tokenized_agent["gt_valid_raw"][ego_mask],  # [n_batch, 18]
            # or use the tokenized gt
            "gt_pos": tokenized_agent["gt_pos"][ego_mask],  # [n_batch, 18, 2]
            "gt_head": tokenized_agent["gt_heading"][ego_mask],  # [n_batch, 18]
    ...
    def inference(
        ...
        for t in range(n_step_future_2hz):  # 0 -> 15
            ...
            # ! get outputs
            feat_a_now = feat_a_now[ego_mask]
            ego_next_logits = self.gmm_logits_head(feat_a_now)
            ego_next_poses = self.gmm_pose_head(feat_a_now).view(
                *ego_next_logits.shape, 3
            )
            ...
            next_token_idx, next_token_traj_all = sample_next_gmm_traj(
                ...
                ego_next_logits=ego_next_logits,  # [n_batch, n_k_ego_gmm]
                ego_next_poses=ego_next_poses,  # [n_batch, n_k_ego_gmm, 3]
                ego_next_cov=self.gmm_cov,  # [2], one for pos, one for heading.
            ...
            pred_valid[:, n_step][ego_mask] = True
        ...
        out_dict = {
            # action that goes from [(10->15), ..., (85->90)]
            # [n_batch, 16, n_k_ego_gmm]
            "ego_next_logits": torch.stack(ego_next_logits_list, dim=1),
            # [n_batch, 16, n_k_ego_gmm, 3]
            "ego_next_poses": torch.stack(ego_next_poses_list, dim=1),
            "ego_next_valid": pred_valid[ego_mask][:, 1:-1],  # [n_batch, 16]
            "ego_next_cov": self.gmm_cov,  # [2], one for pos, one for heading.
            # for step {5, 10, ..., 90} and act [(0->5), (5->10), ..., (85->90)]
            "pred_pos": pos_a[ego_mask],  # [n_batch, 18, 2]
            "pred_head": head_a[ego_mask],  # [n_batch, 18]
            "pred_valid": pred_valid[ego_mask],  # [n_batch, 18]
            # "pred_idx": pred_idx,  # [n_batch, 18]
            # for step {5, 10, ..., 90}
            "gt_pos_raw": tokenized_agent["gt_pos_raw"][ego_mask],  # [n_batch, 18, 2]
            "gt_head_raw": tokenized_agent["gt_head_raw"][ego_mask],  # [n_batch, 18]
            "gt_valid_raw": tokenized_agent["gt_valid_raw"][ego_mask],  # [n_batch, 18]
