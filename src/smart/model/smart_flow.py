...
class SMARTFlow(LightningModule):
    ...
    def _get_self_forced_lr_schedule_total_epochs(self) -> int:
        """실제로 실행될 self-forced curriculum epoch 수를 반환합니다."""
        trainer = getattr(self, "trainer", None)
        max_epochs = None
    ...
    def _apply_self_forced_lr_schedule_for_current_epoch(self) -> None:
        """Generator와 generated estimator optimizer LR을 현재 curriculum epoch에 맞춥니다."""
        if not self.self_forced_enabled:
            return
    ...
    def _sample_closed_loop_sf_see_all_prefix_steps(self, device: torch.device) -> int:
        """현재 stage까지 누적된 구간 전체에서 EMA pre-roll 길이를 샘플합니다."""
    ...
    def _predict_path_flow_clean_estimate(
        ...
        Args:
            ...
            initial_rollout_state: closed-loop prefix rollout이 끝난 현재 state cache입니다.
                값이 있으면 teacher/generated-estimator도 실제 rollout과 같은 context에서
                clean estimate를 계산합니다.
    ...
    def _run_self_forced_rollout(
    ...
    ) -> tuple[Dict[str, Tensor], Dict[str, Tensor], Tensor, Dict[str, object] | None]:
        ...
        try:
            ...
            return rollout, rollout_tokenized_agent, anchor_mask, initial_rollout_state
        finally:
    ...
    def _training_step_self_forced(self, data, batch_idx):
        ...
        if is_estimator_warmup_active:
            with torch.no_grad():
                (
                    rollout,
                    self_forced_tokenized_agent,
                    anchor_mask,
                    initial_rollout_state,
                ) = self._run_self_forced_rollout(
                    tokenized_map_eval,
