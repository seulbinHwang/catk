...
def _get_last_step_contour(contour_local: Tensor) -> Tensor:
    """비교 대상에서 마지막 coarse 시점 사각형만 꺼냅니다.
    ...
    Returns:
        Tensor:
            마지막 coarse 시점 사각형입니다. shape은 ``[n_agent, 4, 2]`` 입니다.

    Raises:
        ValueError: 예상하지 못한 모양의 비교 대상이 들어오면 발생합니다.
    """
    if contour_local.dim() == 4:
        return contour_local[:, -1]
    if contour_local.dim() == 3:
        return contour_local
    raise ValueError(
...
def match_token_idx_from_local_contour(
    ...
    Args:
        ...
        contour_local: 현재 기준 좌표로 옮긴 마지막 시점 사각형입니다.
            ...
            이전 구현과의 호환을 위해 ``[n_agent, 6, 4, 2]`` 도 받을 수 있으며,
            이 경우 마지막 한 시점만 사용합니다.
    ...
    contour_local = _get_last_step_contour(contour_local)
    token_banks = {
