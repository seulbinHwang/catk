...
class FlowTokenProcessor(TokenProcessor):
    ...
    def _build_control_alignment_filter_mask(
        ...
        """raw와 transition-aligned 위치 차이가 너무 큰 control-space 학습 anchor를 제외합니다.

        vehicle/cyclist의 0.5초 endpoint+substep target은 그 자체로 일정 수준의 distortion을
        포함하지만, GT jump / 비정상 dynamics가 섞인 anchor는 supervision을 망가뜨립니다.
        이 helper는 raw GT와 aligned target 사이의 anchor-window L2 오차가 type별 기준을
        넘는 anchor를 train에서 제외합니다. pedestrian/holonomic target은 raw GT를 그대로
        쓰므로 거리가 0이라 영향이 없습니다.
        """
        ...
        threshold[agent_type_device == VEHICLE_TYPE_ID] = float(
            getattr(self, "control_alignment_filter_vehicle_max_error_m", 6.0)
        )
        threshold[agent_type_device == CYCLIST_TYPE_ID] = float(
            getattr(self, "control_alignment_filter_cyclist_max_error_m", 2.2)
        )
