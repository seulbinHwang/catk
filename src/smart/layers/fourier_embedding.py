        ...
        try:
            if self._compiled_embed_continuous is None:
                self._compiled_embed_continuous = torch.compile(
                    ...
                    dynamic=True,
                    # SMART graph edge counts vary by batch. Keep Inductor
                    # compilation, but avoid CUDA graph capture stale storage
                    # failures in long-running sparse graph training.
