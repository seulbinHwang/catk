...
def _reference_graph_attention(
    ...
    CUDA 학습에서는 이 경로를 쓰지 않습니다. CPU unit test에서는 이 함수로 수식
    일치성을 확인합니다.
...
def _load_cuda_segmented_impl():
    ...
    try:
        from src.smart.layers.segmented_graph_attention_cuda import graph_cuda_segmented_attention
...
def segmented_graph_attention(
    ...
    CUDA에서는 기본적으로 CAT-K 전용 segmented CUDA backend를 사용합니다.
    이 경로는 target node 기준으로 정렬된 edge를 읽고, score/softmax/value
    aggregation을 CUDA extension에서 처리합니다. CPU에서는 unit test를 위한
    reference 경로만 제공합니다.
    ...
    if q.is_cuda:
        ...
        raise ValueError(
            "CATK_GRAPH_ATTENTION_BACKEND must be one of cuda_segmented or torch_reference, "
            f"got {backend!r}."
        )
