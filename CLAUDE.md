...
**Train-time HardRMM 미지원**:  DMD step 은 closed-loop rollout + 두 backward 만 으로도
충분히 무겁고, in-process HardRMM 은 validation 단계의 RMM (`val_closed/sim_agents_2025/realism_meta_metric`) 으로 신뢰성 있게 측정.

**Logged keys (wandb)**:  `train/dmd/{gen_loss, fake_loss, score_diff_norm, v_real_norm,
v_fake_norm, normalizer_mean, beta, n_valid_anchors, skip_gen}` + `train/loss` alias.
